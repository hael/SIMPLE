...
type, extends(commander_base) :: commander_oriops
  contains
    procedure :: execute      => exec_oriops
end type commander_oriops
...
contains
    ...
    subroutine exec_oriops( self, cline )
        class(commander_oriops), intent(inout) :: self
        class(cmdline),           intent(inout) :: cline
        type(parameters) :: params
        type(builder)    :: build
        type(ori)        :: orientation
        ...
        if( cline%defined('mirr') )then
            select case(trim(params%mirr))
                ...
                case('no')
                    ! nothing to do
                case DEFAULT
                    THROW_HARD('mirr flag: '//trim(params%mirr)//' is unsupported; exec_oriops')
            end select
        endif
        call binwrite_oritab(params%outfile, build%spproj, build%spproj_field, [1,build%spproj_field%get_noris()])
        call simple_end('**** SIMPLE_oriops NORMAL STOP ****')
    end subroutine exec_oriops
    ...
    subroutine exec_vizoris( self, cline )
        ...
        if( params%tseries.eq.'no' )then
            ...
            call fopen(funit, status='REPLACE', action='WRITE', file=fname,iostat=io_stat)
             if(io_stat/=0)call fileiochk("simple_commanders_ori::exec_vizoris fopen failed "//fname%to_char(), io_stat)
            ! header
            ...
            radius = 0.02
            call fopen(funit, status='REPLACE', action='WRITE', file=fname, iostat=io_stat)
             if(io_stat/=0)call fileiochk("simple_commanders_ori::exec_vizoris fopen failed ", io_stat)
            write(funit,'(A)')".translate 0.0 0.0 0.0"
...
end module simple_commanders_ori
