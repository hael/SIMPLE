...
// C = alpha*A*B + beta*C
extern "C" __global__ void
main_zz2dgemm_kernel_N_T( double *C, 
			  ...
			  double alpha, double beta)
{
  ...
  C[i * ldc + j ] = cuDmul(alpha, cuReCCstarmul( A[ i * lda + j], B[ j * ldb + i] ));
		     ...
		     double alpha, double beta)
{
  ...
  main_zz2dgemm_kernel_N_T<<< grid, threads >>>(C, A, B, 
						m, n,
						lda, ldb, ldc,
						alpha, beta);
