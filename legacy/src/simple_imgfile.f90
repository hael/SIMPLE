...
public :: imgfile, get_mrcfile_info, get_spifile_info
private
...
contains
    ...
    procedure          :: setPixSz
    procedure          :: setMode
    procedure          :: setFileHead
end type

! CLASS VARIABLES
character(len=20) :: endconv = 'NATIVE' !< endian conversion string
...
contains
    ...
    subroutine open( self, fname, mould, ldim, smpd, del_if_exists, ldim_out )
        use simple_jiffys, only: alloc_err, file_exists
        class(imgfile), intent(inout)          :: self          !< Imagefile object to be created
        character(len=*), intent(in)           :: fname         !< Filename
        type(imgfile),intent(in), optional     :: mould         !< An imagefile object, which the new object will be exists to resemble.
        integer, intent(in), optional          :: ldim(3)       !< logical dimension of image/stack
        real, intent(in),    optional          :: smpd          !< Pixel size of image data (in Angstroms)
        logical, intent(in), optional          :: del_if_exists !< If the file already exists on disk, replace it
        integer, intent(out), optional         :: ldim_out(3)   !< logical dimension outputted
        character(len=1)   :: format_descriptor
        integer, parameter :: box_from=20, box_to=1000
        integer            :: alloc_stat, lldim(3), i, j, k, maxim, iform, ldim_read(3), mode
        real               :: ssmpd, smpd_read
        logical            :: founddim, didset
        call self%close_nowrite 
        lldim = 0 
        if( present(ldim) )then
            lldim = ldim
            if( any(lldim == 0) )then
                write(*,*) 'Inputted logical dimension: ', lldim
                stop 'Logical dimensions cannot be zero; open; simple_imgfile'
            endif
        endif
        ssmpd = 1.
        ...
        ! Work out which file format the user wants to use.
        ! First priority is given to any extension present in the fname.
        ! Next, we check if a mould was given
        ! Otherwise, we revert to the default from the runtime parameters.
        self%head_format = default_file_format ! this comes from simple_def
        format_descriptor = self%get_format()
        if( format_descriptor .ne. 'N' )then
            self%head_format = format_descriptor
        else
            if( present(mould) ) self%head_format = mould%head_format
        endif
        ! Allocate head object and more
        select case(self%head_format)
            case ('M')
                allocate(MrcImgHead :: self%overall_head)
                if( file_exists(fname) )then
                    call get_mrcfile_info( fname, ldim_read, mode, smpd_read, .false. )
                    if( any(ldim_read < 1) )then
                        write(*,*) 'Logical dimension read from stack header: ', ldim_read
                        write(*,*) 'Smpd read from stack header: ', smpd_read
                        write(*,*) 'Mode read from stack header: ', mode
                    endif
                    if( .not. present(ldim) ) lldim = ldim_read
                    if( .not. present(smpd) ) ssmpd = smpd_read
                endif
                if( present(ldim) .or. file_exists(fname) )then
                    call self%overall_head%new(lldim)
                else
                    call self%overall_head%new
                endif
            case ('S')
                ! create header
                allocate(SpiImgHead :: self%overall_head)                
                if( file_exists(fname) )then
                    ! we have to get this info for SPIDER files, so if the file doesn't exists 
                    ! there will be an exception rasied
                    call get_spifile_info( fname, ldim_read, iform, maxim, smpd_read, endconv, .false. )
                    if( any(ldim_read < 1) )then
                        write(*,*) 'Logical dimension read from stack header: ', ldim_read
                        write(*,*) 'Format description (iform): ', iform
                        write(*,*) 'Maxim read from stack header: ', maxim
                        write(*,*) 'Smpd read from stack header: ', smpd_read
                    endif
                    if( .not. present(ldim) ) lldim = ldim_read
                    if( .not. present(smpd) ) ssmpd = smpd_read
                ...
                if( present(ldim) .or. file_exists(fname) )then
                    ...
                    stop 'need ldim info to create a SPIDER file header; open; simple_imgfile'
                endif
                if( lldim(3) > 1 ) self%isvol = .true.
                if( present(ldim) )then
                    call self%overall_head%setDims(lldim)
                    didset = .true.
                endif
            case DEFAULT
                stop 'Unsupported file format; new; simple_imgfile' 
        end select
        if( present(ldim_out) ) ldim_out = lldim
        ! open the file
        call self%open_local(del_if_exists)
        ! If the file exists, read the head, if not, setup the head with default values and the given file dims
        if( self%exists() )then
        ...
        if( present(mould) )then
            self%overall_head = mould%overall_head
            didset = .true.
        endif
        if( present(smpd) )then
            call self%overall_head%setPixSz(ssmpd)
            didset = .true.
        endif
        if( (.not. self%exists()) .or. didset )then
            ! write new header to disk since it may be used in following code
    ...
    subroutine get_mrcfile_info( fname, ldim, mode, smpd, doprint, funit )
        use simple_jiffys, only: file_exists, fname2format, get_fileunit
        character(len=*), intent(in)  :: fname
        integer, intent(out)          :: ldim(3), mode
        ...
        integer :: filnum
        if( file_exists(fname) )then    
            if( fname2format(fname) .eq. 'M' )then
    ...
    subroutine get_spifile_info( fname, ldim, iform, maxim, smpd, conv, doprint, funit )
        use simple_jiffys, only: file_exists, fname2format, get_fileunit
        character(len=*), intent(in)   :: fname
        integer, intent(out)           :: ldim(3), maxim, iform
        real, intent(out)              :: smpd
        character(len=20), intent(out) :: conv
        logical, intent(in)            :: doprint
        integer, intent(in), optional  :: funit
        real    :: spihed(40)
    ...
    pure function get_format( self )
        ...
        get_format = fname2format(self%fname)
    end function
    ...
    subroutine rwSlices( self, mode, first_slice, last_slice, rarr, ldim, is_ft, smpd, force_read )
        use simple_jiffys, only: int2str
        use simple_math, only: is_odd
        use, intrinsic :: iso_c_binding
        use simple_imghead, only: ImgHead, SpiImgHead, dataRbytes, dataRinteger, dataRfloat
        ...
        logical, intent (in), optional        :: force_read   !< whether to force read or not        
        ...
        ! Conformance check the formats & shape of input array
        ptr => self%overall_head
        select type(ptr)
            type is (MrcImgHead)
                ! all good
...
!             stop 'Array is not properly allocated; rwSlices; simple_imgfile'
        ...
        ! force read
        if( present(force_read) )then
            if( force_read )then
                if( present(ldim) )then
                    dims = ldim
                else
                    stop 'need optional ldim input to force read; simple_imgfile :: rwSlices'
                endif
            endif
        endif
    ...
    integer function getStackSz( self )
        class(imgfile), intent(in) :: self
        getStackSz = self%overall_head%getStackSz()
    end function
    ...
    !>  \brief  Set the format descriptor of the stack
    subroutine setFileHead( self, ldim, smpd )
        class(imgfile), intent(inout) :: self
        integer, intent(in)           :: ldim(3)
        real, intent(in)              :: smpd
        call self%overall_head%setMinimal(ldim, .false., smpd)
