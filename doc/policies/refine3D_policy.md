...
`envref=yes` replaces the ordinary spherical reference mask with the current
compatible state automask. The selected regular or NU-derived reference is
background-zeroed using the mask transition and multiplied by the mask before
projection. The option does not modify particles, reconstruction volumes on
disk, FSC curves, NU filtering, or matching-bandwidth selection. Missing or
incompatible state masks fall back to spherical reference masking for that
iteration.
