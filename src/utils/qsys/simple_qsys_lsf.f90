...
contains
    ...
    subroutine write_lsf_array_header( self, q_descr, parts_fromto, fhandle, nactive )
        ...
        if( write2file )then
            write(fhandle,'(a)') '#BSUB -oo ' // trim(stderrout)//'outfile.%J_%I'
            write(fhandle,'(a)') '#BSUB -eo ' // trim(stderrout)//'errfile.%J_%I'
            ...
            write(logfhandle,'(a)') '#BSUB -oo ' // trim(stderrout)//'outfile.%J_%I'
            write(logfhandle,'(a)') '#BSUB -eo ' // trim(stderrout)//'errfile.%J_%I'
            if( present(nactive) )then
