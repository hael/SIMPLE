...
contains
    ...
    subroutine div_by_instrfun( self )
        ...
        if( self%is_ft() ) THROW_HARD('Polarizer must be in real-space in div_by_instrfun')
        if( .not.(self.eqdims.self%instrfun_img) ) THROW_HARD('Incompatible dimensions in div_by_instrfun')
