...
contains
    ...
    subroutine exec_stream_p03_initial_analysis( self, cline )
        ...
        integer,                   parameter       :: NMICS_PLAN(3) = [10, 20, 50]
        ...
        type(string)                               :: cavgsstk, mapfileprefix, projfile, cwd_master, cwd_cycle
        type(qsys_env)                             :: qsys
        type(cmdline)                              :: cline_extract, cline_abinitio2D, cline_shape_rank, cline_reextract, cline_abinitio3D
        ...
        integer                                    :: ncls_stk, iori, i_max, i_start, optics_map_id, i_cluster, i_mic
        ...
        integer                                    :: maxbox_in_pix          ! box size (px) corresponding to largest diameter cluster, for IPC routing
        ...
        do
            ...
            call simple_mkdir('cycle_'//int2str(n_increase_cycles + 1))
            call simple_chdir('cycle_'//int2str(n_increase_cycles + 1))
            ...
            call qsys%kill()
            maxbox_in_pix = maxval(boxes_in_pix) ! track largest box size for IPC routing
            do i_cluster = 1, size(projs_clusters)
                call run_extract(projs_clusters(i_cluster), string('extract_cluster_'//int2str(i_cluster)), maxbox_in_pix)
            end do
            call spproj%kill()
            call spproj%read(projs_clusters(1))
            call merge_selected_project_files(projs_clusters, basename(projfile), spproj) ! merge the cluster-specific project files for abinitio2D input; these contain only the particles from each cluster, with mic and particle metadata updated to reflect the new coordinates after extraction
            call spproj%write(basename(projfile))
            call send_meta(string('complete'))
            call send_meta2D(string('classifying particles'), maxval(boxes_in_pix))
            call run_abinitio2D(basename(projfile), string('abinitio2D'), nint(maxval(mskdiams)))
            ...
            call spproj%kill()
            call spproj%read(basename(projfile)) ! read the full project with all clusters merged in, to get the complete set of class averages for quality evaluation and user selection
            ...
            call evaluate_cavg_quality(cavg_imgs, spproj%os_cls2D, maxval(mskdiams), quality, model)
            call model%kill()
            ...
            do i_mic = spproj%os_mic%get_noris(), 1, -1
                if( spproj%os_mic%get(i_mic, 'state') == 0.0 .or. spproj%os_mic%get(i_mic, 'nptcls') == 0.0 ) then
                    call spproj%os_mic%delete(i_mic)
                endif
            end do
            call spproj%write()
            call spproj%read(basename(projfile)) ! read the full project with all clusters merged in, to get the complete set of class averages for user selection
       ...
       !         end if
       !     end if
            if( n_increase_cycles >= size(NMICS_PLAN) - 1 ) then
                call run_abinitio3D(basename(projfile), string('abinitio3D'), nint(maxval(mskdiams)))
            else
            ...
            if( n_increase_cycles < size(NMICS_PLAN) - 1 ) then
                n_increase_cycles = n_increase_cycles + 1
                params%nmics      = NMICS_PLAN(n_increase_cycles + 1)
                call cline%set('nmics', params%nmics)
                write(logfhandle, '(A,I0,A,I0)') '>>> ADVANCING TO PLAN STEP ', n_increase_cycles + 1, ' WITH NMICS=', params%nmics
       ...
       !     end if
                    ...
                    if( mq_stream_master_out%receive(meta_buffer) ) then
                        if( allocated(meta_buffer) ) then
                            ...
                            if( ncavgs > 0 ) then
                                ...
                                !    &cwd_cycle//'/'//STREAM_SELECTED_REFS//JPG_EXT, ncavgs, cluster_id=0)
        ...
        contains
            ...
            subroutine run_extract( cluster_projfile, outdir, box )
            ...
            ! Reextract particles using one thread per partition (nthr partitions).
            subroutine run_reextract( cluster_projfile, outdir, box )
                type(string), intent(in) :: cluster_projfile
                type(string), intent(in) :: outdir
                integer,      intent(in) :: box
                type(string)             :: cwd
                ...
                call cline_reextract%set('outdir',              outdir)
            ...
            subroutine run_abinitio3D( cluster_projfile, outdir, mskdiam )
                type(string), intent(in) :: cluster_projfile
                type(string), intent(in) :: outdir
                integer,      intent(in) :: mskdiam
                integer,     allocatable :: volpops(:), states(:)
                type(sp_project)         :: spproj_cluster
                type(cmdline)            :: cline_reproject
                type(string)             :: cwd, volpath
                type(oris)               :: voloris
                integer                  :: ldim(3)
                integer                  :: ldim_clip(3)
                real                     :: smpd_new, pickrefs_thumbnail_scale
                integer                  :: pickrefs_xtiles, pickrefs_ytiles, n_pickrefs
                integer :: nptcls, ncls, nthr2D, ivol, bestvol
                call simple_getcwd(cwd)
                call spproj_cluster%kill()
                ...
                call cline_abinitio3D%set('lpstop',                  LPSTOP)
                ...
                ldim_clip = spproj%os_stk%get_int(1, 'box') ! pad to particle box size
                ...
                call cline_reproject%set('mskdiam',                   mskdiam)
                call cline_reproject%printline()
                call xreproject%execute(cline_reproject)
                call mrc2jpeg_tiled(string('reprojs.mrcs'), string("reprojs.jpeg"),&
                        &scale=pickrefs_thumbnail_scale, ntiles=n_pickrefs, n_xtiles=pickrefs_xtiles, n_ytiles=pickrefs_ytiles)
               ! call scale_and_clip_imgfile(volpath, string('reprojs'//MRC_EXT), voloris%get(bestvol, 'smpd'), ldim, ldim_clip, smpd_new )
                deallocate(states)
                deallocate(volpops)
                call voloris%kill()
