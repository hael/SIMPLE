...
!  Absolute difference vs. fast 2D:    0.00000000    
!  % time 2D:    54.396864334451770     
!  Pearson correlation vs. fast 3D:    1.00000000    
!  Absolute difference vs. fast 3D:    1.73752734E-09
!  % time 3D:    83.908570931462307 
...
do k = -BOX,0
    ...
    do h = -BOX,BOX
        ...
        enddo
        kbw   = kbw / sum(kbw)
        rt_kb = rt_kb + toc(t_kb)
        ! fast kernel
        t_kb_fast = tic()
        call kbwin%apod_mat_2d(loc, win(1,1), win(1,2), wdim, kbw_fast)
        rt_kb_fast = rt_kb_fast + toc(t_kb_fast)
...
do k = -BOX,0
    do h = -BOX,BOX
        ...
        rt_kb = rt_kb + toc(t_kb)
         ! fast kernel
        t_kb_fast = tic()
        call kbwin%apod_mat_3d(loc3D, win3D(1,1), win3D(1,2), win3D(1,3), wdim, kbw3D_fast)
        rt_kb_fast = rt_kb_fast + toc(t_kb_fast)
        ! accumulate absolute differences
