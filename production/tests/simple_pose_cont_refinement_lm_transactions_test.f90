...
! Production Cholesky is compared with pivoted Gaussian elimination and LAPACK;
! every nonaccepted terminal outcome must preserve the complete input pose.
...
!! Reliable fixtures test (H+mu D)z=-g and the gain-ratio decision. Physical
!! fixtures separately test accepted traces, bounds, terminal states, and rollback.
...
!> Compare scaling, damping, dense solution, step, reductions, ratio, and decision.
    subroutine test_reliable_system(name, hessian, gradient, target_ratio, expect_bounded, &
        ...
        ! Form and solve the same scaled damped system through two independent paths.
        ...
        ! Quadratic model: predicted=-g^T d-1/2 d^T H d; rho=actual/predicted.
...
!> Solve the scaled damped system with pivoted Gaussian elimination.
    subroutine independent_lm_system(hessian, gradient, scaled_gradient, scaled_hessian, diagonal, &
        ...
        ! LM interpolation is explicit here: A=H_scaled+mu*D.
