!@descr: fixed-record sparse profiled-posterior artifact for 3D refinement
module simple_prob_posterior3D
use, intrinsic :: iso_fortran_env, only: int64
use simple_error, only: simple_exception
implicit none

public :: posterior3d_writer, posterior3d_reader
public :: POSTERIOR3D_FNAME, POSTERIOR3D_MAGIC, POSTERIOR3D_VERSION, POSTERIOR3D_MAP_VERSION
private
#include "simple_local_flags.inc"

character(len=*), parameter :: POSTERIOR3D_FNAME = 'posterior_support3d.dat'
integer(int64),   parameter :: POSTERIOR3D_MAGIC = 731984201_int64
integer,          parameter :: POSTERIOR3D_VERSION = 2
integer,          parameter :: POSTERIOR3D_MAP_VERSION = 2
integer,          parameter :: POSTERIOR3D_PGRP_LEN = 64
integer,          parameter :: POSTERIOR3D_LABEL_LEN = 32

type :: posterior3d_writer
    integer :: unit = -1
...
end type posterior3d_writer

type :: posterior3d_reader
    integer :: unit = -1
    logical :: is_open = .false.
    integer :: recl = 0
    integer :: kmax = 0
    ...
    integer :: map_version = 0
    real :: support_mass = 0.
    character(len=POSTERIOR3D_PGRP_LEN) :: pgrp = ''
    character(len=POSTERIOR3D_LABEL_LEN) :: likelihood_transform = ''
    character(len=POSTERIOR3D_LABEL_LEN) :: support_convention = ''
...
subroutine open_writer(self, binfname, nptcls, nstates, source_nspace, source_nspace_sub, kmax, pgrp, pinds)
    ...
    integer(int64) :: magic
    integer :: version, map_version
    character(len=POSTERIOR3D_PGRP_LEN) :: pgrp_buf
    character(len=POSTERIOR3D_LABEL_LEN) :: likelihood_buf, convention_buf
    call self%kill
    if( nptcls < 1 .or. size(pinds) /= nptcls ) THROW_HARD('posterior3d_writer: invalid particle count')
    if( kmax < 1 ) THROW_HARD('posterior3d_writer: invalid support width')
    self%nptcls = nptcls
    self%kmax   = kmax
    self%nsel = 0
    ...
    if( ios /= 0 ) THROW_HARD('posterior3d_writer: failed to open data file '//trim(binfname))
    ...
    if( ios /= 0 )then
        ...
        self%unit = -1
        self%is_open = .false.
        THROW_HARD('posterior3d_writer: failed to open metadata file '//trim(metafname))
    endif
    magic = POSTERIOR3D_MAGIC
    version = POSTERIOR3D_VERSION
    map_version = POSTERIOR3D_MAP_VERSION
    pgrp_buf = ''
    ...
    convention_buf = 'sparse_profiled_posterior'
    write(meta_unit, iostat=ios) magic, version, map_version, nptcls, nstates, source_nspace, source_nspace_sub,&
        &kmax, pgrp_buf, likelihood_buf, convention_buf, pinds
    close(meta_unit)
    if( ios /= 0 ) THROW_HARD('posterior3d_writer: failed writing metadata file '//trim(metafname))
    self%is_open = .true.
...
subroutine write_row(self, pind, nsel, support_mass, state, proj, inpl, has_sh, dist, weight, x, y, euls)
    ...
    integer :: ios, n
    if( .not. self%is_open ) THROW_HARD('posterior3d_writer: data file is not open')
    ...
    write(self%unit, rec=pind, iostat=ios) self%nsel, self%support_mass, self%state, self%proj, self%inpl, self%has_sh,&
        &self%dist, self%weight, self%x, self%y, self%euls
    if( ios /= 0 ) THROW_HARD('posterior3d_writer: failed writing particle record')
...
subroutine open_reader(self, binfname, ok)
    class(posterior3d_reader), intent(inout) :: self
    character(len=*),      intent(in)    :: binfname
    ...
    open(newunit=meta_unit, file=trim(metafname), status='OLD', action='READ', access='STREAM',&
        &form='UNFORMATTED', iostat=ios)
    if( ios /= 0 ) return
    read(meta_unit, iostat=ios) magic, version, self%map_version, self%nptcls, self%nstates, self%source_nspace,&
        &self%source_nspace_sub, self%kmax, pgrp_buf, likelihood_buf, convention_buf
    if( ios /= 0 .or. magic /= POSTERIOR3D_MAGIC .or. version /= POSTERIOR3D_VERSION .or.&
        &self%map_version /= POSTERIOR3D_MAP_VERSION .or. trim(likelihood_buf) /= 'profile_exp_minus_dist' .or.&
        &trim(convention_buf) /= 'sparse_profiled_posterior' .or.&
...
end module simple_prob_posterior3D
