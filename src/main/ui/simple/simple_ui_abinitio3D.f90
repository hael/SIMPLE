...
contains
    ...
    subroutine new_abinitio3D( prgtab )
        ...
        call abinitio3D%add_input(UI_FILT, 'filt_mode', 'multi', 'Filtering mode', &
            &'Filtering mode(none|uniform|fsc|nonuniform|nonuniform_lpset){none}; nonuniform applies staged nonuniform filtering', &
            &'(none|uniform|fsc|nonuniform|nonuniform_lpset){none}', .false., 'none', gui_submenu="filter", gui_advanced=.true.)
