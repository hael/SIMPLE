...
| continuous_inplane_cc_grad | S | 231 | error stop | not run | user-supplied | - | continuous_inplane_hybrid_grad (67%); continuous_inplane_rotation2D_stage1_validation (54%) | manual (needs user-supplied) | merge into unit_pftc_registration2D3D | Hans, 2026-09-23: `continuous in-plane` sub-suite (`simple_pftc_inplane_tester`), hermetic phantom instead of vol1; grid identity, FD gradient, degenerate-denominator penalty kept |
| continuous_inplane_hybrid_grad | S | 208 | error stop | not run | user-supplied | - | continuous_inplane_cc_grad (67%); continuous_inplane_rotation2D_stage1_validation (52%) | manual (needs user-supplied) | merge into unit_pftc_registration2D3D | Hans, 2026-09-23: as `continuous_inplane_cc_grad`; capability flags, grid identity, FD gradient kept |
| continuous_inplane_refine3D | S | 972 | error stop | not run | user-supplied | - | phshift_policy (12%, subset) | manual (needs user-supplied) | merge into unit_pftc_registration2D3D | Hans, 2026-09-23: search_state/joint_state/direct_route/metadata_state/policy became the `refine3D in-plane state` sub-suite (`simple_strategy3D_inplane_tester`), synthetic_recovery joined `continuous in-plane`; the metadata_project and baseline cases (finished project + TSV snapshot) deleted, see Retired tests |
...
| continuous_inplane_cc_grad | 2026-09-23 | merge into unit_pftc_registration2D3D: needed vol1; cc evaluator grid identity, FD gradient, penalty | `simple_pftc_inplane_tester` (`continuous in-plane`) |
| continuous_inplane_hybrid_grad | 2026-09-23 | merge into unit_pftc_registration2D3D: needed vol1; hybrid evaluator | as `continuous_inplane_cc_grad` |
| continuous_inplane_rotation2D_stage1_validation | 2026-09-23 | merge into unit_pftc_registration2D3D: needed vol1; three fixtures x three cases of parity, gradient, stress and recovery, aliasing printed only | as `continuous_inplane_cc_grad` (fixtures reduced to truth 37 deg with shift, 359.375 deg hard-edge full band, 0 deg) |
| continuous_inplane_rotation2D_route_identity | 2026-09-23 | merge into unit_pftc_registration2D3D: needed vol1; route identities, seed parity, strategy2D route flags, hybrid rejection via child process | as `continuous_inplane_cc_grad`; the rejection is not asserted |
| continuous_inplane_refine3D | 2026-09-23 | merge into unit_pftc_registration2D3D: child-process driver over seven cases; two needed a finished project | `simple_strategy3D_inplane_tester` (`refine3D in-plane state`) and `continuous in-plane` (synthetic recovery) |
