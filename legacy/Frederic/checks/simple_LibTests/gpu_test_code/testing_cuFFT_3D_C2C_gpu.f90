...
! ============================================================================
!
program testing_cuFFT_3D_C2C_gpu
...
!  real(sp) :: xi,xf
  ...
  integer :: nx,ny,nz
  !complex(sp)
  complex(sp) :: ui,uf,vi,vf,wi,wf
  complex(sp),allocatable :: u(:),v(:),w(:),fuvw(:,:,:)
...
!  call t_cuFFT_gpu%new_cuFFT_3D_CC_gpu(nx,ny,nz)

  write(*,*)'                                                               '
  write(*,*)'************** GPU 3D cuFFT C2C -> C2C ************************'
  write(*,*)'                                                               '
  ...
  wi = cmplx(-10.0, (4.0 * atan(1.0))**(1/b) )
  wf = cmplx( 10.0, (4.0 * atan(1.0))**(1/b) )
  ...
  fhpq = 0.0
  call t_cuFFT_gpu%gather_fft_3D_C2C_gpu(nx,ny,nz,fuvw,fhpq,CUFFT_FORWARD)

  fuvw_gpu = 0.0
  call t_cuFFT_gpu%gather_fft_3D_C2C_gpu(nx,ny,nz,fhpq,fuvw_gpu,CUFFT_INVERSE)
...
end program testing_cuFFT_3D_C2C_gpu
