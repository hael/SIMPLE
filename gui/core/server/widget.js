...
class Widget{
  ...
  constructor(){
	  this.filetabgeneratorwidget = pug.compileFile(global.appPath + '/core/client/filetabgeneratorwidget.pug')
	  this.guinierwidget = pug.compileFile(global.appPath + '/core/client/guinierplotwidget.pug')
  }
  ...
  getGuinierWidget(arg){
	return simpleexec.getProjectVals(arg['file'], 'out', 'vol,smpd')
	.then(result => {
		var volumes = []
		for(var line of result.stdout.split("\n")){
			var elements = line.split((/[ ]+/))
			if(elements.length == 6 && elements[3] != "0.0000"){
