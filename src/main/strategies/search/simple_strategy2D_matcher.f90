...
use simple_matcher_ptcl_batch,      only: prep_sigmas_objfun, prep_batch_particles2D, build_batch_particles2D, clean_batch_particles2D
...
public :: prep_pftc4align2D, prep_pftc4align2D_polar
...
contains
    ...
    subroutine cluster2D_exec( params, build, cline, which_iter, converged )
        ...
        logical :: l_partial_sums, l_update_frac, l_ctf, l_snhc
        ...
        if( p_ptr%l_prob_align_mode )then
            ! sampling is generated by prob_align2D and reproduced here
            call b_ptr%spproj_field%sample4update_reprod([p_ptr%fromp,p_ptr%top], nptcls2update, pinds)
        else
            call sample_ptcls4update2D(p_ptr, b_ptr, [p_ptr%fromp,p_ptr%top], l_update_frac, nptcls2update, pinds)
        endif
        ...
        if( L_BENCH_GLOB )then
            ...
            rt_build_batch_particles2D = 0.
            rt_prep_strategy2D_batch   = 0.
        ...
        if( p_ptr%l_polar .and. which_iter>1)then
            ...
            call prep_pftc4align2D_polar(batchsz_max, which_iter, l_stream)
        else
            ! Cartesian references
            call prep_pftc4align2D(batchsz_max, which_iter, l_stream)
        endif
        if( L_BENCH_GLOB ) rt_prep_pftc_refs2D = toc(t_prep_pftc_refs2D)
        ...
        ! read probabilistic assignment map (generated by prob_align2D)
        if( p_ptr%l_prob_align_mode )then
        ...
        do ibatch=1,nbatches
            ...
            if( L_BENCH_GLOB ) t_build_batch_particles2D = tic()
            ...
            if( L_BENCH_GLOB ) rt_build_batch_particles2D = rt_build_batch_particles2D + toc(t_build_batch_particles2D)
            ! batch strategy2D objects
            if( L_BENCH_GLOB ) t_prep_strategy2D_batch = tic()
            call prep_strategy2D_batch( p_ptr, b_ptr%spproj, which_iter, batchsz, pinds(batch_start:batch_end) )
            if( L_BENCH_GLOB ) rt_prep_strategy2D_batch = rt_prep_strategy2D_batch + toc(t_prep_strategy2D_batch)
            ...
            do iptcl_batch = 1,batchsz                     ! particle batch index
                ...
                if( p_ptr%l_prob_align_mode ) strategy2Dspec%eulprob_obj_part2D => eulprob_obj_part
        ...
        if( p_ptr%l_prob_align_mode ) call eulprob_obj_part%kill
        ...
        if( L_BENCH_GLOB )then
            if( p_ptr%part == 1 )then
                ...
                write(fnr,'(a,1x,f9.2)') '% accounted for      : ',&
                    ...
                    &rt_prep_strategy2D_batch+&
                    &rt_align+rt_cavg+rt_projio)/rt_tot) * 100.
                call fclose(fnr)
            endif
        endif
    ...
    subroutine prep_pftc4align2D( batchsz_max, which_iter, l_stream )
        ...
        call prep_sigmas_objfun(p_ptr, b_ptr, l_stream)
    ...
    subroutine prep_pftc4align2D_polar( batchsz_max, which_iter, l_stream )
