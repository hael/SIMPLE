...
contains
    ...
    subroutine srch_cluster3D( self )
        ...
        if( self%s%prev_state > 0 )then
            ...
            if( pftcc_glob%objfun_is_ccres() )call pftcc_glob%memoize_bfac(self%s%iptcl, bfac)
            ! specscore
                ...
                if( self%spec%do_extr )then
                    ...
                    if( mi_state > 0.5 )then
                        if( hetsym )then
                            ...
                            if( self%s%prev_roind .ne. pftcc_glob%get_roind(360.-s3D%proj_space_euls(self%s%iptcl_map, iref, 3)) )mi_inpl = 0.
                        endif
                    endif
