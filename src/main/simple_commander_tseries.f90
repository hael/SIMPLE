    ...
    subroutine exec_autorefine3D_nano( self, cline )
        ...
        do i=1,MAXITS
            ...
            if( i == 1 )then
                ...
                call cline_refine3D_nano%set('mskfile', MSK)
            endif
            ...
            ! stash away the in-between model building maps generated by refine3D_nano
            maps_dir = 'maps_iter_'//int2str(i)
            call simple_mkdir(maps_dir)
            call simple_list_files('recvol_state01_iter*', map_names)
            do j = 1, size(map_names)
                call simple_copy_file(trim(map_names(j)), './'//maps_dir//'/'//trim(map_names(j)))
            end do
            ! model building
            call xdetect_atms%execute(cline_detect_atms)
            ! copy critical output
            if( .not. dir_exists('intermediate_results') ) call simple_mkdir('intermediate_results')
            call simple_copy_file(RECVOL,   './intermediate_results/'//trim(fbody)      //'_iter'//int2str_pad(i,3)//'.mrc')
            call simple_copy_file(SIMVOL,   './intermediate_results/'//trim(fbody)      //'_iter'//int2str_pad(i,3)//'_SIM.mrc')
            call simple_copy_file(ATOMS,    './intermediate_results/'//trim(fbody)      //'_iter'//int2str_pad(i,3)//'_ATMS.pdb')
            call simple_copy_file(BINARY,   './intermediate_results/'//trim(fbody)      //'_iter'//int2str_pad(i,3)//'_BIN.mrc')
            call simple_copy_file(CCS,      './intermediate_results/'//trim(fbody)      //'_iter'//int2str_pad(i,3)//'_CC.mrc')
            call simple_copy_file(MSK,      './intermediate_results/'//trim(fbody)      //'_iter'//int2str_pad(i,3)//'_MSK.mrc')
            call simple_copy_file(SPLITTED, './intermediate_results/'//trim(fbody_split)//'_iter'//int2str_pad(i,3)//'.mrc')
            ! clean
        ...
        call simple_copy_file(MSK,      './final_results/'//trim(fbody)      //'_iter'//int2str_pad(i,3)//'_MSK.mrc')
