...
retired tests are `doc/code_overview/test_review_record.md`. This was a large
...
This record is closed. It keeps the design, the migration and the batch
records (section 9.7) as they happened; it is not updated any more. New work on
the test environment follows the policy, and Phase 5 is recorded in its
handover.
...
the retired-tests table, is `doc/code_overview/test_review_record.md`,
which the generator reads and which is updated in the same commit as the
verdicts it records.
`scripts/test_review_dossier.py --coverage-after doc/code_overview/test_review_record.md`
...
moved to the committed `test_review_record.md`, which the generator reads;
...
- *Not fully attainable, by design:* 8. `BUILD_TESTS=OFF` drops
  ...
  point.
- *Outstanding checks (criterion 10), carried to the Phase 5 handover when
  the plan was archived (2026-09-25):* a green `--compile-tests` build of
  the tree with the in-module self-test batch and the build-time records
  (Hans); one run of `simple_test_exec
