...
contains
    ...
    subroutine segdiampick_mics_multi( spproj, pcontrast, mic_to, moldiam_max, box_in_pix, mskdiam, accepted_bins )
        ...
        logical, optional, allocatable, intent(out) :: accepted_bins(:) ! size(MOLDIAMS_PICK)-1; .true. = bin accepted, for reuse by segdiampick_mics_multi_fixed_bins
        ...
        if( present(accepted_bins) )then
            if( allocated(accepted_bins) ) deallocate(accepted_bins)
            allocate(accepted_bins(nclust_hac), source=(abs_z_scores < SIGMA_CRIT) .and. (hac_pops > 0))
        endif
    ...
    subroutine segdiampick_mics_multi_fixed_bins( spproj, pcontrast, mic_to, moldiam_max, box_in_pix, accepted_bins )
        ! Same as segdiampick_mics_multi, except the set of accepted diameter
        ! bins is supplied by the caller (accepted_bins) instead of being
        ! (re)determined here via Z-score thresholding of cluster means, and
        ! the output box size is supplied by the caller (box_in_pix) instead
        ! of being recomputed from the diameters observed in this batch.
        ! Useful e.g. to re-apply a bin selection and box size established in
        ! an earlier stream cycle to a freshly imported batch of micrographs.
        class(sp_project),         intent(inout) :: spproj
        integer,                   intent(in)    :: box_in_pix ! box size (px) to use when writing box coordinates
        integer,                   intent(inout) :: mic_to     ! last micrograph to process
        character(len=*),          intent(in)    :: pcontrast
        real,                      intent(in)    :: moldiam_max ! kept for API compatibility
        logical,                   intent(in)    :: accepted_bins(:) ! size(MOLDIAMS_PICK)-1; .true. = bin accepted
        real,                      parameter     :: SMPD_SHRINK1 = 4.0   ! shrink factor used in diameter correction path
        logical,                   parameter     :: DEBUG        = .false. ! enables verbose diagnostic printing
        type(string),              allocatable   :: micnames(:), mic_den_names(:), mic_topo_names(:), mic_bin_names(:), mic_diam_names(:)
        integer,                   allocatable   :: orimap(:)
        ...
        type(stats_struct) :: diam_stats
        type(nrtxtfile)    :: diams_file
        integer            :: imic, nptcls, i, nclust_hac, n_accepted, i_acc, ibin
        integer            :: nrecs_line, ndatalines, iline
        integer            :: funit, iostat, x_old, y_old, old_box, x_new, y_new
        integer            :: line_data_cap
        integer            :: box_single
        ...
        diam_hi = MOLDIAMS_PICK(size(MOLDIAMS_PICK))
        nclust_hac = size(MOLDIAMS_PICK) - 1
        if( size(accepted_bins) /= nclust_hac ) THROW_HARD('size(accepted_bins) must equal size(MOLDIAMS_PICK)-1='//int2str(nclust_hac)//'; segdiampick_mics_multi_fixed_bins')
        allocate(mic_den_names(mic_to), mic_topo_names(mic_to), mic_bin_names(mic_to), mic_diam_names(mic_to))
        ! Pass 1: gather diameters from either preprocessed outputs or fresh picking.
        do imic = 1, mic_to
            ! Reuse preprocessed artifacts when available.
        ...
        ! Build robust global diameter statistics for reporting purposes only
        ! (bin acceptance itself is supplied directly via accepted_bins).
        diams_arr = diams_arr + 2. * SMPD_SHRINK1 ! because of the 2X erosion in binarization
        ! strict boundary bins: keep only diameters within [MOLDIAMS_PICK(1), MOLDIAMS_PICK(end)]
        tmp = pack(diams_arr, mask=diams_arr >= diam_lo .and. diams_arr <= diam_hi)
        call move_alloc(tmp, diams_arr)
        ...
        call calc_stats(diams_arr, diam_stats)
        call print_diam_stats('CC diameter (in Angs) statistics', diam_stats%avg, diam_stats%minv, diam_stats%maxv, med=diam_stats%med, sde=diam_stats%sdev)
        ! Fixed-bin clustering; acceptance decided by the caller via accepted_bins.
        allocate(hac_centroids(nclust_hac), source=0.)
        allocate(hac_dsum(nclust_hac),      source=0.)
        allocate(hac_pops(nclust_hac),      source=0)
        ! cluster populations (unweighted counts); per-cluster min/max
        ...
        if( DEBUG )then
            ...
            do i = 1, nclust_hac
                ...
                &', min diam: ', hac_dmin(i), ', max diam: ', hac_dmax(i),&
                &', % pop: ', real(hac_pops(i)) * pop_scale,&
                &', accepted: ', accepted_bins(i)
            end do
        endif
        n_accepted = count( accepted_bins .and. (hac_pops > 0) )
        write(logfhandle, *) ">>> N CLUSTERS ACCEPTED (CALLER-SUPPLIED): "//int2str(n_accepted)//" OUT OF "//int2str(nclust_hac)
        if( n_accepted == 0 )then
            THROW_WARN('No caller-accepted cluster of diameters had any population; returning empty cluster set')
            call spproj%write_segment_inside('mic')
            if( allocated(orimap) ) deallocate(orimap)
            call mic_den%kill
            call mic_diam_name_here%kill
        ...
        box_single = box_in_pix
        allocate(cluster_for_bin(nclust_hac), source=0)
        ! Collapse accepted bins into one output box/mask envelope.
        i_acc = 0
        do i = 1, nclust_hac
            if( hac_pops(i) > 0 .and. accepted_bins(i) ) then
                i_acc              = i_acc + 1
                cluster_for_bin(i) = i_acc
                if( DEBUG ) call print_diam_stats('CC diameter (in Angs) statistics for accepted cluster with centroid '//real2str(hac_centroids(i))//':', &
                    hac_centroids(i), hac_dmin(i), hac_dmax(i))
        ...
        call spproj%write_segment_inside('mic')
        call spproj%write()
        deallocate(hac_centroids, hac_dsum, hac_pops, hac_dmin, hac_dmax)
        ! cleanup
        if( allocated(cluster_for_bin) ) deallocate(cluster_for_bin)
        if( allocated(line_data) ) deallocate(line_data)
        if( allocated(tmp) ) deallocate(tmp)
