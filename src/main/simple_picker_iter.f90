...
subroutine iterate( self, cline, smpd, moviename_intg, boxfile, nptcls_out, dir_out )
    ...
    elseif(params_glob%picker .eq. 'phasecorr') then
        ...
        if( cline%defined('refs') .or. cline%defined('vol1') )then
          if( cline%defined('thres') )then
              call init_phasecorr_picker_refs(moviename_intg, params_glob%refs, smpd, lp_in=params_glob%lp,&
                  &distthr_in=params_glob%thres, ndev_in=params_glob%ndev, dir_out=dir_out)
          else
              call init_phasecorr_picker_refs(moviename_intg, params_glob%refs, smpd, lp_in=params_glob%lp, &
                  &ndev_in=params_glob%ndev, dir_out=dir_out)
        ...
        ! phasecorrelation segmentation based
            if( cline%defined('thres') )then
                call init_phasecorr_picker_gauss(moviename_intg, params_glob%min_rad, params_glob%max_rad, &
                    &real(params_glob%stepsz),params_glob%elongated, smpd, lp_in=params_glob%lp,&
                    &distthr_in=params_glob%thres, ndev_in=params_glob%ndev, dir_out=dir_out)
            else
                call init_phasecorr_picker_gauss(moviename_intg, params_glob%min_rad, params_glob%max_rad, &
                    &real(params_glob%stepsz),params_glob%elongated, smpd, lp_in=params_glob%lp,&
        ...
        if(.not. cline%defined('detector'))  params_glob%detector  = 'bin'
            ...
            if(cline%defined('thres')) then
                call seg_picker%new(moviename_intg, params_glob%min_rad, params_glob%max_rad,&
                    &params_glob%elongated,smpd, params_glob%draw_color, params_glob%thres)
            else
                call seg_picker%new(moviename_intg, params_glob%min_rad, params_glob%max_rad,&
                    &params_glob%elongated,smpd, params_glob%draw_color)
            endif
