! ============================================================================
! Name        : testing_GnCrA_3D_gpu.f90
! Author      : Frederic Bonnet
! Version     :
! Date        : 2nd of February 2016
! Description : test full kernel for the Gencor_all method in 3D data Struct.
!             :
! ============================================================================
!
program testing_GnCrA_3D_gpu
...
#define devptr_t integer*8
  ...
  integer, parameter            :: start_npart=256, istep=64
  ...
  double precision              :: elps_corr_ZN_gpu
  double precision,dimension(2) :: st_ZN_r, et_ZN_r
...
!*******************************************************************************
     ...
     write(*,'(7(3x,i5),5x,i3,x,i3,x,i3,8x,i3,8x,i2)') &
          ...
          s_polar%ikrnl
     call start_timer_cpu("corr_ZN_gpu")
     call gettimeofday_c(st_ZN_r)
     err = get_polarft_gencorrall_gpu_c(a_devD,                     &
                                        s_polar,"Z","N",            &
                                        ...
                                        lda,ldb,ldc,alpha,          &
                                        s_bench, s_debug_gpu) 
     call gettimeofday_c(et_ZN_r)
     call elapsed_time_c(st_ZN_r,et_ZN_r,elps_corr_ZN_gpu)
     call stop_timer_cpu("corr_ZN_gpu")
     ...
     write(*,*)"Elapsed time elps_corr_ZN_gpu: ",real(elps_corr_ZN_gpu),"(seconds)"
     ...
     speedup = real(elps_corr_cpu) / real(elps_corr_ZN_gpu)
     ...
     if( ibench_write ) then
        open(4,file='GnCrA_3D_calc_ZN.asc',status='unknown',position='append')
        write(4,'(1x,f15.8,2x,f15.8)')ipart*nrot*nk/1.e6, &
             real(elps_corr_cpu) / real(elps_corr_ZN_gpu)
        close(4)
     end if
#endif     
...
end program testing_GnCrA_3D_gpu
