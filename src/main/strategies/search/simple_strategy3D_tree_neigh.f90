...
module simple_strategy3D_tree_neigh
...
public :: strategy3D_tree_neigh
private
#include "simple_local_flags.inc"

type, extends(strategy3D) :: strategy3D_tree_neigh
contains
    procedure :: new         => new_tree_neigh
    procedure :: srch        => srch_tree_neigh
    procedure :: kill        => kill_tree_neigh
    procedure :: oris_assign => oris_assign_tree_neigh
end type strategy3D_tree_neigh

contains

    subroutine new_tree_neigh( self, params, spec, build )
        use simple_builder, only: builder
        class(strategy3D_tree_neigh), intent(inout) :: self
        class(parameters),             intent(in)    :: params
        class(strategy3D_spec),        intent(inout) :: spec
        class(builder),                intent(in)    :: build
        call self%s%new(params, spec, build)
        self%spec = spec
    end subroutine new_tree_neigh

    subroutine srch_tree_neigh( self, os, ithr )
        class(strategy3D_tree_neigh), intent(inout) :: self
        ...
        if( .not. allocated(self%s%b_ptr%subspace_full2sub_map) )then
            THROW_HARD('tree_neigh search requires subspace_full2sub_map. Check builder construction.')
        endif
        if( .not. allocated(self%s%b_ptr%subspace_inds) )then
            THROW_HARD('tree_neigh search requires subspace_inds. Check builder construction.')
        endif
        if( ntrees <= 0 )then
            THROW_HARD('tree_neigh search requires at least one block tree.')
        endif
        if( ntrees > MAX_NTREES )then
            THROW_HARD('Number of trees exceeds MAX_NTREES; srch_tree_neigh')
        endif
        ! ------------------------------------------------------------------
        ...
        do isub = 1, self%s%p_ptr%nspace_sub
            ...
            if( iproj_full < 1 .or. iproj_full > self%s%p_ptr%nspace ) THROW_HARD('subspace index out of bound; tree_neigh search')
            ...
            if( itree < 1 .or. itree > ntrees ) THROW_HARD('tree index out of bound; tree_neigh search')
        ...
        ! ------------------------------------------------------------------
        do i = 1, npeak_trees
            call descend_tree_greedy_fixed_state(self%s, peak_sel%peak_trees(i), nrefs_tree, self%s%prev_state)
        enddo
        self%s%nrefs_eval = nrefs_tree
        call self%s%inpl_srch_peaks(min(self%s%npeaks_inpl, self%s%nsolns))
        call self%oris_assign
    end subroutine srch_tree_neigh

    subroutine oris_assign_tree_neigh( self )
        class(strategy3D_tree_neigh), intent(inout) :: self
        call extract_peak_ori(self%s)
    end subroutine oris_assign_tree_neigh

    subroutine kill_tree_neigh( self )
        class(strategy3D_tree_neigh), intent(inout) :: self
        call self%s%kill
    end subroutine kill_tree_neigh

end module simple_strategy3D_tree_neigh
