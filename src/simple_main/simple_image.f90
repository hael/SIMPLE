...
contains
    ...
    function constructor( ldim, smpd, imgkind, backgr ) result( self ) !(FAILS W PRESENT GFORTRAN)
        ...
        real, intent(in)                       :: smpd
        character(len=*), intent(in), optional :: imgkind
    ...
    subroutine write( self, fname, i, del_if_exists, formatchar )
        ...
        character(len=1), intent(in), optional :: formatchar
        type(image) :: tmpimg
    ...
    subroutine serialize( self, pcavec, mskrad )
        class(image), intent(inout)      :: self
        real, intent(inout), allocatable :: pcavec(:)
        real, intent(in), optional       :: mskrad
    ...
    function get_fcomp( self, logi, phys_in, phys_out ) result( comp )
        class(image), intent(in)       :: self
        integer, intent(in)            :: logi(3)
        integer, intent(in), optional  :: phys_in(3)
        integer, intent(out), optional :: phys_out(3)
        integer                        :: phys(3)
        complex                        :: comp
    ...
    subroutine vis( self, sect )
        class(image), intent(in)      :: self
        integer, intent(in), optional :: sect
    ...
    subroutine add_2( self, logi, comp, phys_in, phys_out )
        ...
        integer, intent(in)            :: logi(3)
        complex, intent(in)            :: comp
        integer, intent(out), optional :: phys_in(3)
        integer, intent(out), optional :: phys_out(3)
        integer                        :: phys(3)
        complex                        :: comp_here
    ...
    subroutine cos_edge( self, falloff )
        ...
        do i=1,self%ldim(1)
            ...
            do j=1,self%ldim(2)
                ...
                if( self%ldim(3)==1 )then
                    ! 2d
                    if( rmat(i,j,1)==1. )then ! within mask region
                    ...
                    ! 3d
                    do k=1,self%ldim(3)
                        if( rmat(i,j,k)==1. )then ! within mask region
                            ks = max(1,k-1)
                            ke = min(k+1,self%ldim(3))
                            if( any( rmat(is:ie,js:je,ks:ke) < 1.) )then
                                ! has a masked neighbour                            
                                kl = max(1,k-falloff)
                                kr = min(k+falloff,self%ldim(3))
                                call update_mask_3d(i,j,k)
                            endif
                        endif
                    end do
                endif
        ...
        contains
            ...
            subroutine update_mask_2d
                integer :: ii,jj,disti_sq,dist_sq
                real    :: w
                do ii=il,ir
                    disti_sq = (ii-i)**2                       ! 1D squared distance in x dim
                    do jj=jl,jr
                        dist_sq = disti_sq + (jj-j)**2         ! 2D squared distance in x & y dim
                        if( dist_sq <= falloff_sq )then
                            if( rmat(ii,jj,1)<1. )then    ! masked neighbour
                                w = cosedge( sqrt(real(dist_sq) ), rfalloff )
                                rmat(ii,jj,1) = max( w,rmat(ii,jj,1) )
                            endif
                        endif
                    enddo
                enddo
            ...
            ! updates neighbours with cosine weight
            subroutine update_mask_3d(i0,j0,k0)
                integer :: ii,jj,kk,di_sq,dij_sq,dist_sq
                integer :: i0,j0,k0
                do ii=il,ir
                    di_sq = (ii-i0)**2
                    do jj=jl,jr
                        dij_sq = di_sq+(jj-j0)**2
                        do kk=kl,kr
