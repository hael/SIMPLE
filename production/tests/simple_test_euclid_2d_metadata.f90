...
if( command_argument_count() < 4 )then
    write(logfhandle,'(a)') &
        'simple_test_euclid_2d_metadata projfile=xx mskdiam=xx lp=xx smpd=xx inpl_refine=yes|no'
    stop 2
endif
...
do iptcl = 1, nptcls
    ...
    if( delta_e3 > max(1.e-4, 0.01*dang) ) n_offgrid = n_offgrid + 1
enddo
...
if( expect_continuous .and. n_offgrid < 1 ) &
    THROW_HARD('2D metadata contains no continuous off-grid e3 values')
