...
type :: polarft_corrcalc
    ...
    procedure          :: kill
    ! testing
    procedure          :: memoize_ptcls, memoize_refs, kill_memoized
...
contains
    ...
    subroutine memoize_ptcls( self )
        ...
        &self%cvec2(nthr_glob),self%drvec(nthr_glob))
        ...
        do iref = 1, self%nrefs
            do i = 1, self%nptcls
                ...
                if( dist < thres )then
                    ...
                    ptcl_ref_dist = self%geodesic_frobdev(euls_ref, ptcl_eulspace%get_euler(iptcl))
    ...
    subroutine kill_memoized( self )
        ...
        enddo
        deallocate(self%ft_ptcl_ctf,self%ft_ctf2,self%ft_ref_even,self%ft_ref_odd,&
    ...
    subroutine gencorrs_cc_dev( self, iptcl, iref, corrs)
        ...
        do k = self%kfromto(1),self%kfromto(2)
            ...
            self%drvec(ithr)%r = self%drvec(ithr)%r + real(k,dp) * self%rvec1(ithr)%r(1:self%nrots)
            ! FT(X.CTF) x FT(REF)
