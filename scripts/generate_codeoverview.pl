...
sub repository_name {
  my ($repo_root) = @_;
  my $fallback = 'SIMPLE';
  my $git_config = File::Spec->catfile($repo_root, '.git', 'config');

  return $fallback unless -f $git_config;

  open my $fh, '<:raw', $git_config or return $fallback;
  while (my $line = <$fh>) {
    next unless $line =~ /^\s*url\s*=\s*(\S+)\s*$/;
    my $url = $1;
    $url =~ s{\\}{/}g;
    $url =~ s{/+$}{};
    $url =~ s{\.git$}{}i;
...
my $repository_name = repository_name($root);
print $out_fh "<!-- AUTO-GENERATED by scripts/generate_codeoverview.pl; do not manually edit. -->\n\n";
...
while (@stack) {
  ...
  $dname = $repository_name if $dir eq $root;
