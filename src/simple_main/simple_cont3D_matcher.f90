...
!     subroutine tmp_cont3D_exec(b, p, cline, which_iter, converged)
!         class(build),   intent(inout) :: b
!         class(params),  intent(inout) :: p
!         class(cmdline), intent(inout) :: cline
...
!         endif
!     end subroutine tmp_cont3D_exec
...
!             top   = p%fromp-1 + batches(batch,2)
!             ! PREP BATCH
!             allocate(pftccs(fromp:top), cont3Dsrch(fromp:top),&
!                 &batch_imgs(fromp:top), stat=alloc_stat)
!             call alloc_err('In pcont3D_matcher::pcont3D_exec_single',alloc_stat)
!             do iptcl = fromp, top
!                 state = nint(b%a%get(iptcl, 'state'))
...
!                 if( p%ctf.ne.'no' )call pftccs(iptcl)%create_polar_ctfmats(p%smpd, b%a)
!                 call cont3Dsrch(iptcl)%new(p, pftccs(iptcl), b%refvols)
!             enddo
!             ! SERIAL SEARCHES
!             !$omp parallel do default(shared) schedule(guided)&
!             !$omp private(iptcl, state)
!             do iptcl = fromp, top
!                 call cont3Dsrch(iptcl)%exec_srch(b%a, iptcl, 1, 1)
!             enddo
!             !$omp end parallel do
!             ! GRID & 3D REC
!             if(p%norec .eq. 'no')then
...
!                 call batch_imgs(iptcl)%kill
!             enddo
!             deallocate(pftccs, cont3Dsrch, batch_imgs)
!         enddo
!         ! orientations output
!         !call b%a%write(p%outfile, [p%fromp,p%top])
!         p%oritab = p%outfile
