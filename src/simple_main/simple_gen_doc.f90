...
contains
    ...
    subroutine print_doc_pick
        write(*,'(A)', advance='no') 'is a program that implements robust template-based particle picking from integra'
        write(*,'(A)', advance='no') 'ted movies (micrographs) generated with unblur. Use make_pick_refs to generate t'
        write(*,'(A)') 'he refs templates (from a volume or from class averages)'
        stop
    ...
    subroutine print_doc_extract
        ...
        write(*,'(A)', advance='no') 'er SIMPLE programs. We obtain CTF parameters with CTFFIND4 but if you have alrea'
        write(*,'(A)', advance='no') 'dy obtained CTF parameters from CTFFIND4, please see section "CTF parameters con'
        write(*,'(A)') 'vention" in the manual'
        stop
    end subroutine print_doc_extract

    subroutine print_doc_prime2D_init
        write(*,'(A)', advance='no') 'is used to produce the initial random references for prime2D execution. The rand'
        write(*,'(A)', advance='no') 'om clustering and in-plane alignment is printed in the file prime2D_startdoc.txt'
        write(*,'(A)', advance='no') ' produced by the program. This file is used together with the initial references'
        write(*,'(A)') ' (startcavgs.ext) to execute prime2D'
        stop
    ...
    subroutine print_doc_resrange
        ...
        write(*,'(A)', advance='no') 'am to the desired value (in \AA{). This may be necessary if your images have a l'
        write(*,'(A)', advance='no') 'ot of background padding. However, for starting model generation it is probably'
        write(*,'(A)', advance='no') 'better to clip the images snugly around the particle, because smaller images equ'
        write(*,'(A)') 'al less computation'
        stop
    ...
    subroutine print_doc_shellweight3D
        ...
        write(*,'(A)') 's measure by the FRC) are given the appropriate weight'
        stop
    ...
    subroutine print_doc_automask2D
        write(*,'(A)', advance='no') 'is a program for solvent flattening of class averages (MRC or SPIDER). The algor'
        write(*,'(A)', advance='no') 'ithm for background removal is based on low-pass filtering and binarization. Fir'
        write(*,'(A)', advance='no') 'st, the class averages are low-pass filtered to amsklp. Binary representatives a'
        write(*,'(A)', advance='no') 're then generated by assigning foreground pixels using sortmeans. A cosine funct'
        write(*,'(A)', advance='no') 'ion softens the edge of the binary mask before it is  multiplied with the unmask'
        write(*,'(A)') 'ed input averages to accomplish flattening'
        stop
    end subroutine print_doc_automask2D

    subroutine print_doc_automask3D
        write(*,'(A)', advance='no') 'is a program for solvent flattening of a volume (MRC or SPIDER). The algorithm f'
        write(*,'(A)', advance='no') 'or background removal is based on low-pass filtering and binarization. First, th'
        write(*,'(A)', advance='no') 'e volume is low-pass filtered to amsklp. A binary volume is then generated by as'
        write(*,'(A)', advance='no') 'signing foreground pixels (=1) based on the volume calculated from the molecular'
        write(*,'(A)', advance='no') ' weight. A cosine function softens the edge of the binary mask before it is  mul'
        write(*,'(A)') 'tiplied with the unmasked input to generate the flattened map'
        stop
    ...
    subroutine print_doc_makedeftab
        write(*,'(A)', advance='no') 'is a program for creatign a SIMPLE conformant file of CTF parameter values (deft'
    ...
    subroutine print_doc_rotmats2oris
        write(*,'(A)', advance='no') 'convertes a text file (9 records per line) describing rotation matrices into a S'
        write(*,'(A)') 'IMPLE oritab'
        stop
    ...
    subroutine print_doc_merge_similarities
        ...
        write(*,'(A)') ' similarity matrix that can be inoutted to cluster_smat'
        stop
