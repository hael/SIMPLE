...
contains
    ...
    subroutine disc( self, ldim, smpd, radius, npix )
        ...
        integer                     :: i,j,k
        real                        :: ci,cii,cj,cjj,ck,ckk,sqrad
        call self%new(ldim, smpd)
        self%rmat = 1.
        call self%mask(radius, 'hard')
        npix = nint(sum(self%rmat(:ldim(1),:ldim(2),:ldim(3))))
    end subroutine
    ...
    subroutine window( self_in, coord, box, self_out )
        class(image), intent(in)   :: self_in
        integer, intent(in)        :: coord(2), box
        type(image), intent(inout) :: self_out
        integer                    :: fromc(2), toc(2), xoshoot, yoshoot
        if( any(coord < 0) ) return ! box is outside of micrograph, so image is empty
        ...
        if( toc(1) > self_in%ldim(1) .or. toc(2) > self_in%ldim(2) )then
            write(*,*) 'WARNING! Box extends outside micrograph; window; simple_image'
        endif
        xoshoot = 0
        yoshoot = 0
        if( toc(1) > self_in%ldim(1) )then
            xoshoot = toc(1)-self_in%ldim(1)
        endif
        if( toc(2) > self_in%ldim(2) )then
            yoshoot = toc(2)-self_in%ldim(2)
        endif
        if( xoshoot > 0 .or. yoshoot > 0 )then
            self_out%rmat = 0.
            toc(1) = toc(1)-xoshoot
            toc(2) = toc(2)-yoshoot
            self_out%rmat(:box-xoshoot,:box-yoshoot,1) = self_in%rmat(fromc(1):toc(1),fromc(2):toc(2),1)
        else
            self_out%rmat(:box,:box,1) = self_in%rmat(fromc(1):toc(1),fromc(2):toc(2),1)
        endif
    ...
    function extr_pixels( self, mskimg ) result( pixels )
        ...
        real, allocatable :: pixels(:)
        integer :: npix,alloc_stat,cnt,k,j,i
        if( self%is_ft() ) stop 'only 4 real images; extr_pixels; simple_image'
        if( self.eqdims.mskimg )then
            npix = nint(sum(mskimg%rmat(:self%ldim(1),:self%ldim(2),:self%ldim(3))))
            allocate( pixels(npix), stat=alloc_stat )
            call alloc_err('In: extr_pixels; simple_image', alloc_stat)
            cnt = 0
            do k=1,self%ldim(3)
                do j=1,self%ldim(2)
                    do i=1,self%ldim(1)
    ...
    subroutine read( self, fname, i, isxfel )
        ...
        logical, intent(in), optional :: isxfel
        class(imghead), allocatable :: h
        type(imgfile)         :: ioimg
        character(len=1)      :: form
        character(len=20)     :: conv
        integer               :: ldim(3), iform, maxim, first_slice
        integer               :: last_slice, ii, mode, alloc_stat, hind, kind, ipix, jpix
        real                  :: smpd
        logical               :: isvol, die, err, iisxfel, debug=.false.
        real(dp), allocatable :: tmpmat1(:,:,:)
        real(sp), allocatable :: tmpmat2(:,:,:), tmpmat3(:,:,:)
        if( self%existence )then
            ...
            select case(form)
                case('M')
                    call ioimg%open(fname, smpd=self%smpd) ! pixel size of object overrides pixel size in header
                ...
                case('S')
                    call ioimg%open(fname, ldim=self%ldim, smpd=self%smpd) ! pixel size of object overrides pixel size in header
                    if( debug )then
                    ...
                    ! -21 = 3D Fourier odd
                    ! -22 = 3D Fourier even
                    if( iform == 1 .or. iform == -11 .or. iform == -12 )then
                    ...
                    else if( iform == 3 .or. iform == -21 .or. iform == -22 )then
                        ...
                        write(*,*) 'iform: ', iform
                        stop 'unsupported value of iform in overall SPIDER header; read; simple_image'
                    endif
                case('D')
                    if( self%even_dims())then
            ...
            if( present(i) )then
                if( i > ioimg%getStackSz() )then
                    write(*,*) 'stack size: ', ioimg%getStackSz(), 'image index: ', i
                    stop 'ERROR, trying to read past last image in stack; read; simple_image'
                endif
            endif
        ...
        if( iisxfel )then
            ! assume intensities inputted, operate on amplitudes (or overflow)
            self%rmat = sqrt(self%rmat)
            ! minmax normalisation without the subtraction, needed because
            ! the sums are inaccurate for such large numbers
            self%rmat = self%rmat/maxval(self%rmat)
            ! convert to XFEL representation
            call self%em2xfel
        endif
    end subroutine
    ...
    subroutine write( self, fname, i, del_if_exists )
        ...
        if( self%imgkind .eq. 'xfel' )then
             ...
             call tmpimg%write_emkind( fname, i, del_if_exists )
             call tmpimg%kill
        else
            call self%write_emkind( fname, i, del_if_exists )
        endif
    end subroutine

    !>  \brief  for writing emkind images to stack or volumes to volume files
    subroutine write_emkind( self, fname, i, del_if_exists )
        ...
        type(image)       :: img_tmp
        character(len=1)  :: form
        integer           :: first_slice, last_slice, iform, ii, alloc_stat, array_shape(3)
        logical           :: isvol, didft, die
        if( self%existence )then
            die = .false.
            ...
            select case(form)
                case('M')
                    ...
                    ! 'NATIVE' endconv overrides class variable
                    call ioimg%open(fname, ldim=self%ldim, smpd=self%smpd, del_if_exists=die)
                case('S')
                    ! pixel size of object overrides pixel size in header
                    call ioimg%open(fname, ldim=self%ldim, smpd=self%smpd, del_if_exists=die)
                    ! iform file type specifier:
    ...
    ! GETTERS/SETTERS
     ...
     !>  \brief  is a getter
    function get_array_shape( self ) result( shape)
        class(image), intent(in) :: self
        integer :: shape(3)
    ...
    !>  \brief  is a getter
    function get_smpd( self ) result( smpd )
        class(image), intent(in) :: self
        real :: smpd
        smpd = self%smpd
    end function
    ...
    function get_nyq( self ) result( nyq )
        class(image), intent(in) :: self
        integer :: nyq
    ...
    function get_filtsz( self ) result( n )
        class(image), intent(in) :: self
        integer :: n
    ...
    function get_imgkind( self ) result( imgkind )
        class(image), intent(in)      :: self
    ...
    !>  \brief  is a getter
    function get_lfny( self, which ) result( fnyl )
        class(image), intent(in) :: self
    ...
    !>  \brief  is a getter
    function get_lhp( self, which ) result( hpl )
        class(image), intent(in) :: self
        integer, intent(in) :: which
        integer :: hpl
    ...
    !>  \brief  is a getter
    function get_lp( self, ind ) result( lp )
        class(image), intent(in) :: self
    ...
    !>  \brief  is a getter
    function get_find( self, res ) result( ind )
        class(image), intent(in) :: self
        real, intent(in)         :: res
        integer :: ind
    ...
    subroutine subtr_1( self, self_to_subtr )
        class(image), intent(inout) :: self
        class(image), intent(in)    :: self_to_subtr
        if( self.eqdims.self_to_subtr )then
            if( self%imgkind .eq. 'xfel' )then
                self%cmat = self%cmat-self_to_subtr%cmat
            else
                self%rmat = self%rmat-self_to_subtr%rmat
            endif
        else
    ...
    !>  \brief  is for binarizing an image with given treshold value
    !!          binary normalization (norm_bin) assumed
    subroutine bin_1( self, tres )
        class(image), intent(inout) :: self
        real, intent(in)            :: tres
        integer                     :: i, j, k
    ...
    !>  \brief  is for binarizing an image using nr of pixels/voxels treshold
    subroutine bin_2( self, npix )
        ...
        allocate( forsort(product(self%ldim)), stat=alloc_stat )
        call alloc_err('bin_2; simple_image', alloc_stat)
        forsort = 0.
        cnt = 0
        do i=1,self%ldim(1)
            do j=1,self%ldim(2)
                do k=1,self%ldim(3)
                    cnt = cnt+1
                    forsort(cnt) = self%rmat(i,j,k)
                end do
            end do
        end do
        call hpsort(product(self%ldim), forsort)
        tres = forsort(product(self%ldim)-npix-1) ! everyting above this value 1 else 0
        call self%bin( tres )
        deallocate( forsort )
    end subroutine
    ...
    subroutine bin_3( self )
        class(image), intent(inout) :: self
        real, allocatable           :: forsort(:)
        real                        :: cen1, cen2, sum1, sum2, cnt1, cnt2, val1, val2
        integer                     :: cnt, i, j, k, l, alloc_stat, npix
        if( self%ft ) stop 'only for real images; bin_2; simple image'
        ! sort the pixels to initialize k-means
        npix = product(self%ldim)
        allocate( forsort(npix), stat=alloc_stat )
        call alloc_err('bin_3; simple_image', alloc_stat)
        forsort = 0.
        cnt = 0
        do i=1,self%ldim(1)
            do j=1,self%ldim(2)
        ...
        do l=1,100
            ...
            do i=1,self%ldim(1)
                do j=1,self%ldim(2)
                    do k=1,self%ldim(3)
                        if( (cen1-self%rmat(i,j,k))**2. < (cen2-self%rmat(i,j,k))**2. )then
                            sum1 = sum1+self%rmat(i,j,k)
                            cnt1 = cnt1+1.
                        else
                            sum2 = sum2+self%rmat(i,j,k)
                            cnt2 = cnt2+1.
                        endif
                    end do
                end do
        ...
        do i=1,self%ldim(1)
            do j=1,self%ldim(2)
                do k=1,self%ldim(3)
                    if( (cen1-self%rmat(i,j,k))**2. < (cen2-self%rmat(i,j,k))**2. )then
                        self%rmat(i,j,k) = val1
                    else
                        self%rmat(i,j,k) = val2
                    endif
                end do
            end do
        end do
    ...
    !>  \brief  is for determining the center of mass of a binary image
    !!          only use this function for integer pixels shifting
    function masscen( self, msk ) result( xyz )
        class(image), intent(in)   :: self
        ...
        if( self%ft ) stop 'masscen not implemented for Fourier transforms; masscen; simple_image'
        call img%copy(self)
        call img%bin
        if( img%nbackground() < img%nforeground() )then
            call img%bin_inv
        endif
        if( present(msk) ) call img%mask(msk, 'hard')
        spix = 0.
        xyz = 0.
        ci = -real(self%ldim(1))/2.
        do i=1,self%ldim(1)
    ...
    function center( self, lp, msk, tres ) result( xyz )
        class(image), intent(inout) :: self
        real, intent(in)            :: lp
    ...
    subroutine grow_bin( self )
        class(image), intent(inout) :: self
        integer                     :: i,j,k,a,b,c,alloc_stat
            ...
            do i=1,self%ldim(1)
                do j=1,self%ldim(2)
                    if (self%rmat(i,j,1)==0.) then
                        do a=-1,1
                            do b=-1,1
                                if(i+a>0.and.j+b>0.and.i+a<self%ldim(1).and.j+b<self%ldim(2))then
                                    if( self%rmat(i+a,j+b,1)==1. ) add_pixels(i,j,1) = .true.
                                endif
                            end do
                        end do
                    end if
                    ...
                    do k=1,self%ldim(3)
                        if (self%rmat(i,j,k)==0.) then
                            do a=-1,1
                                do b=-1,1
                                    do c=-1,1
                                        if(i+a>0.and.j+b>0.and.k+c>0.and.i+a<self%ldim(1)&
                                        .and.j+b<self%ldim(2).and.k+c<self%ldim(3) )then
                                            if(self%rmat(i+a,j+b,k+c)==1.) add_pixels(i,j,k) = .true.
                                        endif
                                    end do
                                end do
                            end do
        ...
        do i=1,self%ldim(1)
            do j=1,self%ldim(2)
                do k=1,self%ldim(3)
                    if(add_pixels(i,j,k)) self%rmat(i,j,k) = 1.
                end do
            end do
        end do
    ...
    subroutine cos_edge( self, falloff )
        ...
        do i=1,self%ldim(1)
            ...
            do j=1,self%ldim(2)
                ...
                do k=1,self%ldim(3)
                    if( self%rmat(i,j,k)==1. )then ! within mask region
                        ks = max(1,k-1)
        ...
        contains
            ...
            function is_at_edge() result( at_edge )
                integer :: ii,jj,kk
                logical :: at_edge
                at_edge = .false.
                do ii=is,ie
            ...
            subroutine update_mask
                integer :: ii,jj,kk,disti_sq,distij_sq,dist_sq
                real    :: w
                do ii=il,ir
                    disti_sq = (ii-i)**2                            ! 1D squared distance in x dim
                    do jj=jl,jr
                        distij_sq = disti_sq + (jj-j)**2            ! 2D squared distance in x & y dim
                        if( distij_sq<=falloff_sq )then
                            if( self%ldim(3) == 1 )then
                                if( self%rmat(ii,jj,1)<1. )then ! masked neighbour
                                    dist_sq = distij_sq             ! 2D squared distance
                                    if( dist_sq<=falloff_sq )then
                                        w = cosedge( sqrt(real(dist_sq) ), rfalloff)
                                        if( self%rmat(ii,jj,1)<w )self%rmat(ii,jj,1) = w
                                    endif
                                endif
                            else
                                do kk=kl,kr
                                    if( self%rmat(ii,jj,kk)<1. )then ! masked neighbour
                                        dist_sq = distij_sq + (kk-k)**2  ! 3D squared distance
                                        if( dist_sq<=falloff_sq )then
                                            w = cosedge( sqrt(real(dist_sq) ), rfalloff)
                                            if( self%rmat(ii,jj,kk)<w )self%rmat(ii,jj,kk) = w
                                        endif
                                    endif
                                enddo
                            endif
    ...
    function spectrum( self, which, norm ) result( spec )
        ...
        do k=1,lfny
            if( which .ne. 'count' .and. nnorm )then
                if( counts(k) > 0 )then
                    spec(k) = spec(k)/real(counts(k)) ! mean squared amplitude formula
                endif
            endif
        end do
    ...
    function gen_lpfilt( self, lplim, width ) result( filter )
        ...
        real, intent(in), optional  :: width
        integer                     :: nyq, alloc_stat, k, lims(3,2)
    ...
    subroutine phase_rand( self, lp )
        ...
        integer, allocatable        :: find1(:),find2(:)
        complex                     :: comp1,comp2
        logical                     :: didft
        real                        :: freq,lp_freq,sgn1,sgn2,sgn3,ran
            ...
            do i=1,self%ldim(1)
                ...
                do j=1,self%ldim(2)
                    ...
                    do k=1,self%ldim(3)
                        ...
                        if( which .eq. 'foreground' )then
                            if( e > 0.5 )then
                                npix = npix+1
                                pixels(npix) = self%rmat(i,j,k)
                            endif
                        else if( which .eq. 'background' )then
                            if( e < 0.5 )then
                                npix = npix+1
                                pixels(npix) = self%rmat(i,j,k)
                            endif
    ...
    subroutine cure_2( self, maxv, minv, ave, sdev, n_nans )
        ...
        npix   = product(self%ldim)
        n_nans = 0
        maxv   = self%rmat(1,1,1)
        minv   = self%rmat(1,1,1)
        ave    = 0.
        do i=1,self%ldim(1)
            do j=1,self%ldim(2)
                do k=1,self%ldim(3)
                    ...
                    if( self%rmat(i,j,k) >= maxv ) maxv = self%rmat(i,j,k)
                    if( self%rmat(i,j,k) <= minv ) minv = self%rmat(i,j,k)
                    ave = ave+self%rmat(i,j,k)
                end do
            end do
        end do
    ...
    !>  \brief is for correlating two images
    function corr_2( self1, self2, phys, lmsk, lims ) result( r )
        class(image), intent(in)         :: self1, self2
        integer, allocatable, intent(in) :: phys(:,:,:,:)
        logical, allocatable, intent(in) :: lmsk(:,:,:)
        integer, intent(in)              :: lims(3,2)
        real, allocatable                :: xprod(:,:,:), asq(:,:,:), bsq(:,:,:)
        real                             :: r, sumasq, sumbsq
        integer                          :: h, k, l
        allocate(xprod(self1%array_shape(1),self1%array_shape(2),self1%array_shape(3)),&
                   asq(self1%array_shape(1),self1%array_shape(2),self1%array_shape(3)),&
                   bsq(self1%array_shape(1),self1%array_shape(2),self1%array_shape(3)))
        ! real part of the complex mult btw 1 and 2*
        xprod  = real(self1%cmat*conjg(self2%cmat))
        ! normalisation matrices
        asq    = csq(self1%cmat)
        bsq    = csq(self2%cmat)
        ! initialise 4 sum creation
        r      = 0.
        sumasq = 0.
        sumbsq = 0.
        ...
        do h=lims(1,1),lims(1,2)
            do k=lims(2,1),lims(2,2)
                do l=lims(3,1),lims(3,2)
                    r      = r  + xprod(phys(h,k,l,1),phys(h,k,l,2),phys(h,k,l,3))                           
                    sumasq = sumasq+asq(phys(h,k,l,1),phys(h,k,l,2),phys(h,k,l,3))
                    sumbsq = sumbsq+bsq(phys(h,k,l,1),phys(h,k,l,2),phys(h,k,l,3))
                end do
            end do
        end do
        !$omp end parallel do
        r = calc_corr(r,sumasq*sumbsq)
        deallocate(xprod,asq,bsq)
    end function
    ...
    !>  \brief is for correlating two images
    function corrfun( self1, self2, lp_dyn, hp_dyn ) result( corrimg )
        class(image), intent(inout) :: self1, self2
        real, intent(in), optional  :: lp_dyn, hp_dyn
        type(image)                 :: corrimg
        integer                     :: h, hh, k, kk, l, ll, phys(3), lims(3,2), sqarg, sqlp, sqhp
        ...
        if( self1.eqdims.self2 )then
            ...
            call corrimg%new(self1%ldim, self1%smpd)
            corrimg = cmplx(0.,0.)
            if( present(lp_dyn) )then
                lims = self1%fit%loop_lims(1,lp_dyn)
            else
            ...
            if( present(hp_dyn) )then
                ...
                sqhp = 4 ! index 2 default high-pass limit
            endif
            !$omp parallel do default(shared) private(h,hh,k,kk,l,ll,sqarg,phys) &
            !$omp schedule(auto)
            do h=lims(1,1),lims(1,2)
                ...
                do k=lims(2,1),lims(2,2)
                    ...
                    do l=lims(3,1),lims(3,2)
                        ...
                        if( sqarg <= sqlp .and. sqarg >= sqhp  )then
                            phys = self1%fit%comp_addr_phys([h,k,l])
                            corrimg%cmat(phys(1),phys(2),phys(3))=&
                            (self1%cmat(phys(1),phys(2),phys(3))*conjg(self2%cmat(phys(1),phys(2),phys(3))))/&
                            (csq(self1%cmat(phys(1),phys(2),phys(3)))*csq(self2%cmat(phys(1),phys(2),phys(3))))
                         endif
                    end do
                end do
            end do
            ...
            write(*,*) 'self1%ldim:', self1%ldim
            write(*,*) 'self2%ldim:', self2%ldim
            stop 'images to be correlated need to have same dimensions; corrfun; simple_image'
        endif
    end function

    !>  \brief is for calculating a real-space correlation coefficient between images
    function real_corr( self1, self2, msk ) result( r )
        class(image), intent(inout)        :: self1, self2
        class(image), intent(in), optional :: msk
        real, allocatable                  :: diffmat1(:,:,:), diffmat2(:,:,:)
        real, allocatable                  :: tmpmat1(:,:,:), tmpmat2(:,:,:)
        real                               :: r,ax,ay,sxx,syy,sxy,npix ! xt,yt
        ...
        if( present(msk) )then
            if( .not. (self1.eqdims.msk) ) stop 'mask need to have same dim as imgs; real_corr; simple_image'
            allocate(tmpmat1(self1%ldim(1),self1%ldim(2),self1%ldim(3)),&
                     tmpmat2(self2%ldim(1),self2%ldim(2),self2%ldim(3)),&
                     diffmat1(self1%ldim(1),self1%ldim(2),self1%ldim(3)),&
                     diffmat2(self2%ldim(1),self2%ldim(2),self2%ldim(3)))
            tmpmat1  = self1%rmat(:self1%ldim(1),:self1%ldim(2),:self1%ldim(3))&
                     * msk%rmat(:msk%ldim(1),:msk%ldim(2),:msk%ldim(3))
            tmpmat2  = self2%rmat(:self2%ldim(1),:self2%ldim(2),:self2%ldim(3))&
                     * msk%rmat(:msk%ldim(1),:msk%ldim(2),:msk%ldim(3))     
            npix     = sum(msk%rmat(:msk%ldim(1),:msk%ldim(2),:msk%ldim(3)))
            ax       = sum(tmpmat1(:self1%ldim(1),:self1%ldim(2),:self1%ldim(3)))/npix
            ay       = sum(tmpmat2(:self2%ldim(1),:self2%ldim(2),:self2%ldim(3)))/npix
            diffmat1 = tmpmat1(:self1%ldim(1),:self1%ldim(2),:self1%ldim(3))-ax
            diffmat2 = tmpmat2(:self2%ldim(1),:self2%ldim(2),:self2%ldim(3))-ay
            sxx      = sum(diffmat1**2.)
            syy      = sum(diffmat2**2.)
            sxy      = sum(diffmat1*diffmat2)
            deallocate(tmpmat1,tmpmat2,diffmat1,diffmat2)
        else
...
!                     sumbsq(sh) = sumbsq(sh)+csq(self1%cmat(phys(1),phys(2),phys(3)))
    ...
    subroutine gen_argtransf_mats( self, transfmats )
        ...
        integer                     :: h, k, l, lims(3,2), phys(3), alloc_stat
        real                        :: arg(3)
    ...
    subroutine shift( self, x, y, z, lp_dyn, imgout )
        class(image), intent(inout)           :: self
        real, intent(in)                      :: x, y
        real, intent(in), optional            :: z, lp_dyn
        class(image), intent(inout), optional :: imgout
        integer                               :: h, k, l, lims(3,2), phys(3)
        real                                  :: zz
        logical                               :: didft
        if( self%imgkind .eq. 'xfel' )then
    ...
    subroutine mask( self, mskrad, which, inner, width, msksum )
        ...
        real                        :: ci, cj, ck, e, rad
        real                        :: ave, sdev, var, med, wwidth
        integer                     :: i, j, k, minlen
        logical                     :: didft
            ...
            do j=1,self%ldim(2)
                ...
                do k=1,self%ldim(3)
                    if( self%ldim(3) > 1 )then
                        if( which == 'hard' )then
                            ...
                            if( present(inner) )then
                                e = e*hardedge_inner(ci,cj,ck,inner)
                            endif
                        else if( which == 'soft' )then
                            e = cosedge(ci,cj,ck,minlen,mskrad)
                            if( present(inner) )then
                                e = e*cosedge_inner(ci,cj,ck,wwidth,inner)
                            endif
                        else
                            stop 'undefined which parameter; mask; simple_image'
                        endif
                        if( present(msksum) ) msksum = msksum+e**2.
                    else
                        if( which == 'hard' )then
                            e = hardedge(ci,cj,mskrad)
                            if( present(inner) )then
                                e = e*hardedge_inner(ci,cj,inner)
                            endif
                        else if( which == 'soft' )then
                            e = cosedge(ci,cj,minlen,mskrad)
                            if( present(inner) )then
                                e = e*cosedge_inner(ci,cj,wwidth,inner)
                            endif
                        else
                            stop 'undefined which parameter; mask; simple_image'
                        endif
                    ...
                    if( present(msksum) )then
                        ...
                        self%rmat(i,j,k) = self%rmat(i,j,k)*e
                    endif
                    ck = ck+1.
                end do
                cj = cj+1.
        ...
        .and. self_out%ldim(3) <= self_in%ldim(3) )then
            if( self_in%ft )then
                ...
                if( self_in%imgkind .eq. 'xfel' )then
                    ...
                    ratio = real(self_in%ldim(1))/real(self_out%ldim(1))
                    self_out%smpd = self_in%smpd*ratio ! clippingFourier transform, so sampling is coarser
                endif
                self_out%ft = .true.
            else
    ...
    subroutine norm( self, hfun, err )
        ...
        if( self%ldim(3) > 1 )then
            if( present(hfun) ) call normalize_net( self%rmat(:self%ldim(1),:self%ldim(2),:self%ldim(3)), hfun )
        else
            if( present(hfun) ) call normalize_net( self%rmat(:self%ldim(1),:self%ldim(2),1), hfun )
        endif
        if( sdev > 0. )then
    ...
    subroutine noise_norm( self, msk, errout )
        ...
        logical, intent(out), optional :: errout
        type(image)       :: maskimg
        integer           :: npix,nbackgr,alloc_stat,i,j,k,cnt,npix_tot,nsample
        ...
        allocate( pixels(nbackgr), stat=alloc_stat )
        call alloc_err('In: noise_norm; simple_image', alloc_stat)
        cnt = 0
        do k=1,self%ldim(3)
            do j=1,self%ldim(2)
                do i=1,self%ldim(1)
                    if( maskimg%rmat(i,j,k) < 0.5 )then
                        cnt = cnt+1
                        pixels(cnt) = self%rmat(i,j,k)
                    endif
                end do
    ...
    subroutine test_image( doplot )
        ...
        contains
            ...
            subroutine test_image_local( ld1, ld2, ld3, doplot )
                ...
                real                 :: imcorr, recorr, corr, corr_lp, lowpass1, lowpass2
                real, allocatable    :: pcavec1(:), pcavec2(:), spec(:), res(:)
                real                 :: smpd=2., corrs(20)
