...
contains
  ...
  subroutine generate_chunk_coarse_cline( self, new_chunk, nptcls )
    ...
    associate( cline => new_chunk%cline )
      ...
      if( self%coarse_defaults%lpstart > 0.0 ) call cline%set('lpstart', self%coarse_defaults%lpstart)
      if( self%coarse_defaults%lpstop  > 0.0 ) call cline%set('lpstop',  self%coarse_defaults%lpstop)
  ...
  subroutine generate_chunk_fine_cline( self, new_chunk, nptcls )
    ...
    associate( cline => new_chunk%cline )
      ...
      if( self%fine_defaults%lpstart > 0.0 ) call cline%set('lpstart', self%fine_defaults%lpstart)
      if( self%fine_defaults%lpstop  > 0.0 ) call cline%set('lpstop',  self%fine_defaults%lpstop)
      call cline%set('walltime',                      DEFAULT_WALLTIME)
      if( self%nparts > 1             ) call cline%set('nparts',           self%nparts)
      if( server_address%strlen() > 0 ) call cline%set('worker_server', server_address)
    end associate
