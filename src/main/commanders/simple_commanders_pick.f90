        ...
        if( cline%defined('dir_box') )then
            ...
            if( file_exists(params%dir_box) )then
                ...
                if(.not.allocated(boxfiles))then
                    write(logfhandle,*)'No box file found in ', params%dir_box%to_char(), '; simple_commanders_pick::exec_extract 1'
                    THROW_HARD('No box file found; exec_extract, 1')
                endif
                if(size(boxfiles)==0)then
                    write(logfhandle,*)'No box file found in ', params%dir_box%to_char(), '; simple_commanders_pick::exec_extract 2'
