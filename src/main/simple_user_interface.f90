...
contains
    ...
    subroutine new_atoms_register
        ...
        &'single_exec',&                                                                              ! executable
        &1, 0, 0, 0, 0, 0, 1, .false., gui_advanced=.false.)                                          ! # entries in each group, requires sp_project
        ! INPUT PARAMETER SPECIFICATIONS
        ! image input/output
        call atoms_register%set_input('img_ios', 1, 'fname', 'file', 'PDB file list', 'PDB file list', 'e.g. pdb_files.txt', .true., '')
        ! parameter input/output
        ! alternative inputs
        ! <empty>
        ! search controls
