!@descr: motion correction, dose-weighting and frame-weighting of direct electron detector movies
...
contains
    ...
    subroutine motion_correct_mic2spec( img_in, box, speckind, lp_backgr_subtr, img_out )
        ...
        !$omp parallel do private(i,j,ithr) default(shared) schedule(static)&
        ...
        do j = 0,ldim(2)-box,hbox
            do i = 0,ldim(1)-box,hbox
                ...
                call tmp(ithr)%add_rmat2mat_workshare(rmat_sum)
            enddo
        enddo
        !$omp end parallel do
    ...
    subroutine cure_outliers( frames )
        ...
        ! sum
        do iframe = 1,nframes
            call frames(iframe)%add_rmat2mat_workshare(rsum)
        enddo
        ! outliers detection
        ...
        if( noutliers > 0 )then
            ...
            if( l_eer .and. gain_img%exists() )then
                call gain_img%add_zero2mask(outliers)
                noutliers = count(outliers)
                if( noutliers > 0 )then
            ...
            do iframe=1,nframes
                ...
                do k = 1,noutliers
                    ...
                    do jj = j-HWINSZ,j+HWINSZ
                        ...
                        do ii = i-HWINSZ,i+HWINSZ
                            ...
                            n = n + 1
                            vals(n) = frames(iframe)%get([ii,jj,1])
                        enddo
                    enddo
                    if( n > 1 )then
                ...
                do k = 1,noutliers
                    i = pos_outliers_here(1,k)
                    j = pos_outliers_here(2,k)
                    call frames(iframe)%set([i,j,1], new_vals(k,iframe))
                enddo
            enddo
            !$omp end parallel do
        else
