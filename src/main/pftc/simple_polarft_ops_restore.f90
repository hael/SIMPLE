...
contains               
    ...
    module subroutine polar_cavger_merge_eos_and_norm_new( self, reforis, cline, update_frac )
        ...
        if( self%p_ptr%l_trail_rec )then
            ufrac_trec = real(merge(self%p_ptr%ufrac_trec ,update_frac , cline%defined('ufrac_trec')),dp)
            ! read previous polar references to be used first for FSC calculation
            call preproc_prev_arrays( self, reforis, prev_even, prev_odd )
            ! Calculate and write global FSC, FRCs
    ...
    ! local private_routine
    subroutine preproc_prev_arrays( self, reforis, prev_even, prev_odd )
        class(polarft_calc),       intent(in) :: self
        type(oris),                intent(in) :: reforis
        ...
        integer      :: prev_pftsz, prev_kfromto(2), prev_nrefs, ks, ke
        logical      :: l_pad
        call self%get_pft_array_dims(string(POLAR_REFS_FBODY)//'_even'//BIN_EXT, prev_pftsz, prev_kfromto, prev_nrefs)
        if( (prev_nrefs == self%ncls) .and. (prev_pftsz == self%pftsz) )then
            ! Previous refs have the correct dimensions
            call self%read_pft_array(string(POLAR_REFS_FBODY)//'_even'//BIN_EXT, prev_even)
            call self%read_pft_array(string(POLAR_REFS_FBODY)//'_odd'//BIN_EXT,  prev_odd)
        else
            ...
            if( prev_nrefs > self%ncls )then
                ...
                ! Previous refs have different dimensions, need to be mapped to current space
                call symop%new(self%p_ptr%pgrp)
                call prev_eulspace%new(prev_nrefs, is_ptcl=.false.)
                call symop%build_refspiral(prev_eulspace)
                ks = max(self%kfromto(1), prev_kfromto(1))
                ke = min(self%kfromto(2), prev_kfromto(2))
                l_pad = (ks /= prev_kfromto(1)) .or. (ke /= prev_kfromto(2))
                fname = string(POLAR_REFS_FBODY)//'_even'//BIN_EXT
                call map(fname, prev_even)
                fname = string(POLAR_REFS_FBODY)//'_odd'//BIN_EXT
                call map(fname, prev_odd)
                call symop%kill
    ...
    end subroutine preproc_prev_arrays
