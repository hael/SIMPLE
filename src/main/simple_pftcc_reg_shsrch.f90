...
contains
    procedure :: new         => reg_shsrch_new
    procedure :: set_indices => reg_shsrch_set_indices
    procedure :: minimize    => reg_shsrch_minimize
    procedure :: kill        => reg_shsrch_kill
end type pftcc_reg_shsrch

contains

    subroutine reg_shsrch_new( self, maxits )
    ...
    end subroutine reg_shsrch_new

    !> set indicies for shift search
    subroutine reg_shsrch_set_indices( self, ref, ptcl )
        class(pftcc_reg_shsrch), intent(inout) :: self
        integer,                 intent(in)    :: ref, ptcl
        self%reference = ref
        self%particle  = ptcl
    end subroutine reg_shsrch_set_indices

    !> minimisation routine
    function reg_shsrch_minimize( self, irot ) result( cxy )
        class(pftcc_reg_shsrch), intent(inout) :: self
        integer,                 intent(inout) :: irot
        real     :: lowest_shift(2), cxy(3), lowest_cost, rotmat(2,2)
        real(dp) :: lowest_cost_overall, grad(2)
        self%cur_inpl_idx = irot
        call pftcc_glob%gencorr_grad_for_rot_8(self%reference, self%particle, [0.d0,0.d0], self%cur_inpl_idx, lowest_cost_overall, grad)
    ...
    end function reg_shsrch_minimize

    subroutine reg_shsrch_kill( self )
        class(pftcc_reg_shsrch), intent(inout) :: self
    end subroutine reg_shsrch_kill
