...
public :: pick_sigma_group_file, pick_sigma_group_file_for_iter, canonical_sigma2_consumable, load_sigma2_groups
private
...
contains
    ...
    subroutine pick_sigma_group_file( params, sigma_group_fname, found )
        ...
        call pick_sigma_group_file_for_iter(params%which_iter, sigma_group_fname, found)
    end subroutine pick_sigma_group_file

    !> The legacy consumer's selection rule, the single definition of "a
    !! grouped sigma STAR is available for this iteration": the STAR named for
    !! the iteration when it exists, otherwise the highest-numbered STAR in the
    !! current directory (a later stage start continues from the previous
    !! iteration's residual groups, which carry that iteration's number)
    subroutine pick_sigma_group_file_for_iter( iter, sigma_group_fname, found )
        ...
        sigma_group_fname = ''
        found             = .false.
        sigma_group_fname = sigma2_star_from_iter(iter)
        ...
        deallocate(list)
    end subroutine pick_sigma_group_file_for_iter
