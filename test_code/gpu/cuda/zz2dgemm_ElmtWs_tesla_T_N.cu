...
// C = alpha*A*B + beta*C
extern "C" __global__ void
main_zz2dgemm_kernel_T_N( double *C, 
			  ...
			  double alpha, double beta)
{
  ...
  C[i * ldc + j ] = cuDmul(alpha, cuReCCstarmul( A[ j * lda + i], B[ i * ldb + j] ));
		     ...
		     double alpha, double beta)
{
  ...
  main_zz2dgemm_kernel_T_N<<< grid, threads >>>(C, A, B, 
						m, n,
						lda, ldb, ldc,
						alpha, beta);
