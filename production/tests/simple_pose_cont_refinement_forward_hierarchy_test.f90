...
!! Analytic point objects test conventions, brute-force DFT tests arbitrary
!! volumes, slow gather tests source fidelity, and finite projection tests the
!! projection-slice relation without reusing calibration fixtures.
    ...
    subroutine collect_direct_and_gather_planes(volume, workspace, rotation, executed_plane, &
        ...
        do k = -box/2, box/2
            do h = -box/2, box/2
                ...
                ! Oracle: F(Rk)=N^-3 sum_x X(x) exp[-2*pi*i*(Rk).x/N].
