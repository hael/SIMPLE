...
int get_file_list(const char * path, int pathlen, const char * ext,int *count)
{
  char *cpath = F90toCstring(path, pathlen);
...
int get_file_list_reverse_modified(const char * path, int pathlen, const char* ext, int* count)
{
  ...
  if (n < 0)
    ...
    *count = n;
    ...
    if(strcmp(ext,"")){
      ...
      while (n--) {
        if (strstr(namelist[n]->d_name, ext)!=NULL)
          fprintf(f, "%s\n", namelist[n]->d_name);
      }
      free(namelist[n]);
    }
...
int get_subdir_list(const char * path, int pathlen, int* count)
{
  char *cpath = F90toCstring(path, pathlen);
  printf(" simple_posix.c:: get_subdir_list %s\n", cpath);
  // char *buffer;
  extern int errno;
  /*option 2*/
  ...
  if(d) {
    ...
    *count = fcount;
    ...
    if (fcount > 0){
      ...
      while((dir = readdir(d)) != NULL) {
        if(dir-> d_type == DT_DIR && strcmp(dir->d_name, ".") != 0 && strcmp(dir->d_name, "..") != 0) {
          ...
          /*option 2*/
          fprintf(f,"%s\n", dir->d_name);
          //              printf(" get_subdir_list elem %d %s\n", fcount, dir->d_name);
          fcount++;
        }
      }
      fclose(f);
    ...
    printf("%d %s\nget_subdir_list opendir failed to open %s\n", errno, strerror(errno), path);
    perror("Failed : simple_posix.c::get_subdir_list ");
    return -1;
  }
  return 0;
}
...
int glob_file_list(const char *match, int*len_match, int*count, int* sort_by_time) //void** results, int *count)
{
  ...
  if(!f) {
    ...
    char *cmatch = F90toCstring(match, *len_match);
    int glob_val= glob(match, GLOB_PERIOD, NULL, &globlist);
    if(glob_val == GLOB_NOSPACE){
      ...
      *count = (int) globlist.gl_pathc; //account for __simple_filelist__
      ...
      printf(" glob_file_list size after trimming %d \n", *count);
    }
    globfree(&globlist);free(cmatch);
    if(f) fclose(f);
  }
    return err;
}
...
#define FCOPY fcopy_
  int fcopy(char* file1,  char* file2, int len1, int len2)
  {
    ...
    if(!(buffer2 = F90toCstring(file2, len2))) {
      perror("Failed : appendtostring (buf) in posixwrapper:fortranrmdir");
      free(buffer2);
      return 1;
    }
...
int  get_absolute_pathname(const char* in, int* inlen, char out[], int* outlen)
{
