...
contains
    ...
    subroutine init_phasecorr_picker( micfname, refsfname, smpd_in, lp_in, distthr_in, ndev_in, dir_out )
        ...
        ! modify according to PICKER_SHRINK_HERE & PICKER_SHRINKREFINE
    ...
    contains
        ...
        subroutine gen_phase_correlation(field,mask)
            ...
            do iref = 1, nrefs
                ...
                call ref_ext%mask(mskrad=msk, which='soft', backgr=0.)
                aux = field%phase_corr(ref_ext,border=max(ldim_refs(1),ldim_refs(2)),lp=lp) !phase correlation
                if(iref > 1) then
