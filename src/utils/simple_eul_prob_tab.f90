...
type :: eul_prob_tab
  ...
  ! PARTITION-WISE PROCEDURES (used only by partition-wise eul_prob_tab objects)
    procedure :: fill_tab, fill_tab_batch
    procedure :: write_tab
...
contains
    ...
    subroutine fill_tab_batch( self, pftcc )
        use simple_polarft_corrcalc,  only: polarft_corrcalc
        use simple_pftcc_shsrch_grad, only: pftcc_shsrch_grad
        class(eul_prob_tab),     intent(inout) :: self
        class(polarft_corrcalc), intent(inout) :: pftcc
        logical,      parameter :: l_coarse_shift_search = .true.
        integer,      parameter :: HNSTEPS = 4
        type(pftcc_shsrch_grad) :: grad_shsrch_obj(nthr_glob)
        integer,  allocatable   :: locn(:)
        integer :: i, j, iproj, iptcl, projs_ns, ithr, irot, inds_sorted(pftcc%nrots,nthr_glob), istate, iref
        logical :: l_doshift
        integer :: sx,sy,best_sx, best_sy
        real    :: dists_inpl(pftcc%nrots,nthr_glob), dists_inpl_sorted(pftcc%nrots,nthr_glob), scores(pftcc%nrots)
        real    :: dists_projs(params_glob%nspace,nthr_glob), lims(2,2), lims_init(2,2), cxy(3), rotmat(2,2)
        real    :: inpl_athres, ds, best_score
        call seed_rnd
        l_doshift = params_glob%l_prob_sh .and. params_glob%l_doshift
        if( l_doshift )then
            ! make shift search objects
            ...
            if( l_coarse_shift_search )then
                ds = params_glob%trs/real(HNSTEPS+1)
                do istate = 1, self%nstates
                    iref        = (istate-1)*params_glob%nspace
                    inpl_athres = calc_athres('dist_inpl', state=istate)
                    call calc_num2sample(params_glob%nspace, 'dist', projs_ns, state=istate)
                    ...
                    !$omp private(i,j,sx,sy,best_sx,best_sy,scores,best_score,iptcl,ithr,iproj,irot,cxy,rotmat,locn)
                    do i = 1, self%nptcls
                        iptcl = self%pinds(i)
                        ithr  = omp_get_thread_num() + 1
                        if( build_glob%spproj_field%get_sampled(iptcl) == 1 )then
                            ! performs coarse search prior to projection direction evaluations, no continuous search
                            do iproj = 1, params_glob%nspace
                                best_score = -999.
                                do sx = -HNSTEPS,HNSTEPS
                                    do sy = -HNSTEPS,HNSTEPS
                                        call pftcc%gencorrs(iref + iproj, iptcl, ds*real([sx,sy]), scores)
                                        if( maxval(scores) > best_score )then
                                            best_sx    = sx
                                            best_sy    = sy
                                            best_score = maxval(scores)
                                            dists_inpl(:,ithr) = scores
                                        endif
                                    enddo
                                enddo
                                dists_inpl(:,ithr) = eulprob_dist_switch(dists_inpl(:,ithr))
                                ...
                                cxy(2:3) = matmul(ds*real([best_sx,best_sy]), rotmat)
                                self%loc_tab(iproj,i,istate)%x = cxy(2)
                                self%loc_tab(iproj,i,istate)%y = cxy(3)
                            enddo
                        else
                            ! projection direction evaluations followed by continuous search
                            do iproj = 1, params_glob%nspace
                                call pftcc%gencorrs(iref + iproj, iptcl, dists_inpl(:,ithr))
                                dists_inpl(:,ithr) = eulprob_dist_switch(dists_inpl(:,ithr))
                                irot = angle_sampling(dists_inpl(:,ithr), dists_inpl_sorted(:,ithr), inds_sorted(:,ithr), inpl_athres)
