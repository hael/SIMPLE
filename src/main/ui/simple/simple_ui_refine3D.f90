...
contains
    ...
    subroutine new_refine3D( prgtab )
        ...
        call refine3D%add_input(UI_SRCH, 'refine', 'multi', 'Refinement mode', 'Refinement mode(snhc|shc|neigh|shc_neigh|prob|prob_state|prob_neigh|ptree|shc_ptree){shc}', '(snhc|shc|neigh|shc_neigh|prob|prob_state|prob_neigh|ptree|shc_ptree){shc}',&
        &.false., 'shc', gui_submenu="search")
        call refine3D%add_input(UI_SRCH, 'neigh_type', 'multi', 'Neighborhood construction', 'Neighborhood construction for refine=prob_neigh(geom|subspace_srch){subspace_srch}', '(geom|subspace_srch){subspace_srch}', &
