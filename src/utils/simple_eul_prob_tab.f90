...
type :: eul_prob_tab
  ...
  ! PARTITION-WISE PROCEDURES (used only by partition-wise eul_prob_tab objects)
    procedure :: fill_tab, fill_tab_dev
...
contains
    ...
    subroutine fill_tab_dev( self, pftcc )
        use simple_polarft_corrcalc,  only: polarft_corrcalc
        ...
        type(pftcc_shsrch_grad) :: grad_shsrch_obj2(nthr_glob)
        type(ori)               :: o_prev
        integer :: i, j, iproj, iptcl, projs_ns, ithr, irot, inds_sorted(pftcc%nrots,nthr_glob), istate, iref
        logical :: l_doshift
        real    :: dists_inpl(pftcc%nrots,nthr_glob), dists_inpl_sorted(pftcc%nrots,nthr_glob)
        real    :: dists_projs(params_glob%nspace,nthr_glob), lims(2,2), lims_init(2,2), cxy(3), inpl_athres
        real    :: rotmat(2,2), trslim
        call seed_rnd
        if( trim(params_glob%sh_first).eq.'yes' )then
            ...
            if( l_doshift )then
                ...
                do ithr = 1,nthr_glob
                    ...
                    call grad_shsrch_obj2(ithr)%new(lims, lims_init=lims_init, shbarrier=params_glob%shbarrier,&
                    &maxits=params_glob%maxits_sh, opt_angle=.true., coarse_init=.false.)
                end do
                ! fill the table
                do istate = 1, self%nstates
                    iref        = (istate-1)*params_glob%nspace
                    ...
                    call calc_num2sample(params_glob%nspace, 'dist', projs_ns, state=istate)
                    allocate(locn(projs_ns), source=0)
                    !$omp parallel do default(shared) private(i,j,iptcl,ithr,o_prev,iproj,irot,cxy,rotmat,locn) proc_bind(close) schedule(static)
                    do i = 1, self%nptcls
                        iptcl = self%pinds(i)
                        ithr  = omp_get_thread_num() + 1
                        call build_glob%spproj_field%get_ori(iptcl, o_prev)   ! previous ori
                        irot  = pftcc%get_roind(360.-o_prev%e3get())          ! in-plane angle index
                        iproj = build_glob%eulspace%find_closest_proj(o_prev) ! previous projection direction
                        call fm_shsrch_obj(ithr)%exhaustive_search(iref+iproj, iptcl, irot, cxy(1), cxy(2:3))
                        call rotmat2d(360.-pftcc%get_rot(irot), rotmat)
                        ...
                        do j = 1,projs_ns
                            iproj = locn(j)
                            call grad_shsrch_obj2(ithr)%set_indices(iref + iproj, iptcl)
                            irot = self%loc_tab(iproj,i,istate)%inpl
                            cxy  = grad_shsrch_obj2(ithr)%minimize(irot=irot)
                            if( irot > 0 )then
                                self%loc_tab(iproj,i,istate)%inpl = irot
                                self%loc_tab(iproj,i,istate)%dist = eulprob_dist_switch(cxy(1))
                                self%loc_tab(iproj,i,istate)%x    = cxy(2)
