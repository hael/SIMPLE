...
Prints every entry sorted by time and writes the same table next to the log
as <log>.timing.txt, so the numbers are kept from build to build and a suite
that grows is visible. Exits 1 on a failed entry or a run over budget, 2 if
the log holds no ctest results.

usage: scripts/ctest_budget.py LOG [--budget 30] [--no-budget]
"""
import argparse
import re
import sys
