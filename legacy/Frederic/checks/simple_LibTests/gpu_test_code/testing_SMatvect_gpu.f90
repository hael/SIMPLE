...
#define devptr_t integer*8
  ...
  integer, parameter            :: npart= 35000    !4000  !30000
  integer, parameter            :: start_npart=35000, istep=64
  integer, parameter            :: nrot = 15000 !4000 !15000
...
!*******************************************************************************
     ...
     write(*,*)'                                                               '
     write(*,*)'***************GPU Matrix Mul with cublasSgemv*****************'
     write(*,*)'                                                               '

     call start_timer_cpu("mtml_gpu")
     call gettimeofday_c(st_ZM_r)

     call my_SMatvec("N", ipart, nrot, &
                     alpha,            &
                     pft1, lda,        &
                     pft2, incx,       &
                     beta,             &
     ...
     write(*,*)'                                                               '
     write(*,*)'****Comparing product from matvec vs cublasSgemv (GPU)*********'
     write(*,*)'                                                               '
     ...
     write(*,'(3x,a,2x,a,6x,a)') "Matvec CPU (secs)","Sgemm GPU (secs)","Gflp/s"
     write(*,'(x,f15.5,3x,f15.5,7x,f10.5)')elps_mtml_cpu,elps_mtml_gpu,gflops
     write(*,*)
     write(*,'(x,a,x,f10.3)')" Speed up matmul vs cublasSgemv : ",speedup
     write(*,*)'***************************************************************'
...
end program testing_SMatvect_gpu
