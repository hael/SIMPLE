...
contains
    ...
    procedure :: set_indices => grad_carshsrch_set_indices
...
contains
    ...
    subroutine grad_carshsrch_set_indices( self, ref, ptcl )
        class(cftcc_shsrch_grad), intent(inout) :: self
        integer,                  intent(in)    :: ref, ptcl
        self%reference = ref
        self%particle  = ptcl
    end subroutine grad_carshsrch_set_indices
    ...
    function grad_carshsrch_costfun( self, vec, D ) result( cost )
        ...
        select type(self)
            class is (cftcc_shsrch_grad)
                cost = - cartftcc_glob%calc_corr(self%reference, self%particle, real(vec))
            class default
    ...
    subroutine grad_carshsrch_gcostfun( self, vec, grad, D )
        ...
        select type(self)
            class is (cftcc_shsrch_grad)
                cost = - cartftcc_glob%calc_corr(self%reference, self%particle, real(vec), corrs_grad)
                grad = - corrs_grad
            class default
    ...
    subroutine grad_carshsrch_fdfcostfun( self, vec, f, grad, D )
        ...
        select type(self)
            class is (cftcc_shsrch_grad)
                f    = - cartftcc_glob%calc_corr(self%reference, self%particle, real(vec), corrs_grad)
                grad = - corrs_grad
            class default
    ...
    subroutine coarse_search(self, lowest_cost, init_xy)
        ...
        do ix = 1,coarse_num_steps
            ...
            do iy = 1,coarse_num_steps
                ...
                cost = -cartftcc_glob%calc_corr(self%reference, self%particle, real([x,y]))
