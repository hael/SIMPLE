...
static void externalJob(struct mg_connection *nc, std::string messagedata){
	...
	postRunPreproc(nc, messagedata, jobdir);
}
...
static void simpleJob(struct mg_connection *nc, std::string messagedata){
	...
	if(pid == 0){
		...
		jobPreRun(nc, messagedata, argscommand);
...
static void processWebsocketMessage(struct mg_connection *nc, std::string messagedata){
	...
	}else if(command == "fileviewer"){
		fileViewer(nc, messagedata);
	}
}

static void webEventHandler(struct mg_connection *connection, int event, void *eventdata) {
	char 			buf[1000000];
	...
	switch (event) {
		case MG_EV_HTTP_REQUEST: {
			...
			mg_get_http_var(&hm->query_string, "filename", tmpbuf, 1024);
                        std::cout <<"uri " << hm->uri.p << "buf " << tmpbuf << std::endl;
			std::cout << hm->message.p << std::endl;
			snprintf(buf, sizeof(buf), "%.*s", (int) hm->uri.len, hm->uri.p);
			messagedata = buf;
			if (messagedata.find("/getjpeg") != std::string::npos) {
				JPEGData* jpegdata = new JPEGData();
				returnJPEG(messagedata, jpegdata);
				mg_send_head(connection, 200, jpegdata->jpegsize, "Content-Type: img/jpeg");
				mg_send(connection, jpegdata->jpeg, jpegdata->jpegsize);
				delete jpegdata;
			}else {
...
int main(void) {
	...
	connection = mg_bind(&manager, s_http_port, webEventHandler);	// Create Mongoose connection
