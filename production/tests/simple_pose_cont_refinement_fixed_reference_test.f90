module pose_cont_refinement_fixed_reference_test
...
subroutine run_fixed_reference_diagnostic()
    ...
    type(cartesian_pose_refiner) :: raw_truth_workspace, corrected_truth_workspace
    type(cartesian_pose_refiner) :: simulator_scaled_raw_workspace
    type(cartesian_pose_refiner) :: simulator_scaled_corrected_workspace, exact_kb_scaled_workspace
    type(cartesian_pose_refiner) :: reconstructed_workspace, corrected_reconstructed_workspace
    type(pose_refinement_summary) :: summaries(NARMS)
    ...
    call corrected_truth_workspace%new_prepared_test(corrected_truth_volume)
    call corrected_truth_workspace%set_shell_range([2,BOX/2])
    call model_operator%set_volume(reconstructed_volume)
    call reconstructed_workspace%new_prepared_test(reconstructed_volume)
    ...
    call corrected_reconstructed_workspace%new_prepared_test(corrected_reconstructed_volume)
    ...
    call simulator_scaled_raw_workspace%new_prepared_test(simulator_scaled_raw_volume)
    ...
    call simulator_scaled_corrected_workspace%new_prepared_test(simulator_scaled_corrected_volume)
    ...
    call exact_kb_scaled_workspace%new_prepared_test(exact_kb_scaled_volume)
...
subroutine measure_and_polish(workspace,observed,exact_rotmats,rotation_scale,rotmats,shifts, &
    ...
    call refine_fixed_volume_poses(workspace,rotmats,observed,shifts,rotation_scale, &
...
end module pose_cont_refinement_fixed_reference_test
