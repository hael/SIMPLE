...
untie @distr_simple;
tie @distr_simple, 'Tie::File', 'simple_clusterDistr.pm' or die "Could not open file simple_clusterDistr.pm $!";
$distr_simple[2] =~ s/use lib '.+'/use lib '$SIMPLE_SCRIPTS_PATH'/;
$distr_simple[3] =~ s/use lib '.+'/use lib '$SIMPLE_PATH'/;
################################################################################
...
mkdir $SIMPLE_PATH.'/bin';
mkdir $SIMPLE_PATH.'/bin/bin_tests';
...
foreach my $j(0 .. $#prod_dirs){
	...
	if( $PLATFORM == 0 ){                               # MacOSX
	    ...
	    gen_compile_and_link_LINUX($basename, $bindir); # for Linux: Ubuntu_14.10
	}
	close $fh;
