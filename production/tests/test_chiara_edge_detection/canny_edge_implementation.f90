module canny_mod
  ...
  contains
    ...
    function calc_gradient(self) result(Dcr)
      ...
      type(image)                         :: img_p , Gr, img_out            !padded image
      ...
      ldim = self%get_ldim()
      allocate( grad_mat(ldim(1),ldim(2),1), Dc(ldim(1),ldim(2),1), Dcr(2*ldim(1),ldim(2),1), mat_out(ldim(1),ldim(2),1), &
    ...
    subroutine non_max_supp(img_in,dir_mat)
      ...
      type(image)                :: img4viz
      real,  intent(inout)       :: dir_mat(:,:,:)
      real, allocatable          :: rmat(:,:,:)
      integer                    :: ldim(3), i, j, minim(3), maxim(3)
      ldim = shape(dir_mat)
    ...
    do i = 1, ldim(1)
      do j = 1, ldim(2)
               if( pi/4. + pi/8. < dir_mat(i,j,1) .and. dir_mat(i,j,1) <= pi/2.) then         !case(90), north - south
                    if(j+1 > ldim(2)) then
                          if(rmat(i,j,1) < rmat(i,j-1,1)) rmat(i,j,1) = 0.
                    else if(j-1 < 1) then
                          if(rmat(i,j,1) < rmat(i,j+1,1)) rmat(i,j,1) = 0.
                    else
                          if(rmat(i,j,1) < rmat(i,j+1,1) .or. rmat(i,j,1) < rmat(i,j-1,1)) rmat(i,j,1) = 0. !suppressed
                    end if
            else if( 0. <= dir_mat(i,j,1) .and. dir_mat(i,j,1) <= pi/8.) then                !case(0), east - west
                  if(i+1 > ldim(1)) then
                        if(rmat(i,j,1) < rmat(i-1,j,1)) rmat(i,j,1) = 0.
                  else if(i-1 < 1) then
                        if(rmat(i,j,1) < rmat(i+1,j,1)) rmat(i,j,1) = 0.
                  else
                        if(rmat(i,j,1) < rmat(i+1,j,1) .or. rmat(i,j,1) < rmat(i-1,j,1)) rmat(i,j,1) = 0. !suppressed
                  endif
                 ...
                 if( (i-1 < 1 .and. j-1 >0) .or.(j+1 > ldim(2) .and. i+1 <= ldim(1)) ) then
                      if(rmat(i,j,1) < rmat(i+1,j-1,1)) rmat(i,j,1) = 0.   !just northwest
                 else if( (i-1 >0  .and. j-1 < 1) .or. ( i+1 > ldim(1) .and. j+1 <= ldim(2)) ) then
                      if(rmat(i,j,1) < rmat(i-1,j+1,1)) rmat(i,j,1) = 0.  !just southeast
    ...
    subroutine double_thresh(img_in, thresh)
      ...
      mat = img_in%get_rmat()
      print *, "Maximum value of gradient: ", maxval(mat)
      print *, "Minimum value of gradient: ", minval(mat)
      do i = 1, ldim(1)
        do j = 1, ldim(2)
      ...
      subroutine canny_edge(img_in,img_out, thresh)
          ...
          allocate( d_mat(2*ldim(1),ldim(2),1), Dr(ldim(1),ldim(2),1), Dc(ldim(1),ldim(2),1), grad(ldim(1),ldim(2),1), dir_mat(ldim(1),ldim(2),1), source = 0.)
