!==============================================================================
! MODULE: simple_qsys_worker_message_status
!
! PURPOSE:
!   Provides the concrete status message type sent by the queue-system server
!   to worker processes as a reply when no task is available or an error occurs.
!
!   The server sends a status message in response to a worker heartbeat when
...
!
! DESIGN CONTRACT:
!   serialise_qsys_worker_message_status inlines the three-statement
...
!
! USAGE:
!   type(qsys_worker_message_status) :: status
!   call status%new()
...
!   simple_qsys_worker_message_types — WORKER_STATUS_MSG enumerator
!==============================================================================
module simple_qsys_worker_message_status
    use simple_defs,                      only: STDLEN
    use simple_qsys_worker_message_base,  only: qsys_worker_message_base
    use simple_qsys_worker_message_types, only: WORKER_STATUS_MSG
    implicit none

    public  :: qsys_worker_message_status
    private

    !> Status wire message sent by the server to a worker.
    !> Carries an idle/error acknowledgment when no task is available.
    type, extends(qsys_worker_message_base) :: qsys_worker_message_status
        integer               :: status  = 0   !< status code at time of transmission
        character(len=STDLEN) :: message = ''  !< human-readable status description
    contains
        procedure :: new       => new_qsys_worker_message_status       !< constructor
        procedure :: kill      => kill_qsys_worker_message_status      !< destructor
        procedure :: serialise => serialise_qsys_worker_message_status !< byte-buffer serialiser
    end type qsys_worker_message_status
...
contains
    ...
    subroutine new_qsys_worker_message_status( self )
        class(qsys_worker_message_status), intent(inout) :: self
        call self%kill()
        self%msg_type = WORKER_STATUS_MSG
    end subroutine new_qsys_worker_message_status
    ...
    subroutine kill_qsys_worker_message_status( self )
        class(qsys_worker_message_status), intent(inout) :: self
        self%msg_type = 0
        self%status   = 0
        self%message  = ''
    end subroutine kill_qsys_worker_message_status
    ...
    !> the complete qsys_worker_message_status layout — and filled with the
    ...
    !> and silently drop status and message from the buffer.
    subroutine serialise_qsys_worker_message_status( self, buffer )
        class(qsys_worker_message_status), intent(in)    :: self
        character(len=:), allocatable,        intent(inout) :: buffer
        if( allocated(buffer) ) deallocate(buffer)
        allocate(character(len=sizeof(self)) :: buffer)
        buffer = transfer(self, buffer)
    end subroutine serialise_qsys_worker_message_status

end module simple_qsys_worker_message_status