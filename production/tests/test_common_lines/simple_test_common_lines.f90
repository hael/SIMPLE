...
implicit none
integer,          parameter   :: NPLANES = 20, ORI_IND1 = 1, ORI_IND2 = 3
...
real             :: ave, sdev, maxv, minv, total_costs(NPLANES), pair_costs(NPLANES,NPLANES), minval, shifts(3), rval
complex          :: diff, val
...
!$omp parallel do collapse(2) default(shared) private(i,j,k,f_ind,ori_phys,target_phys,diff)&
!$omp proc_bind(close) schedule(static)
do i = 1, spiral%get_noris()
