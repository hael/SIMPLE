...
! caching reference ft/cc
module simple_polarft_corrcache
!$ use omp_lib
!$ use omp_lib_kinds
...
public :: polarft_corrcache, pftcc_glob
private
...
type fftw_carr
    ...
    complex(kind=c_float_complex), pointer :: ptcl_fft(:) => null() ! FT(Particles PFT 1D)
end type fftw_carr

type fftw_vars
    type(c_ptr)                            :: p_ptcl, p_ptcl_fft
    complex(kind=c_float_complex), pointer :: ptcl(:)     => null() ! Particle PFT 1D
    complex(kind=c_float_complex), pointer :: ptcl_fft(:) => null() ! FT(Particle PFT 1D)
    type(c_ptr)                            :: p_ref, p_ref_fft
    complex(kind=c_float_complex), pointer :: ref(:)      => null() ! Reference PFT 1D
    complex(kind=c_float_complex), pointer :: ref_fft(:)  => null() ! FT(Reference PFT 1D)
    type(c_ptr)                            :: p
    complex(kind=c_float_complex), pointer :: product(:)  => null() ! FT(Reference PFT 1D)* x FT(Particle PFT 1D)
    real(   kind=c_float),         pointer :: cc(:)       => null() ! Re(FT-1(FT(Reference PFT 1D)* x FT(Particle PFT 1D)))
end type fftw_vars
...
type :: polarft_corrcache
    ...
    real(sp),            allocatable :: sqsums_refs_even(:)             
    ...
    complex(sp),         allocatable :: pfts_refs_even_cache(:,:,:) !< 3D complex matrix of polar reference sections (nrefs,pftsz,nk), even
    complex(sp),         allocatable :: pfts_refs_odd_cache(:,:,:)  !< -"-, odd
    ...
    type(c_ptr)                      :: plan_fwd_1, plan_bwd_1      !< FFTW plans for gencorrs
    type(c_ptr)                      :: plan_fwd_2                  !< FFTW plans for particle memoization
    ...
    procedure, private :: shellnorm_and_filter_ref_cache
    ...
    procedure, private :: memoize_ref_fft
    ...
    procedure          :: calc_corrs_over_k_cache
    ...
    procedure, private :: gencorrs_cc_cache_1
    ...
    generic            :: gencorrs_cache => gencorrs_cache_1
    procedure, private :: gencorrs_cache_1
    ...
    ! DESTRUCTOR
    procedure          :: kill
end type polarft_corrcache

! CLASS PARAMETERS/VARIABLES
complex(sp), parameter            :: zero            = cmplx(0.,0.) !< just a complex zero
integer,     parameter            :: FFTW_USE_WISDOM = 16
class(polarft_corrcache), pointer :: pftcc_glob => null()
...
contains
    ...
    subroutine new( self, nrefs, pfromto, l_match_filt, ptcl_mask, eoarr )
        class(polarft_corrcache), target, intent(inout) :: self
        ...
        allocate(self%pfts_refs_even(self%pftsz,params_glob%kfromto(1):params_glob%kfromto(2),self%nrefs),&
                &self%pfts_refs_even_cache(self%pftsz+1,params_glob%kfromto(1):params_glob%kfromto(2),self%nrefs),&
                    &self%pfts_refs_odd(self%pftsz,params_glob%kfromto(1):params_glob%kfromto(2),self%nrefs),&
                    &self%pfts_refs_odd_cache(self%pftsz+1,params_glob%kfromto(1):params_glob%kfromto(2),self%nrefs),&
                    ...
                    &self%sqsums_refs_even(1:self%nrefs),&
        ...
        self%pfts_refs_even_cache = zero
        self%pfts_refs_odd  = zero
        self%pfts_refs_odd_cache = zero
        self%pfts_ptcls     = zero
        self%sqsums_ptcls   = 0.
        self%sqsums_refs_even = 0.
        ! set CTF flag
        self%with_ctf = .false.
        ...
        do ithr = 1, nthr_glob
            ...
            ! product & correlation, in place c2r
            self%fftdat(ithr)%p         = fftwf_alloc_complex(int(self%pftsz+1,c_size_t))
            call c_f_pointer(self%fftdat(ithr)%p, self%fftdat(ithr)%product,         [self%pftsz+1])
            call c_f_pointer(self%fftdat(ithr)%p, self%fftdat(ithr)%cc,              [self%nrots+2])
        enddo
        ! thread-safe c-style allocatables particles memoization
        do i = 1,self%nptcls
            do ik = params_glob%kfromto(1),params_glob%kfromto(2)
                self%fftdat_ptcls(i,ik)%p_ptcl_fft = fftwf_alloc_complex(int(self%pftsz+1, c_size_t))
                call c_f_pointer(self%fftdat_ptcls(i,ik)%p_ptcl_fft, self%fftdat_ptcls(i,ik)%ptcl_fft, [self%pftsz+1])
            end do
        end do
        ! FFTW3 wisdoms file
        ...
        ! FFTW plans
        ! reference c2c
        self%plan_fwd_1 = fftwf_plan_dft_1d(self%nrots, self%fftdat(1)%ref,  self%fftdat(1)%ref_fft,  FFTW_FORWARD, ior(FFTW_PATIENT, FFTW_USE_WISDOM))
        ! particles c2c
        self%plan_fwd_2 = fftwf_plan_dft_1d(self%nrots, self%fftdat(1)%ptcl, self%fftdat(1)%ptcl_fft, FFTW_FORWARD, ior(FFTW_PATIENT, FFTW_USE_WISDOM))
        ! reference*particle c2r
        self%plan_bwd_1 = fftwf_plan_dft_c2r_1d(self%nrots, self%fftdat(1)%product, self%fftdat(1)%cc, ior(FFTW_PATIENT, FFTW_USE_WISDOM))
    ...
    subroutine reallocate_ptcls( self, nptcls, pinds )
        class(polarft_corrcache), intent(inout) :: self
        ...
        if( self%nptcls == nptcls )then
            ...
            if( allocated(self%fftdat_ptcls) )then
                do i = 1, size(self%fftdat_ptcls,dim=1)
                    do ik = params_glob%kfromto(1),params_glob%kfromto(2)
                        call fftwf_free(self%fftdat_ptcls(i,ik)%p_ptcl_fft)
                    end do
                end do
    ...
    subroutine set_ref_pft( self, iref, pft, iseven )
        class(polarft_corrcache), intent(inout) :: self     !< this object
    ...
    subroutine cp_refs( self, self2 )
        class(polarft_corrcache), intent(inout) :: self, self2
    ...
    pure function get_nrots( self ) result( nrots )
        class(polarft_corrcache), intent(in) :: self
        integer :: nrots
        nrots = self%nrots
    ...
    subroutine shellnorm_and_filter_ref_cache( self, iref, pft )
        class(polarft_corrcache), intent(in)    :: self
        integer,                  intent(in)    :: iref
        complex(sp),              intent(inout) :: pft(self%pftsz,params_glob%kfromto(1):params_glob%kfromto(2))
        real    :: pw
    ...
    end subroutine shellnorm_and_filter_ref_cache
    ...
    subroutine memoize_sqsum_ptcl( self, i )
        class(polarft_corrcache), intent(inout) :: self
        integer,                 intent(in)    :: i
        self%sqsums_ptcls(i) = sum(csq_fast(self%pfts_ptcls(:,params_glob%kfromto(1):params_glob%kstop,i)))
    ...
    subroutine memoize_ffts( self )
        ...
        self%pfts_refs_even_cache(1:self%pftsz,:,:) = self%pfts_refs_even
        self%pfts_refs_odd_cache( 1:self%pftsz,:,:) = self%pfts_refs_odd
        !$omp parallel do default(shared) private(i) proc_bind(close) schedule(static)
        do i=1,self%nrefs
            call self%memoize_ref_fft(i)
        end do
        !$omp end parallel do
    ...
    subroutine memoize_fft( self, i )
        ...
        integer  :: i, ik, ithr
        ithr = omp_get_thread_num() + 1
        ! memoize particle forward FTs
        do ik = params_glob%kfromto(1),params_glob%kfromto(2)
            self%fftdat(ithr)%ptcl(1:self%pftsz)            = self%pfts_ptcls(:,ik,i)
            self%fftdat(ithr)%ptcl(self%pftsz+1:self%nrots) = conjg(self%fftdat(ithr)%ptcl(1:self%pftsz))
            call fftwf_execute_dft(self%plan_fwd_2, self%fftdat(ithr)%ptcl, self%fftdat(ithr)%ptcl_fft)
            self%fftdat_ptcls(i,ik)%ptcl_fft = self%fftdat(ithr)%ptcl_fft(1:self%pftsz+1)
        end do
    end subroutine memoize_fft

    ! memoize reference fft (both even and odd), serial only, private use
    subroutine memoize_ref_fft( self, iref )
        class(polarft_corrcache), intent(inout) :: self
        integer  :: iref, ik, ithr
        ithr = omp_get_thread_num() + 1
        ! filter and inverse ctf
        call self%shellnorm_and_filter_ref_cache(iref, self%pfts_refs_even_cache(1:self%pftsz,:,iref))
        call self%shellnorm_and_filter_ref_cache(iref, self%pfts_refs_odd_cache( 1:self%pftsz,:,iref))
        ! multiply with CTF
        if( self%with_ctf )then
            self%pfts_refs_even_cache(1:self%pftsz,:,iref) = self%pfts_refs_even_cache(1:self%pftsz,:,iref) * self%ctfmats(:,:,iref)
            self%pfts_refs_odd_cache( 1:self%pftsz,:,iref) = self%pfts_refs_odd_cache( 1:self%pftsz,:,iref) * self%ctfmats(:,:,iref)
        endif
        self%sqsums_refs_even(iref) = sum(csq_fast(self%pfts_refs_even_cache(1:self%pftsz,params_glob%kfromto(1):params_glob%kstop,iref)))
        ! memoize reference forward FT
        do ik = params_glob%kfromto(1),params_glob%kfromto(2)
            ! even
            self%fftdat(ithr)%ref(1:self%pftsz)            = self%pfts_refs_even_cache(1:self%pftsz,ik,iref)
            ...
            self%pfts_refs_even_cache(:,ik,iref) = conjg(self%fftdat(ithr)%ref_fft(1:self%pftsz+1))
            ! odd
            self%fftdat(ithr)%ref(1:self%pftsz)            = self%pfts_refs_odd_cache(1:self%pftsz,ik,iref)
            self%fftdat(ithr)%ref(self%pftsz+1:self%nrots) = conjg(self%fftdat(ithr)%ref(1:self%pftsz))
            call fftwf_execute_dft(self%plan_fwd_1, self%fftdat(ithr)%ref, self%fftdat(ithr)%ref_fft)
            self%pfts_refs_odd_cache(:,ik,iref) = conjg(self%fftdat(ithr)%ref_fft(1:self%pftsz+1))
        end do
    end subroutine memoize_ref_fft
    ...
    subroutine gen_shmat_8( self, ithr, shift_8 , shmat_8 )
        ...
        self%heap_vars(ithr)%argvec       = self%argtransf(:self%pftsz,  params_glob%kfromto(1)) * shift_8(1) +&
                                            & self%argtransf(self%pftsz+1:,params_glob%kfromto(1)) * shift_8(2)
        shmat_8(:,params_glob%kfromto(1)) = dcmplx(dcos(self%heap_vars(ithr)%argvec), dsin(self%heap_vars(ithr)%argvec))
        ...
        do k = params_glob%kfromto(1)+1,params_glob%kfromto(2)
    ...
    subroutine calc_corrs_over_k_cache( self, iref, i, kfromto, corrs_over_k)
        class(polarft_corrcache), intent(inout) :: self
        integer,                 intent(in)    :: i, kfromto(2), iref
        real,                    intent(out)   :: corrs_over_k(self%nrots)
        integer :: ithr, k
        ! get thread index
        ithr = omp_get_thread_num() + 1
        corrs_over_k = 0.
        do k = kfromto(1),kfromto(2)
            self%fftdat(ithr)%product = self%pfts_refs_even_cache(:,k,iref) * self%fftdat_ptcls(i,k)%ptcl_fft
            ! in-place backward FT
            call fftwf_execute_dft_c2r(self%plan_bwd_1, self%fftdat(ithr)%product, self%fftdat(ithr)%cc)
            corrs_over_k = corrs_over_k + self%fftdat(ithr)%cc(1:self%nrots)
        enddo
        ! normalization
        corrs_over_k = corrs_over_k  / real(2*self%nrots)
    end subroutine calc_corrs_over_k_cache
    ...
    subroutine calc_k_corrs( self, pft_ref, i, k, kcorrs )
        ...
        ! get thread index
        ithr = omp_get_thread_num() + 1
        ! forward transform of reference
        self%fftdat(ithr)%ref(1:self%pftsz)            = pft_ref(:,k)
        self%fftdat(ithr)%ref(self%pftsz+1:self%nrots) = conjg(self%fftdat(ithr)%ref(1:self%pftsz))
        call fftwf_execute_dft(self%plan_fwd_1, self%fftdat(ithr)%ref, self%fftdat(ithr)%ref_fft)
        ! non-redudant product and conjugaison
        self%fftdat(ithr)%product = conjg(self%fftdat(ithr)%ref_fft(1:self%pftsz+1)) * self%fftdat_ptcls(i,k)%ptcl_fft
        ! backward in-place transform
        call fftwf_execute_dft_c2r(self%plan_bwd_1, self%fftdat(ithr)%product, self%fftdat(ithr)%cc)
        kcorrs =  self%fftdat(ithr)%cc(1:self%nrots) / real(2*self%nrots)
    ...
    subroutine calc_T1_T2_for_rot_8( self, pft_ref, pft_dref, i, irot, nderivs, T1, T2)
        ...
        real(dp),                intent(out)   :: T1(params_glob%kfromto(1):params_glob%kstop,nderivs), T2(params_glob%kfromto(1):params_glob%kstop,nderivs)
        integer :: k, rot, j
        if( irot >= self%pftsz + 1 )then
            rot = irot - self%pftsz
        else
    ...
    subroutine gencorrs_cc_cache_1( self, iref, iptcl, cc )
        ...
        pft_ref      => self%heap_vars(ithr)%pft_ref
        corrs_over_k => self%heap_vars(ithr)%corrs_over_k
        call self%calc_corrs_over_k_cache(iref, self%pinds(iptcl), [params_glob%kfromto(1),params_glob%kstop], corrs_over_k)
        sqsum_ref = self%sqsums_refs_even(iref)
        print *, sqsum_ref, corrs_over_k(1:5)
        cc = corrs_over_k / sqrt(sqsum_ref * self%sqsums_ptcls(self%pinds(iptcl)))
    end subroutine gencorrs_cc_cache_1
    ...
    subroutine gencorrs_cache_1( self, iref, iptcl, cc )
        class(polarft_corrcache), intent(inout) :: self
        integer,                 intent(in)    :: iref, iptcl
        real(sp),                intent(out)   :: cc(self%nrots)
        select case(params_glob%cc_objfun)
            case(OBJFUN_CC)
                call self%gencorrs_cc_cache_1(iref, iptcl, cc)
            case(OBJFUN_EUCLID)
                call self%gencorrs_euclid_1(iref, iptcl, cc)
        end select
    end subroutine gencorrs_cache_1
    ...
    subroutine kill( self )
        ...
        if( self%existence )then
            do ithr=1,params_glob%nthr
                ...
                call fftwf_free(self%fftdat(ithr)%p)
...
end module simple_polarft_corrcache
