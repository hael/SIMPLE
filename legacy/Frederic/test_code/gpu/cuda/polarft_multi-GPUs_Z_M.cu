...
//---------------------------------------------------------------------
CachingDeviceAllocator gM_allocator(true); //Caching allocator for device memory
//---------------------------------------------------------------------
...
__global__ void
myZMsum_23D(float *r, float *sumA, float *A, int chunk, int npart, int jpart,
           int blocksPerGrid){
...
template <unsigned int blockSize>
__global__ void
myZMsum_23D_0(float *r, float *sumA, float *A, int chunk, int npart, int jpart,
           int blocksPerGrid){
...
template <unsigned int blockSize>
__global__ void
myZMsum_23D_1(float *r, float *sumA, float *A, int chunk, int npart, int jpart,
           int blocksPerGrid){
...
/* which is similar to myZMsum_3D kernel above but some idle threads*/
template <unsigned int blockSize>
__global__ void
myZMsum_23D_2(float *r, float *sumA, float *A, int chunk, int npart, int jpart,
           int blocksPerGrid){
...
extern "C"
__device__ void
wrapMyZMsum_23D_5(volatile float *shared_A, int tid) {
...
template <unsigned int blockSize>
__global__ void
myZMsum_23D_5(float *r, float *sumA, float *A, int chunk, int npart, int jpart,
           int blocksPerGrid){
 ...
 __shared__ float shared_A[blockSize];
  ...
  if (tid < 32 ) wrapMyZMsum_23D_5(shared_A,tid);
  if (tid==0) sumA[blockIdx.x]=shared_A[0];

}
/* summing the 3D matrix treating it a 1D vec, competely unrolled */
/* which is similar to myZMsum_3D kernel but unrolled              */
template <unsigned int blockSize>
__device__ void
wrapMyZMsum_23D_6(volatile float *shared_A, int tid) {
...
template <unsigned int blockSize>
__global__ void
myZMsum_23D_6(float *r, float *sumA, float *A, int chunk, int npart, int jpart,
        int blocksPerGrid){
  ...
  if (tid < 32 ) wrapMyZMsum_23D_6<blockSize>(shared_A,tid);
  if (tid==0) sumA[blockIdx.x]=shared_A[0];

}
/* getter function to get the kernel call for sum 2D for different kernels */
template <typename S>
int get_myZMsum_23D(S iKernel,
  ...
  case 0:
    ...
    case 512: myZMsum_23D_0<512><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 256:
      myZMsum_23D_0<256><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 128:
      myZMsum_23D_0<128><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 64:
      myZMsum_23D_0<64><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 32:
      myZMsum_23D_0<32><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 16:
      myZMsum_23D_0<16><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 8:
      myZMsum_23D_0<8><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 4:
      myZMsum_23D_0<4><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 2:
      myZMsum_23D_0<2><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 1:
      myZMsum_23D_0<1><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    }
    break;
  case 1:
    switch(threadsPerBlock){
    case 512:
      myZMsum_23D_1<512><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 256:
      myZMsum_23D_1<256><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 128:
      myZMsum_23D_1<128><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 64:
      myZMsum_23D_1<64><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 32:
      myZMsum_23D_1<32><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 16:
      myZMsum_23D_1<16><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 8:
      myZMsum_23D_1<8><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 4:
      myZMsum_23D_1<4><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 2:
      myZMsum_23D_1<2><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 1:
      myZMsum_23D_1<1><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    }
    break;
  case 2:
    switch(threadsPerBlock){
    case 512:
      myZMsum_23D_2<512><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 256:
      myZMsum_23D_2<256><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 128:
      myZMsum_23D_2<128><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 64:
      myZMsum_23D_2<64><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 32:
      myZMsum_23D_2<32><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 16:
      myZMsum_23D_2<16><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 8:
      myZMsum_23D_2<8><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 4:
      myZMsum_23D_2<4><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 2:
      myZMsum_23D_2<2><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 1:
      myZMsum_23D_2<1><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    }
    break;
  case 5:
    switch(threadsPerBlock){
    case 512:
      myZMsum_23D_5<512><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 256:
      myZMsum_23D_5<256><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 128:
      myZMsum_23D_5<128><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 64:
      myZMsum_23D_5<64><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 32:
      myZMsum_23D_5<32><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 16:
      myZMsum_23D_5<16><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 8:
      myZMsum_23D_5<8><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 4:
      myZMsum_23D_5<4><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 2:
      myZMsum_23D_5<2><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    case 1:
      myZMsum_23D_5<1><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr, chunk,npart,jpart,blocksPerGrid); break;
    }
    break;
  case 6:
    switch(threadsPerBlock){
    case 512:
      myZMsum_23D_6<512><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr,chunk,npart,jpart,blocksPerGrid); break;
    case 256:
      myZMsum_23D_6<256><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr,chunk,npart,jpart,blocksPerGrid); break;
    case 128:
      myZMsum_23D_6<128><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr,chunk,npart,jpart,blocksPerGrid); break;
    case 64:
      myZMsum_23D_6<64><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr,chunk,npart,jpart,blocksPerGrid); break;
    case 32:
      myZMsum_23D_6<32><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr,chunk,npart,jpart,blocksPerGrid); break;
    case 16:
      myZMsum_23D_6<16><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr,chunk,npart,jpart,blocksPerGrid); break;
    case 8:
      myZMsum_23D_6<8><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr,chunk,npart,jpart,blocksPerGrid); break;
    case 4:
      myZMsum_23D_6<4><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr,chunk,npart,jpart,blocksPerGrid); break;
    case 2:
      myZMsum_23D_6<2><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr,chunk,npart,jpart,blocksPerGrid); break;
    case 1:
      myZMsum_23D_6<1><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr,chunk,npart,jpart,blocksPerGrid); break;
    }
    break;
  default:
    switch(threadsPerBlock){
    case 512:
      myZMsum_23D<512><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr,chunk,npart,jpart,blocksPerGrid); break;
    case 256:
      myZMsum_23D<256><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr,chunk,npart,jpart,blocksPerGrid); break;
    case 128:
      myZMsum_23D<128><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr,chunk,npart,jpart,blocksPerGrid); break;
    case 64:
      myZMsum_23D<64><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr,chunk,npart,jpart,blocksPerGrid); break;
    case 32:
      myZMsum_23D<32><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr,chunk,npart,jpart,blocksPerGrid); break;
    case 16:
      myZMsum_23D<16><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr,chunk,npart,jpart,blocksPerGrid); break;
    case 8:
      myZMsum_23D<8><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr,chunk,npart,jpart,blocksPerGrid); break;
    case 4:
      myZMsum_23D<4><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr,chunk,npart,jpart,blocksPerGrid); break;
    case 2:
      myZMsum_23D<2><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr,chunk,npart,jpart,blocksPerGrid); break;
    case 1:
      myZMsum_23D<1><<<blocksPerGrid,threadsPerBlock>>>
        (d_r,d_partial_1D,startptr,chunk,npart,jpart,blocksPerGrid); break;
    }
    break;
  }
...
/* main kernel entry */
extern "C" int
polarft_multi_GPUs_Z_M(deviceDetails_t * s_devD,
                       ...
                       bench_t *s_bench, debug_gpu_t *s_debug_gpu)
{
  ...
  gM_allocator.DeviceAllocate(&d_temp_storage, temp_storage_bytes);
  ...
  for (ipart = 0 ; ipart < p_pfts_Sizes->get_npart() ; ipart++) {
    ...
    for (irot = 0 ; irot < p_pfts_Sizes->get_nrot() ; irot++) {
      ...
      zMXprod_3D_mat<<<grid,threads>>>(d_C, d_A, d_B,
                                       d1lim[0],d1lim[1],
                                       d2lim[0],d2lim[1],
      ...
      for (jpart = 0 ; jpart < p_pfts_Sizes->get_npart() ; jpart++ ) {
        ...
        rc = get_myZMsum_23D<int>(s_polar->ikrnl, d_r, d_partial_1D, startptr,
                                 chunk, npart, jpart,
      ...
      myZMNorm_3D<<<gridx,nx>>>(d_cormat3d, d_sqsums_A, d_sqsums_B, d_r,
                               blocksPerGrid,
                               p_pfts_Sizes->get_npart(),
                               p_pfts_Sizes->get_nrot(), ipart, irot, d1lim[0]);
    }
