...
type, extends(ImgHead) :: MrcImgHead
    ...
    type(int_imgheadrec)  :: mode      !< data type: 0 image: signed 8-bit bytes rante -128 to 127
    ...
    type(char_imgheadrec) :: label(10) !< 10 80-character text labels
end type
...
!                 call self%setDim(1,ldim(1))
!                 call self%setDim(2,ldim(2))
                call self%setDims(ldim)
            type is( SpiImgHead )
                call self%setPixSz(smpd)
                lenbyt = ldim(1)*4     ! record length in bytes
                labrec = 1024/lenbyt   ! nr of records in file header (label)
                if( mod(1024,lenbyt ) /= 0 ) labrec = labrec+1
    ...
    subroutine print( self )
        ...
        select type( self )
            type is( MrcImgHead )
                ...
                do i=1,self%nlabl%get()
                    write(*,'(a,i2,2a)') 'Label ', i, ': ', self%label(i)%get()
                end do
            type is( SpiImgHead )
    ...
    subroutine read( self, lun, pos, print_entire )
        ...
        select type( self )
            ...
            type is( SpiImgHead )
                ...
                call alloc_err("In: read; simple_imghed", alloc_stat)
                cnt = 0
                ...
                self%nz       = spihed(1)
                if( debug ) write(*,*) '(imghead::read) nz: ', self%nz
                self%ny       = spihed(2)
                if( debug ) write(*,*) '(imghead::read) ny: ', self%ny
                self%irec     = spihed(3)
                self%iform    = spihed(5)
                if( debug ) write(*,*) '(imghead::read) iform: ', self%iform
                self%imami    = spihed(6)
                self%fmax     = spihed(7)
                self%fmin     = spihed(8)
                self%av       = spihed(9)
                self%sig      = spihed(10)
                self%nx       = spihed(12)
                if( debug ) write(*,*) '(imghead::read) nx: ', self%nx
                self%labrec   = spihed(13)
                if( debug ) write(*,*) '(imghead::read) labrec: ', self%labrec
                ...
                if( debug ) write(*,*) '(imghead::read) labbyt: ', self%labbyt
                self%lenbyt   = spihed(23)
                if( debug ) write(*,*) '(imghead::read) lenbyt: ', self%lenbyt
                ...
                deallocate(spihed)
            class DEFAULT
                 stop 'Format not supported; read; simle_imghead'
        end select
        if( debug ) write(*,*) '(imghead::read) done'
    end subroutine
    ...
    subroutine write( self, lun, pos )
        ...
        select type( self )
            ...
            type is( SpiImgHead )
                spihed = 0.
                spihed(1)  = self%nz
                if( debug ) write(*,*) '(imghead::write) nz: ', spihed(1)
                spihed(2)  = self%ny
                if( debug ) write(*,*) '(imghead::write) ny: ', spihed(2)
                spihed(3)  = self%irec
                spihed(5)  = self%iform
                spihed(6)  = self%imami
                spihed(7)  = self%fmax
                ...
                spihed(10) = self%sig
                spihed(12) = self%nx
                if( debug ) write(*,*) '(imghead::write) nx: ', spihed(12)
                spihed(13) = self%labrec
                if( debug ) write(*,*) '(imghead::write) labrec: ', spihed(13)
                ...
                if( debug ) write(*,*) '(imghead::write) labbyt: ', spihed(22)
                spihed(23) = self%lenbyt
                if( debug ) write(*,*) '(imghead::write) lenbyt: ', spihed(23)
            ...
            class DEFAULT
                stop 'Format not supported; write; simle_imghead'
        end select
    end subroutine
    ...
    subroutine setMachineStamp(self)
        ...
        select type(self)
            ...
            type is (SpiImgHead)
                return
            class DEFAULT
                stop 'Format not supported; setMachineStamp; simle_imghead'
        end select
    end subroutine
            ...
            type is( MrcImgHead )
                select case( self%mode%get() )
                    ...
                    case(2,4)
                        bytesPerPix = 4
                    case DEFAULT
                        stop 'Nonsensical MRC mode number; bytesPerPix; simple_imghead'
                end select
            type is (SpiImgHead)
                bytesPerPix = 4 ! this is true for all Spider files
            class DEFAULT
                stop 'Unsupported header type; bytesPerPix; simple_imghead'
        end select
    end function
            ...
            type is( MrcImgHead )
                select case( self%mode%get() )
                    ...
                    case (6,16)
                        pixIsSigned = .false.
                    case DEFAULT
                        stop 'Nonsensical MRC mode number; pixIsSigned; simple_imghead'
                end select
            type is( SpiImgHead )
                pixIsSigned = .true. ! this is true for all Spider files
            class DEFAULT
                stop 'Unsupported header type; pixIsSigned; simple_imghead'
        end select
    end function
        ...
        select type( self )
            type is( MrcImgHead )
                select case( self%mode%get() )
                    ...
                    case DEFAULT
                        stop 'Nonsensical MRC mode number; getPixType; simple_imghead'
                end select
            type is( SpiImgHead )    
                getPixType = dataRfloat ! this is true for all Spider files
            class DEFAULT
                stop 'Unsupported header type; getPixType; simple_imghead'
        end select
    end function
            ...
            type is( MrcImgHead )
                select case( self%mode%get() )
                    ...
                    case( 3,4 )
                        pixIsComplex = .true.
                    case DEFAULT
                        stop 'Nonsensical MRC mode number; pixIsComplex; simple_imghead'
                end select
            type is (SpiImgHead)
                pixIsComplex = .false. ! individual pixel values are never complex in SPIDER files
            class DEFAULT
                stop 'Unsupported header type; pixIsComplex; simple_imghead'
        end select
    end function
        ...
        select type( lhs )
            type is( MrcImgHead )
                select type( rhs )
                    ...
                    class DEFAULT
                        stop 'Format not supported (RHS); assign; simle_imghead'
                end select
            type is (SpiImgHead)
                select type( rhs )
                    type is( SpiImgHead )
    ...
    real function getMinPixVal(self)
        ...
        select type(self)
            ...
            class DEFAULT
                stop 'Format not supported; getMinPixVal; simle_imghead'
        end select
    end function
    ...
    real function getMaxPixVal(self)
        ...
        select type(self)
            ...
            class DEFAULT
                stop 'Format not supported; getMaxPixVal; simle_imghead'
        end select
    end function
    ...
    subroutine setMinPixVal(self,new_value)
        ...
        select type(self)
            ...
            type is (SpiImgHead)
                ...
                self%imami = 1
            class DEFAULT
                stop 'Format not supported; setMinPixVal; simle_imghead'
        end select
    end subroutine
    ...
    subroutine setMaxPixVal(self,new_value)
        ...
        select type(self)
            ...
            type is (SpiImgHead)
                ...
                self%imami = 1
            class DEFAULT
                stop 'Format not supported; setMaxPixVal; simle_imghead'
        end select
    end subroutine
    ...
    real function getPixSz(self)
        ...
        select type(self)
            ...
            class DEFAULT
                stop 'Format not supported; getPixSz; simle_imghead'
        end select
    end function
    ...
    subroutine setPixSz( self, smpd )
        ...
        select type(self)
            ...
            type is (SpiImgHead)
                self%pixsiz = smpd
            class DEFAULT
                stop 'Format not supported; setPixSz; simle_imghead'
        end select
    end subroutine
    ...
    integer function getStackSz(self) result(stack_size)
        ...
        select type(self)
            ...
            class DEFAULT
                stop 'Format not supported; getStackSz; simle_imghead'
        end select
    end function
    ...
    function getDims(self) result( dims )
        ...
        select type(self)
            ...
            class DEFAULT
                stop 'Format not supported; getDims; simle_imghead'
        end select
    end function
    ...
    function getDim( self, which_dim ) result( dim )
        ...
        select type( self )
            ...
            class DEFAULT
               stop 'Format not supported; getDim; simle_imghead'
        end select
    end function
    ...
    subroutine setDims( self, ldim )
        ...
        select type( self )
            ...
            type is( SpiImgHead )
                ...
                self%nz = ldim(3)
            class DEFAULT
                stop 'Unsupported header type; setDims; simple_imghead'
        end select
    end subroutine
    ...
    function getLabrec( self ) result( labrec )
        ...
        select type( self )
            type is( MrcImgHead )
                stop 'Labrec header variable not available for MRC images; getLabrec; simple_imghead'
            type is( SpiImgHead )
                labrec = int(self%labrec)
            class DEFAULT
               stop 'Format not supported; getLabrec; simle_imghead'
        end select
    end function
    ...
    function getLenbyt( self ) result( lenbyt )
        ...
        select type( self )
            type is( MrcImgHead )
                stop 'Lenbyt header variable not available for MRC images; getLenbyt; simple_imghead'
            type is( SpiImgHead )
                lenbyt = int(self%lenbyt)
            class DEFAULT
               stop 'Format not supported; getLenbyt; simle_imghead'
        end select
    end function
    ...
    function getLabbyt( self ) result( labbyt )
        ...
        select type( self )
            type is( MrcImgHead )
                stop 'Labbyt header variable not available for MRC images; getLabbyt; simple_imghead'
            type is( SpiImgHead )
                labbyt = int(self%lenbyt)
            class DEFAULT
               stop 'Format not supported; getLabbyt; simle_imghead'
        end select
    end function
    ...
    function getMaxim( self ) result( maxim )
        ...
        select type( self )
            type is( MrcImgHead )
                stop 'Maxim header variable not available for MRC images; getMaxim; simple_imghead'
            type is( SpiImgHead )
                maxim = int(self%maxim)
            class DEFAULT
               stop 'Format not supported; getMaxim; simle_imghead'
        end select
    end function
    ...
    subroutine setMaxim( self, maxim )
        ...
        select type( self )
            type is( MrcImgHead )
                stop 'Maxim header variable not available for MRC images; setMaxim; simple_imghead'
            type is( SpiImgHead )
                self%maxim = maxim
            class DEFAULT
               stop 'Format not supported; setMaxim; simle_imghead'
        end select
    end subroutine
    ...
    function getIform( self ) result( iform )
        ...
        select type( self )
            type is( MrcImgHead )
                stop 'Iform header variable not available for MRC images; getIform; simple_imghead'
            type is( SpiImgHead )
                iform = int(self%iform)
            class DEFAULT
               stop 'Format not supported; getIform; simle_imghead'
        end select
    end function
    ...
    function getMode( self ) result( mode )
        ...
        select type( self )
            type is( MrcImgHead )
                mode = self%mode%get()
            type is( SpiImgHead )
                stop 'mode header variable not available for SPIDER images; getMode; simple_imghead'
            class DEFAULT
               stop 'Format not supported; getMode; simle_imghead'
        end select
    end function
    ...
    subroutine setIform( self, iform )
        ...
        select type( self )
            type is( MrcImgHead )
                stop 'Iform header variable not available for MRC images; setIform; simple_imghead'
            type is( SpiImgHead )
                self%iform = iform
            class DEFAULT
               stop 'Format not supported; setIform; simle_imghead'
        end select
    end subroutine
    ...
    subroutine setMode( self, mode )
        ...
        select type( self )
            type is( MrcImgHead )
                self%mode = mode
            type is( SpiImgHead )
                stop 'mode header variable not available for SPIDER images; setMode; simple_imghead'
            class DEFAULT
               stop 'Format not supported; setMode; simle_imghead'
        end select
    end subroutine
