! temp shift search used for reg
module simple_pftcc_shsrch_reg
...
contains
    ...
    pure logical function does_opt_angle( self )
        class(pftcc_shsrch_reg), intent(in) :: self
    ...
    subroutine grad_shsrch_optimize_angle( self )
        class(pftcc_shsrch_reg), intent(inout) :: self
        ...
        self%cur_inpl_idx = minloc(corrs, dim=1)
    ...
    subroutine grad_shsrch_optimize_angle_wrapper( self )
        ...
        select type(self)
            ...
            class DEFAULT
                THROW_HARD('error in grad_shsrch_optimize_angle_wrapper: unknown type; simple_pftcc_shsrch_reg')
        end select
...
end module simple_pftcc_shsrch_reg
