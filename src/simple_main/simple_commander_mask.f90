...
contains
    ...
    !> \brief automask3D is a program for solvent flattening of a volume.
    !! The algorithm for background removal is based on low-pass filtering and
    !!   binarization. First, the volume is low pass filtered to amsklp. A binary
    !!   volume is then generated by assigning foreground pixels (=1) based on the
    !!   volume calculated from the molecular weight. A cosine function softens the
    !!   edge of the binary mask before it is multiplied with the unmasked input to generate the
    !!   flattened map
