...
contains
    ...
    module subroutine taper_edges_vol(self, winsz, edge_mean)
        ...
        integer,      intent(in)    :: winsz
        real,         intent(out)   :: edge_mean
        integer :: n, wtap, wavg, wbg, i, j, k, j1, j2, k1, k2, border_count
        real    :: alpha, inv_wtap, inv_n, border_sum
        ! 2D edge profiles for each tapered direction
        real :: x_start(self%ldim(2),self%ldim(3)), x_stop(self%ldim(2),self%ldim(3))
        real :: x_smooth_start(self%ldim(2),self%ldim(3)), x_smooth_stop(self%ldim(2),self%ldim(3))
        real :: y_start(self%ldim(1),self%ldim(3)), y_stop(self%ldim(1),self%ldim(3))
        real :: y_smooth_start(self%ldim(1),self%ldim(3)), y_smooth_stop(self%ldim(1),self%ldim(3))
        real :: z_start(self%ldim(1),self%ldim(2)), z_stop(self%ldim(1),self%ldim(2))
        real :: z_smooth_start(self%ldim(1),self%ldim(2)), z_smooth_stop(self%ldim(1),self%ldim(2))
        real :: edge_avg_x(self%ldim(2),self%ldim(3))
        real :: edge_avg_y(self%ldim(1),self%ldim(3))
        real :: edge_avg_z(self%ldim(1),self%ldim(2))
        if( self%is_ft() ) THROW_HARD('Real space only!')
        if( self%is_2d() ) THROW_HARD('3D volumes only!')
        if( self%ldim(1) /= self%ldim(2) .or. self%ldim(2) /= self%ldim(3) ) THROW_HARD('Cubic volumes only!')
        ! Assume cubic, 3D
        n    = self%ldim(1)
        ! Primary taper width
        wtap = min(max(winsz, 0), n/2)
        ...
        wavg = min(max(1, wtap/8), n)
        if( wtap <= 0 )then
            edge_mean = sum(self%rmat(1:n,1:n,1:n)) / real(n*n*n)
            return
        endif
        ! -----------------------
        ! Pass 1: taper X edges
        ! -----------------------
        inv_n           = 1.0 / real(wavg)
        x_start(:,:)    = sum(self%rmat(1:wavg,     1:n, 1:n), dim=1) * inv_n
        x_stop(:,:)     = sum(self%rmat(n-wavg+1:n, 1:n, 1:n), dim=1) * inv_n
        edge_avg_x(:,:) = 0.5 * (x_start(:,:) + x_stop(:,:))
        x_start(:,:)    = x_start(:,:) - edge_avg_x(:,:)
        x_stop(:,:)     = x_stop(:,:)  - edge_avg_x(:,:)
        do k = 1, n
            do j = 1, n
                j1 = max(1, j-1)
                j2 = min(n, j+1)
                k1 = max(1, k-1)
                k2 = min(n, k+1)
                inv_n = 1.0 / real((j2 - j1 + 1) * (k2 - k1 + 1))
                x_smooth_start(j,k) = sum(x_start(j1:j2,k1:k2)) * inv_n
                x_smooth_stop(j,k)  = sum(x_stop (j1:j2,k1:k2)) * inv_n
            end do
        end do
        inv_wtap = 1.0 / real(wtap)
        do i = 1, wtap
            alpha = real(wtap - i + 1) * inv_wtap
            self%rmat(i, 1:n, 1:n) = self%rmat(i, 1:n, 1:n) - x_smooth_start(:,:) * alpha
        end do
        do i = n - wtap + 1, n
            alpha = real(wtap + i - n) * inv_wtap
            self%rmat(i, 1:n, 1:n) = self%rmat(i, 1:n, 1:n) - x_smooth_stop(:,:) * alpha
        end do
        ! -----------------------
        ! Pass 2: taper Y edges
        ! -----------------------
        inv_n           = 1.0 / real(wavg)
        y_start(:,:)    = sum(self%rmat(1:n, 1:wavg,     1:n), dim=2) * inv_n
        y_stop(:,:)     = sum(self%rmat(1:n, n-wavg+1:n, 1:n), dim=2) * inv_n
        edge_avg_y(:,:) = 0.5 * (y_start(:,:) + y_stop(:,:))
        y_start(:,:)    = y_start(:,:) - edge_avg_y(:,:)
        y_stop(:,:)     = y_stop(:,:)  - edge_avg_y(:,:)
        do k = 1, n
            do i = 1, n
                j1 = max(1, i-1)
                j2 = min(n, i+1)
                ...
                k2 = min(n, k+1)
                inv_n = 1.0 / real((j2 - j1 + 1) * (k2 - k1 + 1))
                y_smooth_start(i,k) = sum(y_start(j1:j2,k1:k2)) * inv_n
                y_smooth_stop(i,k)  = sum(y_stop (j1:j2,k1:k2)) * inv_n
            end do
        end do
        do j = 1, wtap
            alpha = real(wtap - j + 1) * inv_wtap
            self%rmat(1:n, j, 1:n) = self%rmat(1:n, j, 1:n) - y_smooth_start(:,:) * alpha
        end do
        do j = n - wtap + 1, n
            alpha = real(wtap + j - n) * inv_wtap
            self%rmat(1:n, j, 1:n) = self%rmat(1:n, j, 1:n) - y_smooth_stop(:,:) * alpha
        end do
        ! -----------------------
        ! Pass 3: taper Z edges
        ! -----------------------
        inv_n           = 1.0 / real(wavg)
        z_start(:,:)    = sum(self%rmat(1:n, 1:n, 1:wavg), dim=3) * inv_n
        z_stop(:,:)     = sum(self%rmat(1:n, 1:n, n-wavg+1:n), dim=3) * inv_n
        edge_avg_z(:,:) = 0.5 * (z_start(:,:) + z_stop(:,:))
        z_start(:,:)    = z_start(:,:) - edge_avg_z(:,:)
        z_stop(:,:)     = z_stop(:,:)  - edge_avg_z(:,:)
        do j = 1, n
            do i = 1, n
                j1 = max(1, i-1)
                j2 = min(n, i+1)
                ...
                k2 = min(n, j+1)
                inv_n = 1.0 / real((j2 - j1 + 1) * (k2 - k1 + 1))
                z_smooth_start(i,j) = sum(z_start(j1:j2,k1:k2)) * inv_n
                z_smooth_stop(i,j)  = sum(z_stop (j1:j2,k1:k2)) * inv_n
            end do
        end do
        do k = 1, wtap
            alpha = real(wtap - k + 1) * inv_wtap
            self%rmat(1:n, 1:n, k) = self%rmat(1:n, 1:n, k) - z_smooth_start(:,:) * alpha
        end do
        do k = n - wtap + 1, n
            alpha = real(wtap + k - n) * inv_wtap
            self%rmat(1:n, 1:n, k) = self%rmat(1:n, 1:n, k) - z_smooth_stop(:,:) * alpha
        end do
        ! -----------------------
        ! Compute background mean from outer shell of thickness wbg
        ! -----------------------
        border_sum =  &
            sum(self%rmat(1:wbg,       1:n,        1:n)) + &
            sum(self%rmat(n-wbg+1:n,   1:n,        1:n)) + &
            sum(self%rmat(wbg+1:n-wbg, 1:wbg,      1:n)) + &
            sum(self%rmat(wbg+1:n-wbg, n-wbg+1:n,  1:n)) + &
            sum(self%rmat(wbg+1:n-wbg, wbg+1:n-wbg, 1:wbg)) + &
            sum(self%rmat(wbg+1:n-wbg, wbg+1:n-wbg, n-wbg+1:n))
        border_count = n*n*n - (n-2*wbg)*(n-2*wbg)*(n-2*wbg)
        edge_mean = border_sum / real(border_count)
