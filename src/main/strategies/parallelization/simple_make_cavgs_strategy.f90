...
contains
    ...
    subroutine print_recimg_source_verification(params, build, context)
        type(parameters), intent(in)    :: params
        type(builder),    intent(inout) :: build
        character(len=*), intent(in)    :: context
        type(string) :: source_stk
        integer      :: first_ptcl, last_ptcl, ind_in_stk, nptcls
        nptcls = build%spproj%os_ptcl2D%get_noris()
        if( nptcls < 1 ) return
        first_ptcl = 1
        last_ptcl  = nptcls
        ...
        call build%spproj%get_ptcl_source_stkname_and_ind(params%oritype, first_ptcl, params%recimg_source, &
            source_stk, ind_in_stk)
        write(logfhandle,'(A,A,A,A,A,A,A,I0,A,I0,A,A,A,I0)') '>>> MAKE_CAVGS RECIMG_SOURCE ', &
            trim(context), ': ', trim(params%recimg_source), ' oritype=', trim(params%oritype), &
            ' particles=', first_ptcl, '-', last_ptcl, ' stack=', trim(source_stk%to_char()), &
            ' indstk(first)=', ind_in_stk
        call flush(logfhandle)
        call source_stk%kill
    end subroutine print_recimg_source_verification
    ...
    subroutine master_initialize(self, params, cline)
        ...
        call print_recimg_source_verification(params, build_tmp, 'MASTER')
