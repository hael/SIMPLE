...
!*******************************************************************************
  ...
  call yaml_comment('Ressources')
  call yaml_map_open('Ressources',label='id001')

   call yaml_map('System','LeNovo-C30')
   call yaml_map_open('CPU')
  
        call yaml_map('Number of cores',hstD%nCPUcores)
        call yaml_map('Total Mem on system',hstD%mem_Size)
   call yaml_map_close()
   call yaml_map_open('GPU')
        call yaml_map('Number of cores',hstD%nCPUcores)
   call yaml_map_close()
  call yaml_map_close()  
  call yaml_map('node_1','*id001')
  ...
  call yaml_comment('Now we test for just error eglossary')
  egloss1=> egloss_new()
