...
use simple_refine3D_fnames,     only: refine3D_fsc_fname, refine3D_polar_ctf2_fname, &
    &refine3D_polar_refs_fname, refine3D_polar_sums_fname, refine3D_resolution_txt_fbody
implicit none
#include "simple_local_flags.inc"
real(dp), parameter :: POLAR_UNSAMPLED_FLOOR = 1.d3
contains               

    pure real(dp) function unsampled_floor( den )
        real(dp), intent(in) :: den
        unsampled_floor = min(POLAR_UNSAMPLED_FLOOR, POLAR_UNSAMPLED_FLOOR * den)
    ...
    ! Shared polar-reference low-resolution handoff used by polar=yes.
