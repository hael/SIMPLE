...
integer :: n_active, n_continuous, n_invalid_inpl, n_nonfinite_e3, ncls_out
integer :: n_continuous_logged
...
call cline%set('inpl_cont', inpl_mode%to_char())
...
do iptcl = 1, nptcls
    ...
    if( delta_e3 > max(1.e-4, 0.01*dang) )then
        ...
        if( n_continuous_logged < 8 )then
            write(logfhandle,'(a,2(i8,1x),a,3(f14.6,1x))') &
                'EUCLID_2D_METADATA CONTINUOUS_SAMPLE IPTCL/INPL: ', iptcl, inpl, &
                'E3/GRID/DELTA: ', e3, grid_e3, delta_e3
            n_continuous_logged = n_continuous_logged + 1
        endif
    endif
enddo

write(logfhandle,'(a,4i8)') 'EUCLID_2D_METADATA ACTIVE/CONTINUOUS/INVALID/NONFINITE: ', &
    n_active, n_continuous, n_invalid_inpl, n_nonfinite_e3
...
write(logfhandle,'(a,i8)') 'EUCLID_2D_METADATA CONTINUOUS_COUNT: ', n_continuous
