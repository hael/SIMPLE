...
Phase 0 and section 9 of doc/refactoring_notes/completed/uniform_test_environment_refactoring.md.
...
scripts/test_timing_run.sh, and writes
  ...
  * the inventory, one table per area, at --inventory, with the verdicts, notes
    and retired-tests table of the hand-written review record (--record).

The inventory is generated on every build and not committed; the record is the
only file edited by hand.
...
usage:
  scripts/test_review_dossier.py [--timing build_test_runs/<label>/results.tsv ...]
                                 [--inventory doc/code_overview/test_inventory.md]
                                 [--record doc/code_overview/test_review_record.md]
                                 [--dossiers build_test_runs/dossiers]
                                 [--inventory-only] [--quiet]
                                 [--coverage-after INVENTORY]   # report coverage lost by verdicts

--coverage-after reads the verdicts of a review record (or of an inventory), drops every identity marked delete / merge into / retire, and lists
...
def attach_unit_suites(tests):
    ...
    if not os.path.exists(cls_path):
        return
    cls = read(cls_path)
    logical_cls = strip_comments(cls).replace('&\n', '').replace('&', '')
    proc2mod = {}
    for m in re.finditer(r'^\s*use\s+(\w+)\s*,\s*only\s*:\s*([^\n]+)', logical_cls, re.I | re.M):
        mod = m.group(1).lower()
        for imported in m.group(2).split(','):
            name = imported.strip().split('=>', 1)[0].strip().lower()
            if name:
                proc2mod[name] = mod
    modfiles = {}
    ...
    tests_by_lower = {name.lower(): test for name, test in tests.items()}
    for m in re.finditer(r'subroutine\s+suites_(\w+)\s*\(.*?end\s+subroutine\s+suites_\1', cls, re.S | re.I):
        ...
        for p in procs:
            ...
            if mod in modfiles:
                ...
                if mm:
                    extra += mm.group(0)
        test_name = area if area.startswith('lib_') else 'unit_' + area
        t = tests_by_lower.get(test_name)
...
def propose_tier(t):
    ...
    if t.name.startswith('lib_'):
        return 'library', 'registered nightly library suite'
    if t.name == 'units':
...
def md_cell(x):
    """A Markdown table cell: escape the pipes that are not escaped yet, so that a cell read back
    from the record is written unchanged however often the inventory is regenerated."""
    return re.sub(r'(?<!\\)\|', r'\\|', str(x))


def inventory(tests, pairs, out, record):
    ...
    L = ['# Test inventory', '',
         '<!-- AUTO-GENERATED by scripts/test_review_dossier.py during the CMake build; not committed. -->', '',
         'Generated from the sources and `test_review_record.md`, the hand-written review record: the',
         'verdict and note columns and the retired-tests table come from there (section 9 of',
         '`doc/refactoring_notes/completed/uniform_test_environment_refactoring.md`). Edit the record, not this file.', '',
    ...
    for area in sorted(by_area):
        ...
        for t in sorted(by_area[area], key=lambda x: x.name):
            ...
            L.append('| ' + ' | '.join(md_cell(x) for x in row) + ' |')
        L.append('')
    L += ['## Retired tests', '', '| test | date | reason | replacement |', '|---|---|---|---|'] + keep_retired(record) + ['']
    with open(out, 'w') as fh:
        fh.write('\n'.join(L))


def keep_retired(path):
    """Rows of the retired-tests table of the review record (hand-written; kept verbatim)."""
...
def keep_verdicts(tests, path):
    """The hand-written verdict and note of each identity: from the Verdicts table of a review record
    (test | verdict | note), or from the verdict and note columns of an inventory."""
    for t in tests.values():
        t.verdict, t.note = '', ''
    if not os.path.exists(path):
        return {}
    ...
    for ln in open(path):
        if ln.startswith('## '):
            sect = ln.strip()
            continue
        if sect == '## Retired tests' or not ln.startswith('| ') or ln.startswith('| test ') or ln.startswith('|---'):
            continue
        cells = [c.strip() for c in ln.strip().strip('|').split(' | ')]
        if len(cells) >= 11:
            name, verdict, note = cells[0], cells[9], cells[10]
        elif sect == '## Verdicts' and len(cells) == 3:
            name, verdict, note = cells
        else:
            continue
        if name in tests:
            tests[name].verdict, tests[name].note = verdict, note
            kept[name] = verdict
    return kept
...
def main():
    ...
    ap.add_argument('--record', default=os.path.join(ROOT, 'doc', 'code_overview', 'test_review_record.md'),
    ...
    ap.add_argument('--inventory-only', action='store_true',
                    help='regenerate the inventory without writing per-test review dossiers')
    ap.add_argument('--quiet', action='store_true', help='no summary (the build\'s generate_test_inventory target)')
    ap.add_argument('--coverage-after', metavar='RECORD', help='report coverage lost by the verdicts in RECORD (or an inventory) and exit')
    a = ap.parse_args()
    tests = {}
