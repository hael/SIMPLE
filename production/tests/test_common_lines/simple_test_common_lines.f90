...
implicit none
integer,          parameter   :: NPLANES = 20, ORI_IND1 = 1, ORI_IND2 = 15
...
enddo
!$omp end parallel do
pair_costs = pair_costs / real(cnts)
minval     = huge(ave)
do i = 1, spiral%get_noris()
    do k = 1, spiral%get_noris()
        if( pair_costs(i,k) < minval )then
            minval = pair_costs(i,k)
            min_i  = i
            min_k  = k
        endif
    enddo
...
print *, 'minval = ', minval
print *, 'min(ORI_IND1,ORI_IND2) = ', pair_costs(ORI_IND1,ORI_IND2)
