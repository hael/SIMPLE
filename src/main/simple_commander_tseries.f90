...
contains
    ...
    subroutine exec_autorefine3D_nano( self, cline )
        ...
        type(cmdline)                 :: cline_refine3D_nano, cline_detect_atms, cline_reproject, cline_refine3D_cavgs, cline_vizoris
        ...
        do i = 1, params%maxits
            ...
            maps_dir = 'intermediate_maps'
            call simple_mkdir(iter_dir//maps_dir)
            call simple_list_files('recvol_state01_iter*', map_names)
            do j = 1, size(map_names)
                call simple_copy_file(trim(map_names(j)), iter_dir//'/'//maps_dir//'/'//trim(map_names(j)))
            end do
            ! model building
            ...
            call simple_copy_file(MSK,      iter_dir//trim(fbody)      //'_iter'//int2str_pad(i,3)//'_MSK.mrc')
        ...
        if( allocated(maps_dir)  ) deallocate(maps_dir)
