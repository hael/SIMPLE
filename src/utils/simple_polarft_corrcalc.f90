...
contains
    ...
    subroutine new( self, nrefs, pfromto, kfromto, ptcl_mask, eoarr )
        ...
        allocate(self%pfts_refs_even(self%pftsz,self%kfromto(1):self%kfromto(2),self%nrefs),&
                    ...
                    &self%sqsums_ptcls(1:self%nptcls),self%wsqsums_ptcls(1:self%nptcls),self%fftdat(params_glob%nthr),&
    ...
    subroutine ref_reg_cc_dev( self, eulspace, ptcl_eulspace, glob_pinds )
        ...
        real     :: ptcl_ctf(self%pftsz,self%kfromto(1):self%kfromto(2),self%nptcls)
        real     :: euls(3), euls_ref(3), theta
        ptcl_ctf = real(self%pfts_ptcls * self%ctfmats)
        !$omp parallel do collapse(2) default(shared) private(i,iref,euls_ref,euls,iptcl,theta) proc_bind(close) schedule(static)
        do iref = 1, self%nrefs
            do i = 1, self%nptcls
                iptcl    = glob_pinds(i)
                euls_ref =      eulspace%get_euler(iref)  * pi / 180.
    ...
    subroutine gencorrs_2( self, iref, iptcl, shift, cc )
        class(polarft_corrcalc), intent(inout) :: self
        integer,                 intent(in)    :: iref, iptcl
        real(sp),                intent(in)    :: shift(2)
        real(sp),                intent(out)   :: cc(self%nrots)
    ...
    subroutine gencorrs_prob( self, iptcl, iref, prob )
        class(polarft_corrcalc), intent(inout) :: self
        integer,                 intent(in)    :: iptcl, iref
    ...
    real(dp) function gencorr_prob_for_rot_8( self, pft_ref, iptcl )
        class(polarft_corrcalc), intent(inout) :: self
        complex(dp), pointer,    intent(inout) :: pft_ref(:,:)
        integer,                 intent(in)    :: iptcl
        ...
        do k = self%kfromto(1),self%kfromto(2)
            ...
            gencorr_prob_for_rot_8 = gencorr_prob_for_rot_8 + dexp(-diffsq / denom)
        end do
