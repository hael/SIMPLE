...
implicit none
    interface
        function expm1(x) bind(c) result(res)
            use, intrinsic :: iso_c_binding, only: c_double
            implicit none
            real(c_double) :: x,res
        end function expm1

        function expm1f(x) bind(c) result(res)
            use, intrinsic :: iso_c_binding, only: c_float
            implicit none
            real(c_float) :: x,res
        end function expm1f

        function sinhf(x) bind(c) result(res)
            use, intrinsic :: iso_c_binding, only: c_float
            implicit none
            real(c_float) :: x,res
        end function sinhf
    ...
    integer(dp):: i, t1,n, j
    integer(dp),parameter:: nsize=10000, jsize=5000
    real :: ansme,ansm1c,ansintrinsic
    real(kind=4) :: vector_arg(nsize), sarg(nsize),tsum(6), dsum(6), dmin(6),dmax(6), tmp(nsize)
 ...
 !   print *," Gen args ", toc()
    ...
    do j =1,jsize
        t1=tic()
     !   do i=1,nsize
        tmp = exp( vector_arg )/ sqrt(vector_arg) !exp( vector_arg )/ sqrt( vector_arg )
    !    end do
            tsum(1)=tsum(1)+toc(t1)
            tmp=0.
        t1=tic()
        do i=1,nsize
            tmp = (expm1( REAL(vector_arg(i),8) ) +1) / sqrt_flt0( real(vector_arg(i), sp)) !(vector_arg(i)) )!* InvSqrt( vector_arg(i) )! / sqrtf0( vector_arg(i) )
        end do
        tsum(2)=tsum(2)+toc(t1)
        tmp=0.
        t1=tic()
        do i=1,nsize
            tmp = InvSqrt( real(vector_arg(i))) * exp(vector_arg(i))
        end do
        ...
        t1=tic()
        do i=1,nsize
            ansme = sinh_flt_me(real(sarg(i),sp)) / sarg(i)
        end do
...
!        print *, 'arg ',vector_arg(j),'| exp-1 ', exp(vector_arg(j)), '|  my expm1 ',expm1fme(vector_arg(j))+1.
    ...
    print*,"  expm1(x)/sqrt_flt0(x) ", tsum(2)/real(jsize),"   ", tsum(2)/tsum(1)
    print*,"  expm1f(x)*InvSqrt(x)  ", tsum(3)/real(jsize),"   ", tsum(3)/tsum(1)
    print*,''
    print*,"  sinh(x)/x              ", tsum(4)/real(jsize)
    print*,"  sinhf(x)/x             ", tsum(5)/real(jsize), "   ", (tsum(5))/tsum(4)
    print*,"  sinch(x)               ", tsum(6)/real(jsize), "   ", (tsum(6))/tsum(4)
    tsum=0.
    dsum=0.
    print*,''
