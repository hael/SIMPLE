...
contains
    ...
    module subroutine norm_noise_mask_pad_fft(self, lmsk, mskrad, self_out)
        ...
        class(image), intent(inout) :: self_out
        integer       :: n1, n2, n1o, n2o, h1o, h2o
        integer       :: i, j, npix, starts(3), x0, y0, minlen, np
        integer       :: wbg, wbg1, wbg2, i1, i2, border_count
        integer       :: xo_map(self%ldim(1)), yo_map(self%ldim(2)), yo
        real(dp)      :: mean_dp, var_dp, sum_dp, sum_sq_dp, rnpix, invstd_dp, sv, border_sum_dp, edge_mean_dp
        real(c_float) :: mean_sp, invstd_sp, scale_cmat, x_sp, rad_sq, ave, r2, cis2, cjs2, v_val, edge_mean_sp
        ...
        ! ============================================================
        ! 1) NOISE NORMALIZATION stats (mask-based, DP scalars)
        ! ============================================================
        sum_dp    = 0.0_dp
        sum_sq_dp = 0.0_dp
        npix      = 0
        ...
        invstd_sp = real(invstd_dp, c_float)
        ! ============================================================
        ! 2) Compute outside-mask average using normalized values.
        ! ============================================================
        ...
        do i = 1, n1
            ...
            do j = 1, n2
                ...
                if( r2 > rad_sq )then
                    np = np + 1
                    if( do_norm )then
                        v_val = (self%rmat(i,j,1) - mean_sp) * invstd_sp
                    else
        ...
        ! ============================================================
        ! 3) Compute the post-mask edge mean, matching ifft_mask_pad_fft.
        ! ============================================================
        ...
        do i = 1, wbg1
            cis2 = mem_msk_cs2(i)
            do j = 1, n2
                call normalized_masked_value(i, j, cis2, mem_msk_cs2(j), v_val)
        ...
        ! ============================================================
        ! 4) Write normalized, masked pixels into padded+fftswapped output.
        ! ============================================================
        self_out%ft   = .false.
        ...
        do j = 1, n2
            ...
            yo   = yo_map(j)
            do i = 1, n1
                call normalized_masked_value(i, j, mem_msk_cs2(i), cjs2, v_val)
                self_out%rmat(xo_map(i), yo, 1) = v_val
            end do
        end do
        ! ============================================================
        ! 5) FFT (FFTW r2c) + scale with reciprocal (OUTPUT size)
        ! ============================================================
        call fftwf_execute_dft_r2c(self_out%plan_fwd, self_out%rmat, self_out%cmat)
        scale_cmat    = 1.0_c_float / real(n1o*n2o, c_float)
        self_out%cmat = self_out%cmat * scale_cmat
    ...
    contains
        ...
        subroutine normalized_masked_value(i, j, cis2, cjs2, v_val)
            integer,       intent(in)  :: i, j
            real(c_float), intent(in)  :: cis2, cjs2
            real(c_float), intent(out) :: v_val
            real(c_float) :: r2, e
            if( do_norm )then
                v_val = (self%rmat(i,j,1) - mean_sp) * invstd_sp
            else
        ...
        end subroutine normalized_masked_value
    ...
    module subroutine mask_fft( self, mskrad )
        ...
        ! fftshift (store back into self%rmat)
        do j = 1, n2
            jj = mod(j + h2 - 1, n2) + 1
            do i = 1, n1
                ii = mod(i + h1 - 1, n1) + 1
    ...
    module subroutine mask_pad_fft( self, mskrad, self_out )
        ...
        ! ============================================================
        ! 1) Compute average outside radius (ave)
        ! ============================================================
        minlen = minval(self%ldim(1:2))
        minlen = min(nint(2.0*(mskrad + COSMSKHALFWIDTH)), minlen)
        rad_sq = real(mskrad*mskrad, c_float)
        ...
        ! ============================================================
        ! 2) Compute post-mask edge_mean from a border band (like taper routine)
        ...
        edge_mean_sp = real(edge_mean_dp, c_float)
        ! ============================================================
        ! 3) Pad background with edge_mean (post-mask) then write masked pixels
