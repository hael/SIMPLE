...
contains
    ...
    subroutine init_cmd_dict
        ...
        call cmd_dict%push('k_nn',          'local nearest-neighbor count for graph-based diffusion methods; 0=auto where supported{10}')
        ...
        call cmd_dict%push('match_src',     'particle image source for matching(raw|den){raw}')
        ...
        call cmd_dict%push('prob_neigh_mode', 'prob_neigh neighborhood mode(state|geom|sum){state}')
        ...
        call cmd_dict%push('rec_src',       'particle image source for reconstruction(raw|den){raw}')
        ...
        call cmd_dict%push('steering',      'graph steering representation(none|so2|se2){none}')
