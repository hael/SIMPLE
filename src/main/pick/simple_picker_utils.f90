...
contains
    ...
    subroutine exec_refpick( params, micname, boxfile_out, thumb_den_out, smpd, nptcls, pickrefs, dir_out, nboxes_max )
        ...
        l_roi           = trim(params%pick_roi).eq.'yes'
        ...
        if( present(nboxes_max) )then
            call refp%new(params%pcontrast, params%particle_density, SMPD_SHRINK1, pickrefs, offset=OFFSET, roi=l_roi, nboxes_max=params%nboxes_max, thres=params%thres, backend=params%refpick_backend)
        else
            call refp%new(params%pcontrast, params%particle_density, SMPD_SHRINK1, pickrefs, offset=OFFSET, roi=l_roi, thres=params%thres, backend=params%refpick_backend)
        endif
        call refp_refine%new(params%pcontrast, params%particle_density, SMPD_SHRINK2, pickrefs, offset=1, thres=params%thres, backend=params%refpick_backend)
    ...
    subroutine exec_segdiampick( params, micname, boxfile_out, smpd, nptcls, moldiam_max, dir_out )
        ...
        boxfile = basename(fname_new_ext(micname,'box'))
        if( present(dir_out) ) boxfile = dir_out//'/'//boxfile%to_char()
        call picker%pick(micname, SMPD, moldiam_max, params%pcontrast)
    ...
    subroutine exec_gaupick( params, micname, boxfile_out, smpd, nptcls, dir_out, nboxes_max )
        ...
        ! execution fork
        nmoldiams = 1
        if( params%nmoldiams > 1 )then
            ! array of diameters
            nmoldiams = params%nmoldiams
            box       = round2even((1.0+BOX_EXP_FAC)*params%moldiam_max/ smpd) + 2
            moldiams  = equispaced_vals(params%moldiam, params%moldiam_max, nmoldiams)
            call alloc_imgarr(1, [box, box,1], smpd, pickrefs)
            l_competitive  = .true.
        else if( (params%nmoldiams==1) .and. (params%interactive.eq.'yes') )then
            ! still relevant?
            box = round2even((1.0+BOX_EXP_FAC)*params%moldiam/ smpd) + 2
            sig = params%moldiam / (smpd * GAUSIG)
            call alloc_imgarr(1, [box, box,1], smpd, pickrefs)
            call pickrefs(1)%gauimg2D(sig, sig)
        else if( .not.(params%multi_moldiams.eq.'') )then
            ! multiple moldiam pick that uses multiple gaussians, generates .box file outputs
            istr        = 1
            num_entries = 0
            do
                if(params%multi_moldiams(istr:istr) .eq. ' ') then
                    num_entries = num_entries + 1
                    exit
                end if
                if(istr > len(params%multi_moldiams)) exit
                if(params%multi_moldiams(istr:istr) .eq. ',') then
                    num_entries = num_entries + 1
                end if
                istr = istr + 1
            ...
            call parsestr(params%multi_moldiams,',',moldiams_str,nmoldiams)
            ...
            kind = trim(params%pickkind)
            if(params%interactive == "yes") then
                if( params%ring == "yes" ) kind = 'ring'
            endif
            box = round2even((1.0+BOX_EXP_FAC)*params%moldiam/ smpd) + 2
            sig = params%moldiam / (smpd * GAUSIG)
            select case(kind%to_char())
                ...
                case('ring')
                    ...
                    call pickrefs(1)%gauimg2D(sig, sig)
                    call pickrefs(2)%soft_ring([box,box,1], smpd, 0.5*params%moldiam/smpd)
                case DEFAULT
