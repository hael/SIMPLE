...
`automsk=yes|tight` requires `filt_mode=nonuniform|nonuniform_lpset`; all other
filtering modes require `automsk=no`.

`envfsc=no` is the default and the provisional broad-sphere FSC remains the
reported curve. **`envfsc=yes` is an unfinished branch and exits with a hard
error.** Re-enabling it needs a simple, fast density-based mask generated on the
fly from the current map. The NU-evidence envelope must never reach the FSC. FSC
...
`automsk != no`, the selected NU-derived reference is solvent-flattened with the
...
8. writes NU-filtered even, odd, merged, and local-resolution products
...
- `nu_envmask3D_stateNN.mrc` is generated from the completed NU evidence bank
  and serves matching-reference preprocessing whenever `automsk != no`
- legacy density-derived `automask3D_stateNN.mrc` may be reused as a
  reference-mask transition fallback, but is not generated by `volassemble` or
  consumed by the FSC path
...
Each accepted challenger also updates the raw best-candidate cost used by the
NU-evidence envelope before candidate-scale objective smoothing. Consequently
the envelope null and margin describe the same accepted bank that generated the
...
Whenever `automsk != no`, the selected independent or merged reference is
background-zeroed from the NU-envelope transition and multiplied by
`nu_envmask3D_stateNN.mrc` before projection. Reference preparation falls back
to a compatible legacy density mask and then to the ordinary spherical mask.
This lag-by-one bootstrap leaves the first iteration spherical when no prior
artifact exists.
...
- `envfsc=yes`: exit with a hard error because the branch is unfinished
- future completed branch: generate a fast density mask on the fly, find the
  unmasked FSC 0.8 crossing, randomize both half-map phases beyond that shell,
  apply the transient mask, and use `(FSC_masked - FSC_randomized_masked) /
  (1 - FSC_randomized_masked)` above the transition
...
With `automsk=yes`, matching references are solvent-flattened with the lagged
NU-evidence envelope; there is no separate reference-mask control.

The `envfsc` default is overridable, but `envfsc=yes` exits with a hard error
until fast on-the-fly density-mask generation is implemented.
