...
contains
    ...
    subroutine exec_prime2D_srch( self, pftcc, a, pfromto, greedy, shclogic, nnmat )
        ...
        if( self%refine .eq. 'greedy' .or. ggreedy )then
            ...
            call self%stochastic_srch(pftcc, a, pfromto, shclogic=shclogic)
        endif
    ...
    !>  \brief  executes the greedy rotational search
    subroutine stochastic_srch( self, pftcc, a, pfromto, shclogic, nnmat )
        use simple_oris, only: oris
        use simple_ori,  only: ori
        ...
        real      :: prevcorrs(pfromto(1):pfromto(2))
        logical   :: sshclogic, found_better
