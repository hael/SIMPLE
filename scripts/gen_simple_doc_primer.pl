...
# extract comments
my $comments = extract_comments($simple_exec_src_abspath);
chomp($comments);
my $comments_distr = extract_comments($simple_distr_exec_src_abspath);
...
foreach my $prg (@prgnames_distr){
    $prgcmdlineinstr_distr{$prg} = `SIMPLE_PATH=$SIMPLE_SOURCE_PATH  $SIMPLE_INSTALL_PATH/bin/simple_distr_exec prg=$prg`;
