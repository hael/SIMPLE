...
contains
    ...
    subroutine init_cmd_dict
        ...
        call cmd_dict%push('k_nn',          'local nearest-neighbor count for graph-based diffusion methods{10}')
        ...
        call cmd_dict%push('so3_graph',     'SO(3) cls_split graph construction(cluster2d|projection_registration){projection_registration}')
        call cmd_dict%push('so3_local_cluster2d_maxits', '# local cluster2D iterations for pca_mode=diff_map_so3 cluster2d graph{1}')
        call cmd_dict%push('so3_steering',  'SO(3) steering representation(none|so2|se2){so2}')
        call cmd_dict%push('speckind',      'power spectrum kind(real|power|sqrt|log|phase){sqrt}')
        call cmd_dict%push('steerable_denoise_mode', 'steerable denoise mode(coeffproj|transport){coeffproj}')
