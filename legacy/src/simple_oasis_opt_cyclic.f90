...
contains
    ...
    subroutine oasis_minimize( self, spec, lowest_cost )
        ...
        logical                         :: arezero(spec%ndim), corrected, corrections(spec%ndim)
        integer                         :: i, j, k, nbetter, conv, niter!, cnt(spec%ndim)
        real                            :: y
        ...
        do i=1,spec%nrestarts
            ...
            do j=1,spec%maxits  ! iterations loop
                ...
                if( nbetter > 0 )then
                    ...
                    do k=1,spec%ndim
                        if( corrections(k) )then
                            self%spec_linmin%xi(k) = 0.
                        else
                            self%spec_linmin%xi(k) = spec%x(k)-self%spec_linmin%x(k)
                        endif
                    end do
                    ...
                    if( y < self%yb )then
                        ...
                        ! correct if cyclic
                        call check_and_correct_vec(spec, spec%x, corrected)
                        ! add to probabilistic model
                        do k=1,spec%ndim
                            call self%vars(k)%add(spec%x(k))
                        end do
                        if( corrected )then ! re-set the means
                            do k=1,spec%ndim
                                call self%vars(k)%reset_mean(spec%x(k))
                            end do 
                        endif
                    endif
                endif
            ...
            subroutine oasis_srch
                ...
                do ii=1,spec%nsample 
                    ...
                    if( y <= self%yb )then
                        ...
                        ! correct if cyclic
                        corrections(i) = .false.
                        if( spec%cyclic(i) ) then
                            do while(spec%x(i) < spec%limits(i,1)) 
                                spec%x(i) = spec%x(i)+spec%limits(i,2)
                                corrections(i) = .true.
                            end do
                            do while(spec%x(i) > spec%limits(i,2))
                                spec%x(i) = spec%x(i)-spec%limits(i,2)
                                corrections(i) = .true.
                            end do
                        endif
                        if( corrections(i) ) call self%vars(i)%reset_mean(spec%x(i))
                        ! update the standard deviations
                        self%sdevs(i) = max(spec%ftol,sqrt(self%vars(i)%get_var())) 
                    else
