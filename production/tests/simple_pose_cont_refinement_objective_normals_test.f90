...
!> Compare the fused implementation with an independently accumulated scalar oracle.
    subroutine run_objective_normals()
        ...
        character(len=:), allocatable :: component_path, normal_path, summary_path, blas_path
        integer :: blas_unit, box_index, component_unit, normal_unit, summary_unit
        ...
        blas_path = evidence_directory//'/blas_normal_equations.tsv'
        ...
        open (newunit=blas_unit, file=blas_path, status='replace', action='write')
        ...
        write (blas_unit, '(a)') 'box'//achar(9)//'variance'//achar(9)//'pose'//achar(9)// &
            &'active_samples'//achar(9)//'objective_error_fused'//achar(9)// &
            &'objective_error_explicit'//achar(9)//'gradient_error_fused'//achar(9)// &
            &'gradient_error_explicit'//achar(9)//'hessian_error_fused'//achar(9)// &
            &'hessian_error_explicit'

        do box_index = 1, size(ACCEPTANCE_BOXES)
            call build_acceptance_fixture(ACCEPTANCE_BOXES(box_index), fixture)
            call test_fixture(fixture, component_unit, normal_unit, summary_unit, blas_unit)
        end do
...
!> Exercise all variance and pose combinations for one untouched acceptance fixture.
    subroutine test_fixture(fixture, component_unit, normal_unit, summary_unit, blas_unit)
        type(calibration_fixture), intent(in) :: fixture
        integer, intent(in) :: component_unit, normal_unit, summary_unit, blas_unit
        ...
        do variance_profile = 1, N_VARIANCE_PROFILES
            ...
            do pose_state = 1, N_POSE_STATES
                call test_case(workspace, fixture, raw_observed, sigma2, variance_profile, pose_state, &
                    &component_unit, normal_unit, summary_unit, blas_unit)
            end do
        end do
        call workspace%kill
...
!> Run one weighted pose case and retain each compared component.
    subroutine test_case(workspace, fixture, raw_observed, sigma2, variance_profile, pose_state, &
        ...
        integer :: active_samples, blas_active_samples, data_status
