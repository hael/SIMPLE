!==Class simple_rinv_ppca
!
! simple_rinv_ppca is the SIMPLE class for rotationally invarienat probabilistic principal 
! component analysis. This code should be able to deal with many millions of particle 
! images. The code is distributed with the hope that it will be useful, but _WITHOUT_ 
! _ANY_ _WARRANTY_. Redistribution or modification is regulated by the 
! GNU General Public License. *Author:* Hans Elmlund, 2015-08-23.
!
!==Changes are documented below
!
module simple_rinv_ppca
...
type rinv_ppca
    ...
    integer           :: I           !< number of in-plane rotations
...
contains
    ...
    !>  \brief  is a constructor
    subroutine new( self, N, D, I, Q )
        use simple_jiffys, only: alloc_err
        class(rinv_ppca), intent(inout) :: self
        integer, intent(in)        :: N, D, I, Q
        integer                    :: alloc_stat
        call self%kill
        self%N = N
        self%D = D
        ...
        self%X(self%I,self%D,1), self%E_znzn(self%Q,self%Q), stat=alloc_stat )
        call alloc_err( 'new; simple_rinv_ppca', alloc_stat )
        self%W      = 0.
        self%E_zn   = 0.
        self%W_1    = 0.
        self%W_2    = 0.
    ...
    function get_feats_ptr( self ) result( ptr )
        class(rinv_ppca), intent(inout), target :: self
        real, pointer :: ptr(:,:)
        ptr => self%E_zn(:,:,1)
    end function
    ...
    function generate_2( self, feat, AVG ) result( dat )
        class(rinv_ppca), intent(in)    :: self
    ...
    subroutine master( self, datastk, recsz, featstk, maxpcaits, feats_txt )
        ...
        call fopen_err('master; simple_rinv_ppca, 1', file_stat)
        ...
        call fopen_err('master; simple_rinv_ppca, 2', file_stat)
        ...
        if( present(feats_txt) )then
            ...
            call fopen_err('master; simple_rinv_ppca, 3', file_stat)
        endif 
        p = 0.
        k = 0
        call self%init
    ...
    subroutine em_opt( self, p, err )
        ...
        do i=1,self%N
            ...
            do j=self%I,1,-1 ! unrotated always last
                self%E_zn(i,:,:) = matmul(self%MinvWt,self%X(j,:,:))
                self%E_znzn = matmul(self%E_zn(i,:,:),transpose(self%E_zn(i,:,:)))
                ! Prepare for update of W (M-step)
                self%W_1 = self%W_1+matmul(self%X(j,:,:),transpose(self%E_zn(i,:,:)))
