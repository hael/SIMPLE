...
struct JSONResponse {
	...
	std::map<std::string, std::string> 						newformat;
...
void viewIni3D (JSONResponse* response, struct http_message* message) {
	...
	if (getRequestVariable(message, "folder", directory)){
		...
		if(fileExists(directory + "/reprojs.mrc") && fileExists(directory + "/prime2D_selected_clusters.mrc")){
			...
			response->newformat["targetflags"] += "{\"id\":\"cluster averages\", \"contrast\" : \"6\", \"brightness\" : \"128\", \"scale\" : \"1\", \"zoom\" : \"1\"} ]";
			response->newformat["sourcefiles"] = "[\"reprojs.mrc\", \"prime2D_selected_clusters.mrc\"]";
			clusters = new MRCFile();
			...
			for(count = 0; count < clustercount; count++){
				...
				thumbnails += "{ \"targetflag\": \"1\", \"sourcefile\": \"1\", \"image\": \""+ std::to_string(count) +"\"}";
				thumbnails += "],";
			}
			thumbnails.pop_back();
			thumbnails += "]";
...
void webServer(bool& multiuser){
	...
	if (multiuser) {
		std::cout << "" << std::endl;
		std::cout << "Multiuser is enabled" << std::endl;
