...
contains
    ...
    subroutine obsfield_new( self, lims, nyq )
        ...
        allocate(self%grid_den(self%grid_lims(1,1):self%grid_lims(1,2), &
            self%grid_lims(2,1):self%grid_lims(2,2), self%grid_lims(3,1):self%grid_lims(3,2)), &
            source=0.d0)
        allocate(self%grid_w(self%grid_lims(1,1):self%grid_lims(1,2), &
            self%grid_lims(2,1):self%grid_lims(2,2), self%grid_lims(3,1):self%grid_lims(3,2)), &
            source=0.d0)
    ...
    subroutine obsfield_reset( self )
        ...
        if( allocated(self%grid_w) ) self%grid_w = 0.d0
    ...
    subroutine obsfield_kill( self )
        ...
        if( allocated(self%grid_w) ) deallocate(self%grid_w)
        self%pf          = OSMPL_PAD_FAC
        self%nyq         = 0
        self%iwinsz      = 0
        self%wdim        = 0
    ...
    ! Use one work-sharing loop over h and protect possible cell collisions
    ! with OpenMP atomics; the wider h-coloring used by reconstructor splats
    ! is only needed when every sample writes a full KB stencil.
    subroutine insert_plane_oversamp( self, se, o, fpl, pwght )
        ...
        integer     :: nsym, isym, h, k, hp, kp, pf_local
        ...
        !$omp parallel default(shared) private(isym,R,h,k,h_sq,k_max_h,k_lo,k_hi,&
        ...
        do isym = 1, nsym
            ...
            do h = fpllims(1,1), fpllims(1,2)
                ...
                do k = k_lo, k_hi
                    ...
                    !$omp atomic update
                    self%grid_w(coord(1),coord(2),coord(3)) = &
                        self%grid_w(coord(1),coord(2),coord(3)) + cell_w
                    nobs_add = nobs_add + 1
                enddo
            enddo
            !$omp end do
    ...
    subroutine obsfield_append_field( self, src )
        ...
        self%grid_w   = self%grid_w   + src%grid_w
    ...
    subroutine obsfield_extract_polar( self, eulspace, nrefs, kfromto, polar_x, polar_y, pfts, ctf2 )
        ...
        !$omp& win1_1,win1_2,win1_3,c1,c2,c3,a1,a2,a3,l,m,n,l_conj,cell_num,cell_w) proc_bind(close)
