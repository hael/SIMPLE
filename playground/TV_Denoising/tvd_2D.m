...
% M. Figueiredo, etc.
function ret = tvd_2D(image, lambda, Niter)
[X, Y]    = size(image);
image_1D  = image(:);
N         = length(image_1D);
sparse_Id = speye(N);
der_mat   = sparse_Id(2:N, :) - sparse_Id(1:N-1, :);
...
for k = 1:Niter
    banded_mat = sparse(1:N-1, 1:N-1, abs(der_x)/lambda) + adjoint; % sparse banded matrix
