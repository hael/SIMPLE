!@descr: greedy block search + greedy tree descent strategy for 2D classification
module simple_strategy2D_greedy_tree
use simple_pftc_srch_api
use simple_strategy2D_alloc,      only: s2D
use simple_strategy2D_tree_utils, only: descend_tree_greedy, get_tree_for_ref
use simple_strategy2D,            only: strategy2D
use simple_strategy2D_srch,       only: strategy2D_spec
use simple_builder,               only: builder
implicit none

public :: strategy2D_greedy_tree
private
#include "simple_local_flags.inc"

logical, parameter :: DEBUG = .false.

type, extends(strategy2D) :: strategy2D_greedy_tree
contains
    procedure :: new  => new_greedy_tree
    procedure :: srch => srch_greedy_tree
    procedure :: kill => kill_greedy_tree
end type strategy2D_greedy_tree

contains

    subroutine new_greedy_tree( self, params, spec, build )
        class(strategy2D_greedy_tree), intent(inout) :: self
        class(parameters),             intent(in)    :: params
        class(strategy2D_spec),        intent(inout) :: spec
        class(builder),                intent(in)    :: build
        call self%s%new(params, spec, build)
        self%spec = spec
    end subroutine new_greedy_tree

    subroutine srch_greedy_tree( self, os )
        class(strategy2D_greedy_tree), intent(inout) :: self
        ...
        if( .not. allocated(self%s%b_ptr%subspace_full2sub_map) )then
            THROW_HARD('greedy_tree search requires subspace_full2sub_map. Check builder construction.')
        endif
        ntrees = self%s%b_ptr%block_tree%get_n_trees()
        if( ntrees <= 0 )then
            THROW_HARD('greedy_tree search requires at least one block tree.')
        endif
        nrefs_coarse_eval = 0
        nrefs_tree_eval   = 0
        ...
        if( nrefs_coarse_eval > 0 )then
            ...
            if( itree > 0 ) call descend_tree_greedy(self%s, itree, nrefs_tree_eval)
        endif
        ...
        if( DEBUG ) write(logfhandle,*) '>>> strategy2D_greedy_tree::FINISHED GREEDY+PTREE SEARCH'
    end subroutine srch_greedy_tree

    subroutine kill_greedy_tree( self )
        class(strategy2D_greedy_tree), intent(inout) :: self
        call self%s%kill
    end subroutine kill_greedy_tree

end module simple_strategy2D_greedy_tree
