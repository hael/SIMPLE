!@descr: platform test of OpenMP target offload: device setup, persistence, async, cuFFT against FFTW, cuBLAS, KB device forms
! Cyril's offload test program, moved into the library by the utils review (plan, section 9.7) and
! run as `OMP_TARGET_OFFLOAD=MANDATORY simple_test_exec test=openmp_offload nthr=<n> device=<id>`
! (the openmp_offload platform entry, registered with USE_OPENMP_OFFLOAD). Its checks used to stop
! the program, the text ones with status 0; they now stop through THROW_HARD with the same message
! (the numeric ones with their code in the message), so a failure exits non-zero. Built without
! USE_OPENMP_OFFLOAD it says that it was skipped.
...
implicit none
private
public :: run_openmp_offload_tests
...
contains
    ...
    !> the offload checks on device params%device with params%nthr host threads (nthr= and device=
    !! on the command line)
...
!$omp end target 
        if( is_host ) then
            THROW_HARD('Fatal error: offloading failed, code is running on host')
        endif
        print *, 'Device ID from omp_get_device_num:            ', device_id
        if( device_id /= p%device ) then
            THROW_HARD('Fatal error: offloading failed, could not set the correct device')
        endif
      ...
      contains
        ...
        subroutine persistence_test
            ...
            if( abs(b(1) - 6.0d0) > 1e-10 ) then
                THROW_HARD('Fatal error: persistence test failed, data did not persist on device')
            endif
        ...
        subroutine persistence_test2
            ...
            print *, '  Final result (b(N)) : ', b(N)
            if( abs(b(1) - 6.0d0) > 1e-10 ) then
                THROW_HARD('Fatal error: persistence test2 failed, data did not persist on device')
            endif
            if( abs(b(N) - 6.0d0) > 1e-10 ) then
                THROW_HARD('Fatal error: persistence test2 failed, data did not persist on device')
            endif
        ...
        subroutine numerical_test
            ...
            if( any(abs(c-d) > 1e-4) ) then
                THROW_HARD('Fatal error: numerical test failed, results do not match')
            endif
        ...
        subroutine async_test
            ...
            if( tserial > 0.9*(tcpu+tgpu) .and. tasync>0.9*(max(tcpu,tgpu)) ) then
                ...
                THROW_HARD('Fatal error: asynchronous execution should not be slower than serial execution')
            endif
        ...
        subroutine test_fftw_vs_cufft_1d_in_place
            ...
            if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 58')
            plan_fftw = fftwf_plan_dft_r2c_1d(int(n1,c_int), buf1_fftw, buf1_fftw_c, FFTW_ESTIMATE)
            if (.not. c_associated(plan_fftw)) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 59')
            ...
            !$omp target data map(tofrom:buf1_cufft)
              ...
              if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 60')
            !$omp end target data
            ...
            if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 62')
        ...
        subroutine test_fftw_vs_cufft_2d_in_place
            ...
            if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 63')
            plan_fftw = fftwf_plan_dft_r2c_2d(int(ny,c_int), int(nx,c_int), &
              buf2_fftw, buf2_fftw_c, FFTW_ESTIMATE)
            if (.not. c_associated(plan_fftw)) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 64')
            ! execute
            ...
            !$omp target data map(tofrom:buf2_cufft)
              ...
              ierr = cufftExecR2C(plan_cufft, p_r, p_c)
              if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 65')
            !$omp end target data
            ! report
            ...
            ierr = cufftDestroy(plan_cufft)
            if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 67')
            call fftwf_destroy_plan(plan_fftw)
        ...
        subroutine test_fftw_vs_cufft_2d_in_place_roundtrip_timing
            ...
            if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 84')
            ierr = cufftPlan2d(plan_cufft_inv, int(ny,c_int), int(nx,c_int), CUFFT_C2R)
            if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 85')
            plan_fftw_fwd = fftwf_plan_dft_r2c_2d(int(ny,c_int), int(nx,c_int), &
              buf2_fftw, buf2_fftw_c, FFTW_ESTIMATE)
            if (.not. c_associated(plan_fftw_fwd)) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 86')
            plan_fftw_inv = fftwf_plan_dft_c2r_2d(int(ny,c_int), int(nx,c_int), &
              buf2_fftw_c, buf2_fftw, FFTW_ESTIMATE)
            if (.not. c_associated(plan_fftw_inv)) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 87')
            ! execute
            t = tic()
            do rep = 1, repeats
            ...
            !$omp target data map(tofrom:buf2_cufft)
              ...
              if (ierr /= CUDA_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 88')
              t = tic()
              do rep = 1, repeats
                ierr = cufftExecR2C(plan_cufft_fwd, p_r, p_c)
                if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 89')
                ierr = cufftExecC2R(plan_cufft_inv, p_c, p_r)
                if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 90')
                ierr = cudaDeviceSynchronize()
                if (ierr /= CUDA_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 91')
            ...
            if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 93')
            ierr = cufftDestroy(plan_cufft_inv)
            if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 94')
        ...
        subroutine test_fftw_vs_cufft_3d_in_place_roundtrip_timing
            ...
            if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 95')
            ierr = cufftPlan3d(plan_cufft_inv, int(nz,c_int), int(ny,c_int), int(nx,c_int), CUFFT_C2R)
            if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 96')
            ...
            if (.not. c_associated(plan_fftw_fwd)) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 97')
            plan_fftw_inv = fftwf_plan_dft_c2r_3d(int(nz,c_int), int(ny,c_int), int(nx,c_int), &
              buf3_fftw_c, buf3_fftw, FFTW_ESTIMATE)
            if (.not. c_associated(plan_fftw_inv)) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 98')
            ! Execution
            t = tic()
            do rep = 1, repeats
            ...
            !$omp target data map(tofrom:buf3_cufft)
              ...
              ierr = cudaDeviceSynchronize()
              if (ierr /= CUDA_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 99')
              t = tic()
              do rep = 1, repeats
                ierr = cufftExecR2C(plan_cufft_fwd, p_r, p_c)
                if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 100')
                ierr = cufftExecC2R(plan_cufft_inv, p_c, p_r)
                if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 101')
                ierr = cudaDeviceSynchronize()
                if (ierr /= CUDA_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 102')
            ...
            ierr = cufftDestroy(plan_cufft_fwd)
            if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 104')
            ierr = cufftDestroy(plan_cufft_inv)
            if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 105')
            call fftwf_destroy_plan(plan_fftw_fwd)
            call fftwf_destroy_plan(plan_fftw_inv)
        ...
        subroutine test_fftw_vs_cufft_1d_many
            ...
            if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 68')
            ...
            if (.not. c_associated(plan_fftw)) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 69')
            ! execute
            call fftwf_execute_dft_r2c(plan_fftw, buf1_fftw, buf1_fftw_c)
            !$omp target data map(tofrom:buf1_cufft)
              ...
              ierr = cufftExecR2C(plan_cufft, p_r, p_c)
              if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 70')
            !$omp end target data
            ! report
            do batch_idx = 1, nbatch
            ...
            ierr = cufftDestroy(plan_cufft)
            if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 72')
            call fftwf_destroy_plan(plan_fftw)
            deallocate(buf1_cufft, buf1_fftw)
        ...
        subroutine test_fftw_vs_cufft_2d_many
            ...
            if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 73')
            ...
            if (.not. c_associated(plan_fftw)) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 74')
            call fftwf_execute_dft_r2c(plan_fftw, buf2_fftw, buf2_fftw_c)
            !$omp target data map(tofrom:buf2_cufft)
                p_r = omp_get_mapped_ptr(c_loc(buf2_cufft(1,1,1)), 0)
                p_c = omp_get_mapped_ptr(c_loc(buf2_cufft(1,1,1)), 0)
                ierr = cufftExecR2C(plan_cufft, p_r, p_c)
                if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 75')
            !$omp end target data
            do batch_idx = 1, nbatch
            ...
            ierr = cufftDestroy(plan_cufft)
            if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 77')
            call fftwf_destroy_plan(plan_fftw)
            deallocate(buf2_cufft, buf2_fftw)
        ...
        subroutine test_fftw_vs_cufft_1d_many_c2r
            ...
            if (.not. c_associated(plan_fftw_fwd)) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 78')
            ...
            if (.not. c_associated(plan_fftw_inv)) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 79')

            ierr = cufftPlanMany(plan_cufft_inv, rank, n, inembed, istride, idist, onembed, ostride, odist, CUFFT_C2R, howmany)
            if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 80')
            ...
            !$omp target data map(tofrom:buf1_cufft)
              ...
              p_c = omp_get_mapped_ptr(c_loc(buf1_cufft(1,1)), 0)
              ierr = cufftExecC2R(plan_cufft_inv, p_c, p_r)
              if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 81')
            !$omp end target data
            ...
            ierr = cufftDestroy(plan_cufft_inv)
            if (ierr /= CUFFT_SUCCESS) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 83')
            call fftwf_destroy_plan(plan_fftw_inv)
            deallocate(buf1_cufft, buf1_fftw)
        ...
        subroutine test_fftw_vs_cufft_movie
            ...
            if (.not. c_associated(plan_fftw_r2c)) THROW_HARD('Fatal error: CUDA, cuFFT or FFTW call failed, code 74')
        ...
        subroutine test_cublas_sgemm
            ...
            if( ierr /= CUBLAS_STATUS_SUCCESS ) THROW_HARD('Fatal error: cublasCreate failed')
            if( .not. c_associated(handle) ) THROW_HARD('Fatal error: cublasCreate returned null handle')
            ...
            if( .not. c_associated(d_a) ) THROW_HARD('Fatal error: CUBLAS SGEMM A is not mapped')
            if( .not. c_associated(d_b) ) THROW_HARD('Fatal error: CUBLAS SGEMM B is not mapped')
            if( .not. c_associated(d_c) ) THROW_HARD('Fatal error: CUBLAS SGEMM C is not mapped')
            ...
            if( ierr /= CUBLAS_STATUS_SUCCESS ) THROW_HARD('Fatal error: cublasSgemm failed')
            ierr = cudaDeviceSynchronize()
            if( ierr /= CUDA_SUCCESS ) THROW_HARD('Fatal error: cudaDeviceSynchronize failed after cublasSgemm')
            !$omp end target data
            maxerr = maxval(abs(c - cref))
            print *, 'CUBLAS SGEMM max error = ', maxerr
            if( maxerr > 1.0e-5_c_float ) THROW_HARD('Fatal error: CUBLAS SGEMM accuracy test failed')
            ierr = cublasDestroy(handle)
            if( ierr /= CUBLAS_STATUS_SUCCESS ) THROW_HARD('Fatal error: cublasDestroy failed')
        ...
        subroutine test_kb
          ...
          diff = diff / real(n)
          if( diff > 1e-6 ) then
              THROW_HARD('Fatal error: KB interpolation test failed, results do not match')
          endif
        ...
        subroutine test_pointers
            ...
            if( passed ) then
                ...
                THROW_HARD('FAIL: pointer test failed')
            endif
...
#else
        write(logfhandle,'(A)') '>>> openmp_offload: skipped, this build has no OpenMP offload (USE_OPENMP_OFFLOAD)'
#endif
    end subroutine run_openmp_offload_tests

end module simple_openmp_offload_tester
