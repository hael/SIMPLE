...
public :: ori_dp, test_ori_dp
private

!>  orientation parameter stuct and operations
type :: ori_dp
    private
    real(dp)      :: euls(3)=0._dp      !< Euler angle
    real(dp)      :: normal(3)=0._dp    !< Fourier plane normal
    real(dp)      :: rmat(3,3)=0._dp    !< rotation matrix
    real(dp)      :: drmat(3,3,3)=0._dp !< derivative of rotation matrices (w.r.t. last index)
    type(hash_dp) :: htab               !< hash table for the parameters
...
real(dp), parameter :: zvec(3) = [0.,0.,1.]
...
contains
    ...
    subroutine new_ori( self )
        class(ori_dp), intent(inout) :: self
        call self%kill
        self%htab = hash_dp()
        call self%set_euler([0._dp,0._dp,0._dp])
        call self%htab%set('e1',0._dp)
        call self%htab%set('e2',0._dp)
        call self%htab%set('e3',0._dp)
        call self%htab%set('x',0._dp)
        call self%htab%set('y',0._dp)
        call self%htab%set('dist',180._dp)
        call self%htab%set('state',1._dp)
        call self%htab%set('frac',0._dp)
        call self%htab%set('eo',-1._dp) ! -1. is default (low-pass set); 0. for even; 1. for odd
        self%chtab = chash()
        self%existence = .true.
    ...
    subroutine ori_from_rotmat( self, rotmat )
        class(ori_dp), intent(inout) :: self
        real(dp),      intent(in)    :: rotmat(3,3) !< rotation matrix
    ...
    subroutine reject( self )
        class(ori_dp), intent(inout) :: self
        call self%set_euler([0._dp, 0._dp, 0._dp])
        call self%set_shift([0._dp, 0._dp])
        call self%htab%set('state', 0._dp)
        if( self%isthere('corr') )     call self%htab%set('corr',     -1._dp)
        if( self%isthere('specscore') )call self%htab%set('specscore', 0._dp)
        if( self%isthere('eo') )       call self%htab%set('eo', -1._dp)
    end subroutine reject

    !>  \brief  is a polymorphic assigner
    subroutine assign_ori( self_out, self_in )
        type(ori_dp), intent(in)     :: self_in
        class(ori_dp), intent(inout) :: self_out
        call self_out%copy(self_in)
    end subroutine assign_ori

    !>  \brief  is a polymorphic copier
    subroutine copy_ori( self_out, self_in )
        class(ori_dp), intent(in)    :: self_in
        class(ori_dp), intent(inout) :: self_out
    ...
    subroutine set_euler( self, euls  )
        class(ori_dp), intent(inout) :: self
        real(dp),      intent(in)    :: euls(3)!< Euler angle
    ...
    subroutine e1set( self, e1 )
        class(ori_dp), intent(inout) :: self
        real(dp),      intent(in)    :: e1     !< Euler angle
    ...
    subroutine e2set( self, e2 )
        class(ori_dp), intent(inout) :: self
        real(dp),      intent(in)    :: e2      !< Euler angle
    ...
    subroutine e3set( self, e3 )
        class(ori_dp), intent(inout) :: self
        real(dp),      intent(in)    :: e3      !< Euler angle
    ...
    subroutine set_shift( self, shvec )
        class(ori_dp), intent(inout) :: self
        real(dp),      intent(in)    :: shvec(2) !< shift vector
    ...
    subroutine rnd_romat( self )
        ...
        class(ori_dp), intent(inout) :: self
        real(dp) :: theta, phi, z, vx, vy, vz
        real(dp) :: r, st, ct, sx, sy
        ! init
        theta = ran3()*2._dp*DPI
        phi   = ran3()*2._dp*DPI
        z     = ran3()*2._dp
        ! V
        r  = sqrt( z )
    ...
    subroutine rnd_euler_2( self, o_prev, athres, proj )
        class(ori_dp),     intent(inout) :: self   !< instance
        class(ori_dp),     intent(in)    :: o_prev !< template ori
        real(dp),          intent(in)    :: athres !< angle threshold in degrees
        logical, optional, intent(in)    :: proj   !< projection only thres or not
        real(dp) :: athres_rad, dist
        logical  :: pproj
        pproj = .false.
        if( present(proj) ) pproj = proj
        athres_rad = deg2rad(athres)
        dist = 2._dp*athres_rad
    ...
    subroutine rnd_ori( self, trs, eullims )
        class(ori_dp),      intent(inout) :: self
        real(dp), optional, intent(in)    :: trs         !< threshold
        real(dp), optional, intent(inout) :: eullims(3,2)!< Euler angles
        real(dp) :: x, y, z
        if( present(trs) )then
    ...
    subroutine rnd_inpl( self, trs )
        class(ori_dp),  intent(inout)  :: self
        real(dp), intent(in), optional :: trs         !< threshold
        if( present(trs) )call self%rnd_shift(trs)
        call self%e3set(ran3()*359.99_dp)
    end subroutine rnd_inpl

    !>  \brief  for generating random in-plane parameters
    subroutine rnd_shift( self, trs )
        class(ori_dp), intent(inout)  :: self
        real(dp),      intent(in)     :: trs         !< threshold
        real(dp) :: x, y
        if( abs(trs) < 1e-3 )then
            x = 0._dp
            y = 0._dp
    ...
    subroutine round_shifts( self )
        class(ori_dp), intent(inout) :: self
        call self%set('x', real(nint(self%get('x')),kind=dp))
        call self%set('y', real(nint(self%get('y')),kind=dp))
    ...
    subroutine shift( self )
        ...
        logical :: doshift
        doshift = .false.
        if( self%euls(1) < 0._dp .or. self%euls(1) > 360._dp ) doshift = .true.
        if( self%euls(2) < 0._dp .or. self%euls(2) > 180._dp ) doshift = .true.
        if( self%euls(3) < 0._dp .or. self%euls(3) > 360._dp ) doshift = .true.
    ...
    function compeuler( self1, self2 ) result( self_out )
        class(ori_dp), intent(in) :: self1, self2
        type(ori_dp) :: self_out
    ...
    subroutine compose3d2d( ori3d, ori2d, o_out )
        class(ori_dp), intent(inout) :: ori3d, ori2d, o_out
        real(dp)                     :: ori3dx,ori3dy,x,y,e3,euls(3)
        real(dp)                     :: R3d(3,3), R2d(3,3), R(3,3)
        logical                      :: mirr
        ! inpl from 3D
        ori3dx = ori3d%get('x')
        ori3dy = ori3d%get('y')
        ...
        contains
            ...
            subroutine transfmat2inpls( R, psi, tx, ty )
                real(dp), intent(inout) :: psi,tx,ty
                real(dp), intent(in)    :: R(3,3)
                psi = rad2deg( myacos( R(1,1) ))
                if( R(1,2)<0._dp)psi=360._dp-psi
                tx  = R(1,3)
                ty  = R(2,3)
    ...
    subroutine map3dshift22d( self, sh3d )
        class(ori_dp), intent(inout) :: self
        real(dp),      intent(in)    :: sh3d(3) !< 3D shift
        real(dp)                     :: old_x,old_y,x,y,cx,cy
        real(dp)                     :: u(3),v(3),shift(3)
        real(dp)                     :: phi,cosphi,sinphi
        ...
        u = [  cosphi,sinphi,0._dp ]
        v = [ -sinphi,cosphi,0._dp ]
    ...
    subroutine mirror2d( self )
        ...
        type(ori_dp) :: old
        old = self
        self%rmat = euler2m(self%euls(1), 180._dp+self%euls(2), 180._dp-self%euls(3))
        ! the mirrored rotation matrix is constructed
        ! convert to euler
        self%euls = m2euler(self%rmat)
        call self%set('e1',self%euls(1))
    ...
    !!          129, 883--886 (2007)
    !! \param self1,self2 ori class type rotational matrices
    pure real(dp) function geodesic_dist( self1, self2 )
        class(ori_dp), intent(in) :: self1, self2
        real(dp) :: Imat(3,3), sumsq, diffmat(3,3)
        Imat      = 0._dp
        Imat(1,1) = 1._dp
        Imat(2,2) = 1._dp
        Imat(3,3) = 1._dp
    ...
    !! \param self1,self2 ori class type rotational matrices
    pure real(dp) function geodesic_dist_scaled( self1, self2 )
        class(ori_dp), intent(in) :: self1, self2
        real(dp), parameter :: old_max = 2._dp*sqrt(2._dp)
    ...
    pure function inpldist( self1, self2 ) result( dist )
        class(ori_dp), intent(in) :: self1, self2
        real(dp) :: dist
        dist = ((self1.inplrotdist.self2)+(self1.euldist.self2))/2._dp
    ...
    pure function inplrotdist( self1, self2 ) result( dist )
        class(ori_dp), intent(in) :: self1, self2
        real(dp) :: dist, mat(2,2), u(2), x1(2), x2(2)
        u(1) = 0._dp
        u(2) = 1._dp
    ...
    pure function m2euler( mat ) result ( euls )
        real(dp), intent(in), dimension(3,3) :: mat(3,3)
        real(dp), dimension(3,3):: tmp, anticomp1z, anticomp2y, get_euler3eul
        real(dp) :: imagevec(3), absxy, mceul1deg, mceul2deg, phitmp, euls(3)
        ! get_euler image of (0,0,1)
        imagevec = matmul( zvec, mat )
        ! extract eul1 from imagevec:
        absxy = sqrt(imagevec(1)**2+imagevec(2)**2)
        if(absxy < 0.0000001_dp) then
            ! normal parallel to z, phi undefined
            euls(1) = 0._dp
        else
            phitmp = myacos(imagevec(1)/absxy)
            if (imagevec(2) >= 0._dp) then
                euls(1) = phitmp
            else
                euls(1) = -phitmp
            endif
        ...
        if( get_euler3eul( 1,2 ) > 0._dp ) then
        ...
        do while( euls(3) < 0._dp )
            euls(3)=euls(3)+360._dp
        end do
        do while( euls(1) < 0._dp )
            euls(1)=euls(1)+360._dp
        end do
    ...
    subroutine test_ori_dp
    ! this test only tests the Euler part of ori,
    ! the rest is tested in the oris class
        type(ori_dp) :: e1, e2, e3
        real(dp) :: euls(3), normal(3), mat(3,3), normal2(3), dist
        logical  :: passed
        call e1%new_ori
        call e2%new_ori
        call e3%new_ori
        ...
        passed = .false.
        call e1%set_euler([1._dp,2._dp,3._dp])
        euls = e1%get_euler()
        if( abs(euls(1)-1._dp+euls(2)-2._dp+euls(3)-3._dp) < 0.001_dp )then
            passed = .true.
        endif
        if( .not. passed ) call simple_stop('Euler assignment/getters corrupt!')
        passed = .false.
        call e1%e1set(99._dp)
        call e1%e2set(98._dp)
        call e1%e3set(97._dp)
        euls = e1%get_euler()
        if( abs(euls(1)-99._dp+euls(2)-98._dp+euls(3)-97._dp) < 0.001_dp ) passed = .true.
        if( .not. passed ) call simple_stop('Euler e-setters corrupt!')
        passed = .false.
        call e1%set_euler([0._dp,0._dp,0._dp])
        normal = e1%get_normal()
        if( abs(normal(1))+abs(normal(2))+abs(normal(3)-1._dp)<3._dp*TINY ) passed = .true.
        if( .not. passed ) call simple_stop('Euler normal derivation corrupt!')
        passed = .false.
        mat = e1%get_mat()
        if( abs(mat(1,1)-1._dp)+abs(mat(2,2)-1._dp)+abs(mat(3,3)-1._dp)<3._dp*TINY )then
            mat(1,1) = 0._dp
            mat(2,2) = 0._dp
            mat(3,3) = 0._dp
        ...
        passed = .false.
        call e2%set_euler([20._dp,20._dp,20._dp])
        e3 = e2
        normal = e2%get_normal()
        normal2 = e3%get_normal()
        if( pearsn(normal,normal2) > 0.99_dp ) passed = .true.
        passed = .false.
        dist = rad2deg(e1.euldist.e2) ! IFORT CANNOT DEAL WITH THE OPERATORS HERE
        if( dist < 20.00001_dp .and. dist > 19.99999_dp ) passed = .true.
        passed = .false.
        e3 = e1%compeuler(e2)         ! IFORT CANNOT DEAL WITH THE OPERATORS HERE
        euls = e3%get_euler()
        if( euls(1) < 20.0001_dp .and. euls(1) > 19.9999_dp .and.&
            euls(2) < 20.0001_dp .and. euls(2) > 19.9999_dp .and.&
            euls(3) < 20.0001_dp .and. euls(3) > 19.9999_dp ) passed = .true.
        if( .not. passed ) call simple_stop('Euler composer corrupt!')
        write(*,'(a)') 'SIMPLE_ORI_UNIT_TEST COMPLETED SUCCESSFULLY ;-)'
    end subroutine test_ori_dp

end module simple_ori_dp
