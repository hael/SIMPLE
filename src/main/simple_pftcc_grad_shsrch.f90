...
contains
    ...
    function grad_shsrch_costfun( self, vec, D ) result( cost )
        ...
        vec_here = vec        
        where( abs(vec) < 1.e-6 ) vec_here = 0.        ! <- what is this for?
        cost = - self%pftcc_ptr%gencorr_for_rot(self%reference, self%particle, vec_here, self%cur_inpl_idx)        
    ...
    subroutine grad_shsrch_gcostfun( self, vec, grad, D )
        ...
        vec_here = vec        
        where( abs(vec) < 1.e-6 ) vec_here = 0.        ! <- what is this for?
        call self%pftcc_ptr%gencorr_grad_only_for_rot(self%reference, self%particle, vec_here, self%cur_inpl_idx, corrs_grad)
        grad = - corrs_grad        
    ...
    subroutine grad_shsrch_fdfcostfun( self, vec, f, grad, D )
        ...
        vec_here = vec        
        where( abs(vec) < 1.e-6 ) vec_here = 0.      ! <- what is this for?
        call self%pftcc_ptr%gencorr_grad_for_rot(self%reference, self%particle, vec_here, self%cur_inpl_idx, corrs, corrs_grad)
        f    = - corrs
        grad = - corrs_grad
    ...
    function grad_shsrch_minimize( self, irot ) result( cxy )
        ...
        do i = 1,self%max_evals
            if (i > 1) then
                call self%pftcc_ptr%gencorrs(self%reference, self%particle, self%ospec%x, corrs)            
                loc               = maxloc(corrs)            
                if  (loc(1) == self%cur_inpl_idx) then                
                    exit
                end if
            end if
            self%cur_inpl_idx = loc(1)
