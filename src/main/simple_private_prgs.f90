...
contains
    ...
    subroutine init_cmd_dict
        ...
        call cmd_dict%push('ctfpatch',      'whether to perform patch-based ctf estimation(yes|no){no}')
        ...
        call cmd_dict%push('dfmin',         'minimum expected defocus(in microns){0.5}')
        ...
        call cmd_dict%push('dir_refine',    'directory with oridistributions_part*.bin; e.g. 1_refine3D')
        ...
        call cmd_dict%push('dose_rate',     'dose rate(in e/A2/s)')
        ...
        call cmd_dict%push('exp_time',      'exposure time(in s)')
        ...
        call cmd_dict%push('fromf',         'start frame index')
        ...
        call cmd_dict%push('neigh',         'nearest neighbor refinement(yes|no)')
        ...
        call cmd_dict%push('pssnr',         'whether to use pssnr scheme in conjunction with match filter(yes|no){no}')
        ...
        call cmd_dict%push('sigma2_fudge',  'fudge factor for sigma2_noise {1}')
        ...
        call cmd_dict%push('time_inactive', 'end time limit(in seconds){7200}')
        call cmd_dict%push('time_per_image', 'Estimated time per image in seconds for forecasting total execution time')
        ...
        call cmd_dict%push('tof',           'stop frame index')
        call cmd_dict%push('tomo',          'tomography mode(yes|no){no}')
    ...
    subroutine new_private_prgs
        ...
        ! O_PEAKSSTATS, for obtaining o_peaks statistics
        call private_prgs(14)%set_name('o_peaksstats')
        ! required keys
        call private_prgs(14)%push_req_key('dir_refine')
        call private_prgs(14)%push_req_key('nparts')
        call private_prgs(14)%push_req_key('part')
        call private_prgs(14)%push_req_key('fromp')
