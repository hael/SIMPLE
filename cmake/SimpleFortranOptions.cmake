#------------------------------------------------------------------------------!
# SIMPLE              Elmlund Lab               https://github.com/hael/SIMPLE !
#------------------------------------------------------------------------------!
...
message(STATUS "Making sure your Fortran compiler points to the correct binary")
  ...
  if(ACTUAL_FC_TARGET MATCHES "Clang|clang")
    ...
    find_file(
      ...
      NAMES gfortran- gfortran-15 gfortran-14 gfortran-13 gfortran-12 gfortran-11 gfotran-10 gfortran-9 gfortran-8 gfortran-7 gfortran-6 gfortran-5 gfortran-4.9 gfortran13 gfortran12 gfortran11 gfortran10 gfortran9 gfortran8 gfortran7 gfortran6 gfortran5
...
message(STATUS "GCC version: ${ACTUAL_C_TARGET}")
  ...
  if(ACTUAL_C_TARGET MATCHES "Clang|clang")
    ...
    find_file (
      ...
      NAMES gcc- gcc-15 gcc-14 gcc-13 gcc-12 gcc-11 gcc-10 gcc-9 gcc-8 gcc-7 gcc-6 gcc-5 gcc-4.9 gcc-fsf-6 gcc-fsf-5 gcc13 gcc12 gcc11 gcc10 gcc9 gcc8 gcc7 gcc6 gcc5 gcc4.9
  ...
  if(EXISTS "${CMAKE_C_COMPILER_NEW}")
    ...
    message( FATAL_ERROR  "Cannot find GNU ${CMAKE_C_COMPILER_NEW} -- ${CLANG_FATAL_MSG}")
  endif()
endif()
...
NO_DEFAULT_PATH NO_CMAKE_PATH NO_CMAKE_ENVIRONMENT_PATH NO_SYSTEM_ENVIRONMENT_PATH NO_CMAKE_SYSTEM_PATH
    ...
    message( FATAL_ERROR  "Cannot find GNU ${CMAKE_CXX_COMPILER} -- ${CLANG_FATAL_MSG}")
  endif()
endif()
...
<<<<<<< HEAD
      CPP_COMPILER_NEW
      NAMES  cpp-15 cpp-14 cpp-13 cpp-12 cpp-11 cpp-10 cpp-9 cpp-8 cpp-7 cpp-6 cpp-5 cpp-4.9 cpp10 cpp9 cpp8 cpp7 cpp6 cpp5 cpp4.9
      PATHS ${FORTRAN_PARENT_DIR} /sw/bin /usr/local/bin /opt/local/bin /opt/homebrew/bin /usr/bin
=======
      ...
      #  [PATH_SUFFIXES suffix1 [suffix2 ...]]
>>>>>>> 8fce6ca4 (new branch to revamp cmake files)
...
if(USE_LIBTIFF)
  ...
  if(TIFF_FOUND)
    ...
    # LZMA
    find_library(LZMA_LIBRARY lzma HINTS /opt/homebrew/lib)
    ...
    # ZLIB
    find_library(Z_LIBRARY z HINTS /opt/homebrew/opt/zlib/lib)
    ...
    # Extra dependencies for tifflib4.5 and above
    if( APPLE )
      # no issue with Apple-ARM CPUs
    else()
      find_package(TIFF 4.5.1 QUIET)
      if( TIFF_FOUND )
          find_library(Lerc_LIBRARY Lerc)
          message("-- Lerc_LIBRARY: ${Lerc_LIBRARY}")
          if( Lerc_LIBRARY STREQUAL "Lerc_LIBRARY-NOTFOUND" )
            set(TIFFLIBS "0")
          else()
            set(EXTRA_LIBS ${EXTRA_LIBS} -lLerc)
          endif()
          find_library(SHARPYUV_LIBRARY sharpyuv)
          message("-- SHARPYUV_LIBRARY: ${SHARPYUV_LIBRARY}")
          if( SHARPYUV_LIBRARY STREQUAL "SHARPYUV-NOTFOUND" )
            set(TIFFLIBS "0")
          else()
            set(EXTRA_LIBS ${EXTRA_LIBS} -lsharpyuv)
          endif()
      endif()
    endif()
  endif()
