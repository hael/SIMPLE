...
contains
    ...
    subroutine pick_valid( AFM_in, outname )
        ...
        call avg_p%pick(trim(outname)//'avg.mrc')
        call HeightTrace%write(trim(outname)//'trace.mrc')
        call trace_p%pick(trim(outname)//'trace.mrc')
        call HeightRetrace%write(trim(outname)//'retrace.mrc')
        call retrace_p%pick(trim(outname)//'retrace.mrc')
    ...
    subroutine hough_lines( img_in, theta_range, mask )
        ...
        logical, intent(inout)        :: mask(1024, 1024, 1)
