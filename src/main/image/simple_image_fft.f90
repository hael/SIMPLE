...
contains
    ...
    module subroutine norm_noise_taper_edge_mask_pad_fft(self, lmsk, mskrad, self_out)
        ...
        ! --- taper vars (square, 2D, n3=1) ---
        integer       :: n, wtap, wavg, wbg, j1, j2, border_count
        real(dp)      :: border_sum_dp, edge_mean_dp
        real(c_float) :: alpha_sp, inv_wtap_sp, inv_n_sp, bg_fill
        ! mask
        real(c_float) :: val, e, r2, cjs2
        integer       :: minlen
        ! Edge profiles remain single precision (cheap+stable)
        real(c_float) :: x_start(self%ldim(2)), x_stop(self%ldim(2))
        real(c_float) :: x_smooth_start(self%ldim(2)), x_smooth_stop(self%ldim(2))
        ...
        invstd_sp = real(invstd_dp, c_float)
        ! ============================================================
        ! 3) MASK + apply normalization while copying
        !    Pad background is edge_mean transformed when normalization is used
        ! ============================================================
        minlen = minval(self%ldim(1:2))
        minlen = min(nint(2.0*(mskrad + COSMSKHALFWIDTH)), minlen)
        if (do_norm) then
            bg_fill = real((edge_mean_dp - mean_dp) * invstd_dp, c_float)
        else
            bg_fill = real(edge_mean_dp, c_float)
        end if
        if (do_norm) then
            do j = 1, n2
                cjs2 = mem_msk_cs2(j)
                do i = 1, n1
                    ...
                    if (e < 0.0001) then
                        self%rmat(i,j,1) = bg_fill
                    else if (e < 0.9999) then
                        val = (self%rmat(i,j,1) - mean_sp) * invstd_sp
                        self%rmat(i,j,1) = e*val + (1.0-e)*bg_fill
                    else
                        self%rmat(i,j,1) = (self%rmat(i,j,1) - mean_sp) * invstd_sp
                    endif
                end do
            end do
        else
            do j = 1, n2
                ...
                do i = 1, n1
                    ...
                    if (e < 0.0001) then
                        ...
                        self%rmat(i,j,1) = e*self%rmat(i,j,1) + (1.0-e)*bg_fill
                    endif
                end do
            end do
        end if
        ! ============================================================
        ! 4) PAD + FFTSHIFT (fused)
        ! ============================================================
        starts = (self_out%ldim - self%ldim) / 2 + 1
        self_out%ft   = .false.
        self_out%rmat = bg_fill
        do j = 1, n2
            y0 = starts(2) + j - 1
            yo = modulo((y0 - 1) + h2o, n2o) + 1
            do i = 1, n1
