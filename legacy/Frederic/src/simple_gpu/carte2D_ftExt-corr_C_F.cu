...
rCFsum_1D( const cuFloatComplex *A, const cuFloatComplex *B, float *partial, int N) {
...
extern "C" __global__ void
absqsumCF_1D( const cuFloatComplex *A, float *partial, int N) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  int sharedIndex = threadIdx.x;
  //allocating the shared memory array
...
extern "C" __global__ void
CFprod_3D_mat(cuFloatComplex *C,
...
/* main kernel entry */
extern "C" int
carte2d_ftExt_corr_C_F(deviceDetails_t * s_devD,
                       ...
                       bench_t *s_bench, debug_gpu_t *s_debug_gpu)
{
  ...
  cuFloatComplex *d_cmat2sh = NULL;
  cuFloatComplex *d_shmat = NULL;
  ...
  err = cudaMalloc((void**)&d_r, size_r);
  if ( (int)err != CUDA_SUCCESS ) {rc = err;}

  size_m = npts*sizeof(cuFloatComplex);
  err = cudaMalloc((void**)&d_A, size_m);
  err = cudaMalloc((void**)&d_B, size_m);

  err = cudaMalloc((void**)&d_shmat, size_m);
  err = cudaMalloc((void**)&d_cmat2sh, size_m);
  ...
  err = cudaMemcpy(d_shmat, shmat, npts*sizeof(cuFloatComplex), cudaMemcpyHostToDevice);

  //First getting the product cmat2sh

  CFprod_3D_mat<<<grid,threads>>>(d_cmat2sh, d_B, d_shmat, npart,nrot,nk);
  cuCtxSynchronize();
  ...
  rCFsum_1D<<<blocksPerGrid,threadsPerBlock>>>(d_A, d_cmat2sh, d_partial_c,N);
  cuCtxSynchronize();
  ...
  absqsumCF_1D<<<blocksPerGrid,threadsPerBlock>>>(d_A,d_partial_ca,N);
  ...
  absqsumCF_1D<<<blocksPerGrid,threadsPerBlock>>>(d_cmat2sh,d_partial_cb,N);
