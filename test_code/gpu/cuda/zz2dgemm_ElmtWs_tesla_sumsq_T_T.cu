...
// C = alpha*A*B + beta*C
extern "C" __global__ void
main_zz2dgemm_kernel_sumsq_T_T( double *C, 
				...
				double alpha, double beta)
{
  ...
  C[i * ldc + j ] = cuDmul(alpha,cuReCCstar( A[ j * lda + i] ));
...
extern "C" int
zz2dgemm_kernel_sumsq_T_T( double *C, 
			   const cuDoubleComplex *A, 
			   int m, int n, int k,
			   int lda, int ldb, int ldc, 
			   double alpha, double beta)
{
  ...
  main_zz2dgemm_kernel_sumsq_T_T<<< grid, threads >>>(C, A,
						      m, n, k,
						      lda, ldb, ldc,
						      alpha, beta);
  

  //sumddaxpy<<<1,1>>>(A, B, lda, ldb, sumasq, sumbsq);
