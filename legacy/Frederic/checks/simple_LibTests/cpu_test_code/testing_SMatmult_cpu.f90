...
program testing_SMatmult_cpu
  ...
  implicit none
  logical :: ibench_local=.true.        !< benchmark result or not
  logical :: ibench_write_local=.true.  !< write benchmark result or not
...
#define devptr_t integer*8
  ...
  integer, parameter            :: npart= 1000    !4000  !30000
  integer, parameter            :: start_npart=1000, istep=64
  integer, parameter            :: nrot = 400 !4000 !15000
...
!*******************************************************************************
     ...
     write(*,*)'                                                               '
     write(*,*)'***************CPU Matrix Mul with sgemm***********************'
     write(*,*)'                                                               '

     call start_timer_cpu("mtml_gpu")
     call gettimeofday_c(st_ZM_r)

     call sgemm("N", "N", ipart, nrot, nk, &
                alpha,                       &
                pft1, lda,                   &
                pft2, ldb,                   &
                beta,                        &
     ...
     write(*,*)"Elapsed time Sgemm        : ",real(elps_mtml_gpu),"(seconds)"
     write(*,*)'                                                               '
     write(*,*)'*********Comparing product from matmul vs Sgemm (CPU)**********'
     write(*,*)'                                                               '
     ...
     write(*,'(3x,a,2x,a,6x,a)') "Matmul CPU (secs)","Sgemm CPU (secs)","Gflp/s"
     write(*,'(x,f15.5,3x,f15.5,7x,f10.5)')elps_mtml_cpu,elps_mtml_gpu,gflops
     write(*,*)
     write(*,'(x,a,x,f10.3)')" Speed up from matmul vs sgemmn: ",speedup
     write(*,*)'***************************************************************'
...
end program testing_SMatmult_cpu
!*******************************************************************************
