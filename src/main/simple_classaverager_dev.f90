...
contains
    ...
    subroutine cavger_new( b, p, which )
        ...
        do istate=1,p%nstates
            do icls=1,ncls
                call cavgs_even(istate,icls)%new([p%box,p%box,1],p%smpd)
                call cavgs_odd(istate,icls)%new([p%box,p%box,1],p%smpd)
                call cavgs_merged(istate,icls)%new([p%box,p%box,1],p%smpd)
                call ctfsqsums_even(istate,icls)%new([p%box,p%box,1],p%smpd)
                call ctfsqsums_odd(istate,icls)%new([p%box,p%box,1],p%smpd)
                call ctfsqsums_merged(istate,icls)%new([p%box,p%box,1],p%smpd)
            end do
        end do
    ...
    subroutine cavger_assemble_sums
        ...
        complex   :: comp, zero
        real      :: loc(2), mat(2,2), pw
        integer   :: icls, iptcl, icls_pop, iprec, iori, cnt_progress, sh
        integer   :: i, nbatches, batch, batchsz, cnt, lims(3,2), istate
        integer   :: logi(3), phys(3), win(2,2)
        integer   :: cyc_lims(3,2), alloc_stat, wdim, incr, h, k, l, m, nyq
        integer   :: batchsz_max
        ...
        deallocate(batches)
        ! pre-allocations
        allocate(batch_imgs(batchsz_max), padded_imgs(batchsz_max))
        ...
        cmat_even = padded_imgs(1)%get_cmat()
        cmat_odd  = padded_imgs(1)%get_cmat()
        nyq       = padded_imgs(1)%get_lfny(1)
    !     lims      = cyc_lims
    !     lims(1,1) = 0
        allocate(cmat_even(cyc_lims(1,1):cyc_lims(1,2),cyc_lims(2,1):cyc_lims(2,2),1),&
                &cmat_odd(cyc_lims(1,1):cyc_lims(1,2),cyc_lims(2,1):cyc_lims(2,2),1), stat=alloc_stat)
        ! state loop 
        cnt_progress = 0
        do istate=1,nstates
            ! class loop
            do icls=1,ncls
                ...
                do batch=1,nbatches
                    ...
                    do i=1,batchsz
                        ...
                        ! create padded imgs
                        call padded_imgs(i)%new(ldim_pd, pp%smpd)
                        call prep4cgrid(batch_imgs(i), padded_imgs(i), kbwin)
                    enddo
                    call batch_imgsum_even%new(ldim_pd, pp%smpd)
                    call batch_imgsum_odd%new(ldim_pd, pp%smpd)
                    call batch_imgsum_even%zero_and_flag_ft
                    call batch_imgsum_odd%zero_and_flag_ft
                    cmat_even         = zero
                    cmat_odd          = zero
                    !$omp parallel do default(shared) private(i,iprec,sh,iori,h,k,l,m,loc,mat,logi,phys,cyc1,cyc2,w,comps,win,incr,pw)&
    ...
    subroutine apply_ctf_and_shift( iprec, iori, img )
        use simple_ctf,   only: ctf
        ...
        integer     :: state, class
        call ctfsq%new(ldim, pp%smpd)
        call ctfsq%set_ft(.true.)
        call img%fwd_ft
        ! prep indices
        state = precs(iprec)%states(iori)
        class = precs(iprec)%classes(iori)
        ! take care of the nominator
        x      = -precs(iprec)%shifts(iori,1)
        y      = -precs(iprec)%shifts(iori,2)
        dfx    =  precs(iprec)%dfx
        dfy    =  precs(iprec)%dfy
        angast =  precs(iprec)%angast
        select case(pp%tfplan%flag)
            case('yes')  ! multiply with CTF
                call precs(iprec)%tfun%apply_and_shift(img, ctfsq, x, y, dfx, 'ctf', dfy, angast)
            case('flip') ! multiply with abs(CTF)
                call precs(iprec)%tfun%apply_and_shift(img, ctfsq, x, y, dfx, 'abs', dfy, angast)
            case('mul','no')
                call precs(iprec)%tfun%apply_and_shift(img, ctfsq, x, y, dfx, '', dfy, angast)
        end select
        ! prep weight
        if( l_hard_assign )then
            pw = precs(iprec)%pw
        ...
        select case(precs(iprec)%eo)
            case(0,-1)
                call ctfsqsums_even(state, class)%add(ctfsq, pw)
            case(1)
                call ctfsqsums_odd(state, class)%add(ctfsq, pw)
        end select
