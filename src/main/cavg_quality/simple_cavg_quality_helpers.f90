...
use simple_cavg_quality_types, only: CAVG_REJECT_REASON_NONE, CAVG_REJECT_REASON_POP, &
                                     ...
                                     CAVG_REJECT_REASON_BP_CENTER_EDGE_LOW, CAVG_REJECT_REASON_LOCVAR_FG_LOW, CAVG_REJECT_REASON_FUZZY_BALL_SIGNAL_NEG
implicit none
private
    ...
    function cavg_rejection_reason_string( reason_code ) result( reason_text )
        ...
        select case( reason_code )
            ...
            case( CAVG_REJECT_REASON_FUZZY_BALL_SIGNAL_NEG )
                reason_text = 'suspected overfitting: negative fuzzy-ball signal'
            case default
