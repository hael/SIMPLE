...
contains
    ...
    subroutine master_complex_ppca( self, cvecs )
        ...
        complex(dp) :: cov(self%D,self%D), xc(self%D)
        integer :: i
        if( .not. self%existence )then
            THROW_HARD('complex_ppca not allocated; master')
        endif
        self%mu = sum(cvecs, dim=2) / real(max(1,self%N), dp)
        cov = cmplx(0._dp, 0._dp, kind=dp)
        do i = 1,self%N
            xc = cvecs(:,i) - self%mu
            cov = cov + outer_hermitian(xc)
        enddo
        cov = cov / real(max(1,self%N), dp)
        call self%fit_covariance(self%mu, cov)
        do i = 1,self%N
    ...
    subroutine fit_covariance_complex_ppca( self, mu, cov, sigma2_in )
        ...
        integer :: j, ntail
        if( .not. self%existence )then
            THROW_HARD('complex_ppca not allocated; fit_covariance')
        endif
        self%mu = mu
    ...
    subroutine denoise_complex_ppca( self, x, xhat )
        ...
        if( .not. self%existence )then
            THROW_HARD('complex_ppca not allocated; denoise')
        endif
        centered = x - self%mu
    ...
    subroutine reconstruct_external_complex_ppca( self, centered_in, centered_out, feat )
        ...
        integer :: j
        if( .not. self%existence )then
            THROW_HARD('complex_ppca not allocated; reconstruct_external')
        endif
        do j = 1,self%Q
    ...
    function get_feat_complex_ppca( self, i ) result( feat )
        ...
        if( .not. self%existence )then
            THROW_HARD('complex_ppca not allocated; get_feat')
        endif
    ...
    subroutine generate_complex_ppca( self, i, avg, dat )
        ...
        if( .not. self%existence )then
            THROW_HARD('complex_ppca not allocated; generate')
        endif
        dat = avg + self%data(:,i)
