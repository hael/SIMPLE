module pose_cont_refinement_shift_test
...
subroutine run_shift_gradient()
    ...
    write(*,'(a,3(es14.6,1x))') 'POSE_CONT_REFINEMENT_SHIFT min-Jv/adjoint/min-objective error: ', &
        &minval(directional_errors), adjoint_error, minval(objective_errors)
    write(*,'(a,3(es14.6,1x))') 'POSE_CONT_REFINEMENT_SHIFT Jv centred-FD curve: ', directional_errors
    write(*,'(a,3(es14.6,1x))') 'POSE_CONT_REFINEMENT_SHIFT objective centred-FD curve: ', objective_errors
    write(*,'(a,3(es14.6,1x),i0)') 'POSE_CONT_REFINEMENT_SHIFT sign/initial/final/accepted: ', &
        &sign_error, initial_error, recovery_error, recovery_naccepted
    write(*,'(a,es14.6,1x,i0)') 'POSE_CONT_REFINEMENT_SHIFT max-step/attempted: ', &
        &recovery_max_trial_step, recovery_nattempted
    write(*,'(a,3(es14.6,1x))') 'POSE_CONT_REFINEMENT_SHIFT weighted-gradient/recovery/whitening error: ', &
        &weighted_fd_error, weighted_recovery_error, whitening_error
    write(*,'(a)') 'POSE_CONT_REFINEMENT_SHIFT_GRADIENT: PASS'
...
end module pose_cont_refinement_shift_test
