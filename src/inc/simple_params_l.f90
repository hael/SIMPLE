...
private
    ...
    type :: CTFMODETYPE
        private
        integer(kind(CTFMODE_ABS)) :: m_ctfmode=CTFMODE_ABS
    end type CTMODETYPE
    !! OPT type
    enum, bind(c)
...
type :: params
    ...
    type(YESORNO)      :: acf=NO             !< calculate autocorrelation function(yes|no){no}
    type(YESORNO)      :: append=NO          !< append in context of files(yes|no){no}
    type(YESORNO)      :: async=NO           !< asynchronous (yes|no){no}
    type(YESORNO)      :: autoscale=NO       !< automatic down-scaling(yes|no){yes}
    type(YESORNO)      :: avg=NO             !< calc average automatic (yes|no){no}
    type(YESORNO)      :: bin=NO             !< binarise image(yes|no){no}
    type(YESORNO)      :: center=YES         !< center image(s)/class average(s)/volume(s)(yes|no){no}
    type(YESORNO)      :: classtats=NO       !< calculate class population statistics(yes|no){no}
!    type(YESORNO)      :: clustvalid=NO      !< validate clustering(yes|homo|no){no}
    type(YESORNO)      :: compare=NO         !< do comparison(yes|no){no}
    type(YESORNO)      :: countvox=NO        !< count # voxels(yes|no){no}
    type(YESORNO)      :: ctfstats=NO        !< calculate ctf statistics(yes|no){no}
    type(YESORNO)      :: cure=NO
    type(YESORNO)      :: discrete=NO        !< be discrete(yes|no){no}
    type(YESORNO)      :: diverse=NO         !< diverse or not flag (yes|no){no}
    type(YESORNO)      :: doalign=YES
    type(YESORNO)      :: dopca=YES
    type(YESORNO)      :: dopick=YES
    type(YESORNO)      :: doprint=NO
    type(YESORNO)      :: dynlp=YES          !< automatic resolution limit update(yes|no){yes}
    type(YESORNO)      :: eo=YES             !< use FSC for filtering and low-pass limit update(yes|no){no}
    type(YESORNO)      :: errify=NO          !< introduce error(yes|no){no}
    type(YESORNO)      :: even=NO            !< calculate even eo-pair(yes|no){no}
    type(YESORNO)      :: ft2img=NO          !< convert Fourier transform to real image of power(yes|no){no}
    type(YESORNO)      :: guinier=NO         !< calculate Guinier plot(yes|no){no}
    type(YESORNO)      :: kmeans=YES
    type(YESORNO)      :: local=NO
    type(YESORNO)      :: masscen=NO         !< center using binarisation and mass centering(yes|no){no}
    type(YESORNO)      :: merge=NO
    type(YESORNO)      :: mirr=NO            !< mirror(no|x|y){no}
    type(YESORNO)      :: neg=NO             !< invert contrast of images(yes|no)
    type(YESORNO)      :: noise_norm =NO
    type(YESORNO)      :: noise=NO           !< noise initialisation(yes|no){no}
    type(YESORNO)      :: norec=NO           !< do not reconstruct volume(s)(yes|no){no}
    type(YESORNO)      :: norm=NO            !< do statistical normalisation avg
    type(YESORNO)      :: odd=NO             !< calculate odd eo-pair(yes|no){no}
    type(YESORNO)      :: order=NO           !< order ptcls according to correlation(yes|no){no}
    type(YESORNO)      :: outside=NO         !< extract boxes outside the micrograph boundaries(yes|no){no}
    type(YESORNO)      :: pad=NO
    type(YESORNO)      :: pgrp_known=NO      !< point-group known a priori(yes|no){no}
    type(YESORNO)      :: phaseplate=NO      !< images obtained with phaseplate(yes|no){no}
    type(YESORNO)      :: phrand=NO          !< phase randomize(yes|no){no}
    type(YESORNO)      :: plot=NO            !< make plot(yes|no){no}
    type(YESORNO)      :: pproc=YES          !< whether to perform volume post-processing(yes|no){yes}
    type(YESORNO)      :: projstats=NO
    type(YESORNO)      :: readwrite=NO
    type(YESORNO)      :: remap_classes=NO
    type(YESORNO)      :: restart=NO
    type(YESORNO)      :: rnd=NO             !< random(yes|no){no}
    type(YESORNO)      :: rm_outliers=YES    !< remove outliers{yes}
    type(YESORNO)      :: roalgn=NO
    type(YESORNO)      :: round=NO
    type(YESORNO)      :: shalgn=NO          !< do 2D shift alignment(yes|no){no}
    type(YESORNO)      :: shellnorm=NO
    type(YESORNO)      :: shbarrier=YES      !< use shift search barrier constraint(yes|no){yes}
    type(YESORNO)      :: single=NO          !< simulate a single image(yes|no){no}
    type(YESORNO)      :: soften=NO          !< soften envelope with cosine edge(yes|no){no}
    type(YESORNO)      :: stats=NO           !< provide statistics(yes|no){yes}
    type(YESORNO)      :: stream=NO          !< sream (real time) execution mode(yes|no){no}
    type(YESORNO)      :: swap=NO
    type(YESORNO)      :: test=NO
    type(YESORNO)      :: tomo=NO            !< tomography mode(yes|no){no}
    type(YESORNO)      :: time=NO
    type(YESORNO)      :: trsstats=NO        !< provide origin shift statistics(yes|no){no}
    type(YESORNO)      :: tseries=NO         !< images represent a time-series(yes|no){no}
!    type(YESORNO)      :: verbose=NO        !< verbosity flag (yes|no){no}
    type(YESORNO)      :: vis=NO             !< visualise(yes|no)
    type(YESORNO)      :: weights2D=NO
    type(YESORNO)      :: zero=NO            !< zeroing(yes|no){no}


    !! Character variables in ascending alphabetical order
    ...
    !! Enumerated types
    type(ANGLEUNIT)    :: angastunit=DEGREES !< angle of astigmatism unit (radians|degrees){degrees}
    type(AUTOMASKTYPE) :: automsk = NOAUTOMASK
    type(boxtype) :: boxtype = EMANBOX
    type(ctfflagtype) :: ctfflag = CTFFLAG_NO
...
!    character(len=STDLEN) :: endian='native'      !< endiannesss of files(big|little|native){native}
    ...
    type(opttype) :: opt=SIMPLEX        !< optimiser (powell|simplex|oasis|bforce|pso|de){simplex}
    type(speckindtype) :: speckind = sqrtPS
    type(splitmode) :: split_mode=EVEN
    type(refinetype) :: refine=NOREFINE
...
!>>> END, IMAGE-PROCESSING-RELATED
        ...
        contains
            ...
            subroutine check_carg( carg, var )
                ...
                type(YESORNO), intent(out) :: var
                if( cline%defined(carg) )then
                    select case(cline%get_carg(carg))
                    case ('no')
                        var=NO
                    case('yes')
                        var=YES
