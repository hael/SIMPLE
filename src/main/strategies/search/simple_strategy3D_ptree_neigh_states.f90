...
use simple_strategy3D_tree_utils, only: peak_tree_selection, init_peak_tree_selection,&
&select_peak_trees, descend_tree_prob, MAX_NTREES
...
public :: strategy3D_ptree_neigh_states
private
#include "simple_local_flags.inc"

type, extends(strategy3D) :: strategy3D_ptree_neigh_states
contains
    procedure :: new         => new_ptree_neigh_states
    procedure :: srch        => srch_ptree_neigh_states
    procedure :: kill        => kill_ptree_neigh_states
    procedure :: oris_assign => oris_assign_ptree_neigh_states
end type strategy3D_ptree_neigh_states

contains

    subroutine new_ptree_neigh_states( self, params, spec, build )
        use simple_builder, only: builder
        class(strategy3D_ptree_neigh_states), intent(inout) :: self
        class(parameters),                    intent(in)    :: params
        class(strategy3D_spec),               intent(inout) :: spec
        class(builder),                       intent(in)    :: build
        call self%s%new(params, spec, build)
        self%spec = spec
    end subroutine new_ptree_neigh_states

    subroutine srch_ptree_neigh_states( self, os, ithr )
        class(strategy3D_ptree_neigh_states), intent(inout) :: self
        ...
        if( .not. allocated(self%s%b_ptr%subspace_full2sub_map) )then
            THROW_HARD('ptree_neigh_states search requires subspace_full2sub_map. Check builder construction.')
        endif
        if( .not. allocated(self%s%b_ptr%subspace_inds) )then
            THROW_HARD('ptree_neigh_states search requires subspace_inds. Check builder construction.')
        endif
        if( ntrees <= 0 )then
            THROW_HARD('ptree_neigh_states search requires at least one block tree.')
        endif
        if( ntrees > MAX_NTREES )then
            THROW_HARD('Number of trees exceeds MAX_NTREES; srch_ptree_neigh_states')
        endif
        ! ------------------------------------------------------------------
        ...
        do isub = 1, self%s%p_ptr%nspace_sub
            ...
            if( iproj_full < 1 .or. iproj_full > self%s%p_ptr%nspace )then
                THROW_HARD('subspace index out of bound; ptree_neigh_states search')
            endif
            ...
            if( itree < 1 .or. itree > ntrees )then
                THROW_HARD('tree index out of bound; ptree_neigh_states search')
            endif
        ...
        if( peak_sel%npeak_trees <= 0 )then
            THROW_HARD('No geometry-selected tree found; srch_ptree_neigh_states')
        endif
        itree_selected = peak_sel%peak_trees(1)

        ! ------------------------------------------------------------------
        ! Single-tree probabilistic descent with multi-state evaluation.
    ...
    end subroutine srch_ptree_neigh_states

    subroutine oris_assign_ptree_neigh_states( self )
        class(strategy3D_ptree_neigh_states), intent(inout) :: self
        call extract_peak_ori(self%s)
    end subroutine oris_assign_ptree_neigh_states

    subroutine kill_ptree_neigh_states( self )
        class(strategy3D_ptree_neigh_states), intent(inout) :: self
        call self%s%kill
    end subroutine kill_ptree_neigh_states

end module simple_strategy3D_ptree_neigh_states
