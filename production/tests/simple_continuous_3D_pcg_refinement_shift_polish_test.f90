module continuous_3D_pcg_refinement_shift_polish_test
...
use simple_pcg_pose_polisher, only: pcg_pose_polish_summary, polish_fixed_volume_shifts
...
subroutine run_shift_polish()
    ...
    type(pcg_pose_polish_summary) :: even_summary, odd_summary, weak_summary
    ...
    call polish_fixed_volume_shifts(even_workspace,rotmats(:,:,even_ids),observed(:,:,even_ids),&
    ...
    call polish_fixed_volume_shifts(odd_workspace,rotmats(:,:,odd_ids),observed(:,:,odd_ids),&
    ...
    call weak_operator%begin_fourier_workspace(weak_workspace)
    weak_rotation = 0._dp
    ...
    call polish_fixed_volume_shifts(weak_workspace,weak_rotation,observed(:,:,1:1)*0.,weak_shift,&
    ...
    write(*,'(a,4(i0,1x))') 'CONTINUOUS_3D_PCG_SHIFT_POLISH particles/improved/unchanged/steps: ', &
    ...
    write(*,'(a,2(es14.6,1x))') 'CONTINUOUS_3D_PCG_SHIFT_POLISH objective before/after: ', &
        &even_summary%objective_before+odd_summary%objective_before, &
        &even_summary%objective_after+odd_summary%objective_after
    write(*,'(a,es14.6)') 'CONTINUOUS_3D_PCG_SHIFT_POLISH maximum trial step: ', &
        &max(even_summary%max_trial_step,odd_summary%max_trial_step)
    write(*,'(a)') 'CONTINUOUS_3D_PCG_SHIFT_POLISH: PASS'
...
end module continuous_3D_pcg_refinement_shift_polish_test
