...
contains
    ...
    subroutine build_strategy3D_tbox( self, params )
        ...
        if( str_has_substr(params%refine, 'clsneigh') )then
            ncls = self%spproj%os_cls3D%get_noris()
            allocate(self%nnmat(ncls,params%nnn))
            call self%spproj%os_cls3D%nearest_proj_neighbors(params%nnn, self%nnmat)
        else if( str_has_substr(params%refine, 'neigh') )then
