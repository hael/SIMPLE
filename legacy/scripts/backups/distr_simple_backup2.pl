...
$name_value{'simemimgs'}      = $SIMPLEBIN.'/simple_simemimgs';
$name_value{'prime2'}         = $SIMPLEBIN.'/simple_prime2';
$name_value{'prime2_cluster'} = $SIMPLEBIN.'/simple_prime2_cluster';
$name_value{'comlin_smat'}    = $SIMPLEBIN.'/simple_comlin_smat';
...
if( $name_value{'prg'} =~ /prime2_cluster/ ){
    $exec_mode = 3;
}
if( $name_value{'prg'} =~ /comlin_smat/ ){
    $exec_mode = -1;
}
...
if( !defined($name_value{'filetab'}) ){
    if( !defined($name_value{'stk'}) ){
        if( $name_value{'prg'} ne $name_value{'simemimgs'} ){
...
if( !defined($name_value{'filetab'}) ){
    ...
    if( $name_value{'prg'} eq $name_value{'simemimgs'} ){
...
if( $exec_mode > 0 ){
    while( $round < $name_value{'maxits'} ){
        ...
        mkdir $folder;
        # parallell execution
        if( $exec_mode == 1 and $name_value{'diversify_defined'} eq 'no' and $name_value{'refine'} ne 'shc' ){
            if( defined($name_value{'diversify'}) ){
                delete $name_value{'diversify'};
...
sub exec_para{
    ...
    $cmd_string = add2string('npart', $cmd_string);
    $cmd_string =~ s/\s+$//; # removes trailing whitespace
    print $cmd_string, "\n";
    my $ptcls_per_part = int(int($name_value{'nptcls'})/int($name_value{'npart'}));
    my$leftover = int($name_value{'nptcls'})-$ptcls_per_part*int($name_value{'npart'});
    my $stop  = 0;
    my $start = 0;
