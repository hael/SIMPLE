class CreatePickingReferencesWidget {
	...
	generate(){
		...
		return postAjaxPromise(request)
			.then((response) => response.json())
			.then ((json) => {
				(<HTMLInputElement>document.getElementById('keyrefs')).value = json.pickrefs
				this.hide()
			})
	}
...
var pickrefswidget
window.addEventListener('load', () => {pickrefswidget = new CreatePickingReferencesWidget()})
