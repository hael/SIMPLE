...
contains
    ...
    subroutine lmat2file( mat, fname )
     ...
     !> \brief  for converting an integer matrix to file
    subroutine imat2file( mat, fname )
        integer,       intent(in) :: mat(:,:)    !< Input matrix
        class(string), intent(in) :: fname       !< Output filename
        integer :: funit, io_stat

        call fopen(funit, fname, access='STREAM', action='WRITE', status='REPLACE', iostat=io_stat)
        call fileiochk("imat2file fopen failed: "//fname%to_char(), io_stat)
        write(unit=funit, pos=1) shape(mat)
        write(unit=funit, pos=(2*sizeof(funit)+1)) mat
        call fclose(funit)
    end subroutine imat2file
    ...
    !> \brief  for converting a file generated by imat2file back to an integer matrix
    subroutine file2imat( fname, imat )
        class(string),         intent(in)    :: fname       !< input filename
        integer, allocatable,  intent(inout) :: imat(:,:)   !< output matrix
        integer :: dims(2), funit, io_stat

        if( .not. file_exists(fname) ) THROW_HARD(fname%to_char()//' does not exist')
        call fopen(funit, fname, access='STREAM', action='READ', status='OLD', iostat=io_stat)
        call fileiochk('file2imat; fopen failed: '//fname%to_char(), io_stat)
        read(unit=funit, pos=1) dims
        if( allocated(imat) ) deallocate(imat)
        allocate(imat(dims(1), dims(2)))
        read(unit=funit, pos=(sizeof(dims)+1)) imat
        call fclose(funit)
    end subroutine file2imat
