...
use simple_mini_stream_utils,     only: segdiampick_mics_multi, segdiampick_mics_multi_fixed_bins
...
contains
    ...
    subroutine exec_stream_p03_initial_analysis( self, cline )
        ...
        type(rec_list)                             :: extracted_project_list ! growing list of successfully extracted per-project project files
        type(project_rec)                          :: prec
        type(project_rec)                          :: extracted_prec
        type(string)                               :: sentinel_file
        integer                                    :: nprojects, i
        ...
        allocate(cycle_plan_status(size(NMICS_PLAN)), source=0)
        ...
        do
            ...
            if( nprojects > 0 ) then
                ...
                write(logfhandle,'(A,I6)') '>>> # MICROGRAPHS IMPORTED : ', n_mics_imported
                last_import_time = simple_gettime()
                write(logfhandle,'(A,A)') '>>> LAST IMPORT AT         : ', cast_time_char(last_import_time)
            end if
            ! pick extract
            if( allocated(accepted_bins) .and. project_list%size() > 0 ) then
                ! bins already known (set by an earlier segdiampick_mics_multi call): re-pick &
                ! extract every not-yet-picked project in project_list individually (no
                ! concatenation across projects), using the fixed bin selection, skipping the
                ! Z-score bin-acceptance determination. Once a project has been picked, all of
                ! its project_list records are flagged (%included = .true.) so it is never
                ! re-processed on a later iteration.
                call simple_mkdir(DIR_STREAM)
                call simple_mkdir(DIR_STREAM//'all')
                call simple_getcwd(cwd_cycle)
                write(logfhandle,'(A,I0)') '>>> PICKING AND EXTRACTING ALL UNPROCESSED PROJECTS IN project_list; # INCLUDED : ', count(project_list%get_included_flags())
                if( project_list%size() > 0 .and. count(project_list%get_included_flags()) < NMICS_PLAN(2) ) then
                    do iproj = 1, project_list%size()
                        ...
                        proj_local      = cwd_cycle//'/'//string(DIR_STREAM)//'all/'//int2str_pad(extract_counter, 5)//string(METADATA_EXT)
                        ...
                        call spproj_part%read(proj_local)
                        nmics = spproj_part%os_mic%get_noris()
                        if( nmics == 0 )then
                        ...
                        call segdiampick_mics_multi_fixed_bins(spproj_part, params%pcontrast, nmics, params%moldiam_max, box_in_pix, accepted_bins)
                        call simple_chdir(cwd_cycle)
                        call send_meta(string('extracting particles'))
                        call start_extract(spproj_part, proj_local, string(DIR_EXTRACT//'all'), box_in_pix, part=extract_counter)
                        call send_meta(string('complete'))
                        call spproj_part%kill()
                        ! flag all of this project's records in project_list as picked, to avoid replication
                        do i = 1, project_list%size()
                            call project_list%at(i, prec)
                            if( prec%included ) cycle
                            if( prec%projname%to_char() /= cur_projname%to_char() ) cycle
                            prec%included = .true.
                            prec%projname = proj_local
                            call project_list%replace_at(i, prec)
                        end do
                    end do
                end if
                do iextract = 1, extract_counter
                    sentinel_file = DIR_EXTRACT//'all'//'/JOB_FINISHED_'//int2str(iextract)
                    if( file_exists(sentinel_file) ) then
                        ! stage complete
                        proj_local = cwd_cycle//'/'//string(DIR_STREAM)//'all/'//int2str_pad(iextract, 5)//string(METADATA_EXT)
                        call finish_extract(spproj_part, proj_local, string(DIR_EXTRACT//'all'), box_in_pix)
                        ! one extracted_prec per micrograph (mirroring project_list's id/projname/
                        ! micind/nptcls convention, see import_new_projects), so that downstream
                        ...
                        do imic = 1, spproj_part%os_mic%get_noris()
                            ...
                            extracted_prec%nptcls     = spproj_part%os_mic%get_int(imic, 'nptcls')
                            extracted_prec%nptcls_sel = extracted_prec%nptcls
                            extracted_prec%included   = .false.
                            call extracted_project_list%push_back(extracted_prec)
                        end do
                        ! remove the sentinel so this extraction is never processed again
                        call del_file(sentinel_file)
                ...
                if( l_sieve_init ) then
                    call sieve%cycle(extracted_project_list)
                    if( count(project_list%get_included_flags()) >= NMICS_PLAN(2) .and. n_extract_done == extract_counter .and. cycle_plan(2) == 0 ) then
            ...
            ! cycle 1 is different to later cycles
            if( n_cycles == 1 ) then
                ! cycle 1 stage 0: import micrographs until params%nmics is reached
                call simple_getcwd(cwd_cycle)
                cycle_projfile = cwd_cycle//'/'//string(DIR_STREAM)//'init/init'//METADATA_EXT
                if( cycle_plan(n_cycles) == 0 ) then
                    ...
                    call simple_mkdir(cwd_cycle//'/'//string(DIR_STREAM))
                    call simple_mkdir(cwd_cycle//'/'//string(DIR_STREAM)//'init')
                ...
                ! cycle 1 stage 1: segmentation-based picking and particle extraction
                if( cycle_plan(n_cycles) == 1 ) then
                    if( n_mics_imported > params%nmics ) then
                        ...
                        call segdiampick_mics_multi(spproj, params%pcontrast, nmics, params%moldiam_max, box_in_pix, mskdiam, accepted_bins=accepted_bins)
                        ! send the NTHUMB_MAX most recent micrograph thumbnails to the GUI
                ...
                ! cycle 1 stage 2: send picking thumbnails to GUI and extract particles
                if( cycle_plan(n_cycles) == 2 ) then
                    if( cycle_plan_status(n_cycles) == 0 ) then
                        ! stage start
                        call send_meta(string('extracting particles'))
                        call start_extract(spproj, cycle_projfile, string(DIR_EXTRACT//'init'), box_in_pix, part=1)
                        ...
                        if( file_exists(DIR_EXTRACT//'init'//'/JOB_FINISHED_1') ) then
                            write(logfhandle,'(A)') '>>> PARTICLE EXTRACTION COMPLETE'
                            ! stage complete
                            call finish_extract(spproj, cycle_projfile, string(DIR_EXTRACT//'init'), box_in_pix)
                            call send_meta(string('complete'))
        ...
        contains
            ...
            subroutine start_extract( spproj_inout, cluster_projfile, outdir, box, part )
                type(sp_project), intent(inout) :: spproj_inout
                type(string), intent(in) :: cluster_projfile
                type(string), intent(in) :: outdir
                integer,      intent(in) :: box, part
                type(string)             :: cwd, cwd_extract, server_address
                call simple_getcwd(cwd)
                call simple_mkdir(outdir)
                call simple_chdir(outdir)
                call simple_getcwd(cwd_extract)
                CWD_GLOB       = cwd_extract%to_char()
                ...
                call cline_extract%set('nthr',                     4)
                call cline_extract%set('nparts',                   1)
                call cline_extract%set('part',                  part)
                call cline_extract%set('mkdir',                 'no')
                call cline_extract%set('dir',                    '.')
                call cline_extract%set('stream',               'yes')
                call cline_extract%set('fromp',                    1)
                call cline_extract%set('top',   spproj_inout%os_mic%get_noris())
                call cline_extract%set('projfile',  cluster_projfile)
                call cline_extract%set('worker_priority',     'high')
                if( server_address%strlen() > 0 ) call cline_extract%set('worker_server', server_address)
                call cline_extract%printline()
                call qenv%exec_simple_prg_in_queue_async( cline_extract, string('./distr_extract_')//int2str(part), string('simple_log_extract_')//int2str(part) )
                call simple_chdir(cwd)
                CWD_GLOB = cwd%to_char()
            end subroutine start_extract

            subroutine finish_extract( spproj_inout, cluster_projfile, outdir, box )
                type(sp_project), intent(inout) :: spproj_inout
                type(string), intent(in) :: cluster_projfile
                type(string), intent(in) :: outdir
            ...
            ! Evaluate class-average quality, map accepted classes, and emit GUI-ready cavgs.
            subroutine run_cavg_quality_selection_2(spproj_inout, cluster_projfile, outdir, cwd_cycle_in, mskdiam_inout, imgfiles_inout, smpd_stk_out, n_selected )
                type(string),     allocatable, intent(inout) :: imgfiles_inout(:)
                type(sp_project),              intent(inout) :: spproj_inout
                real,                          intent(inout) :: mskdiam_inout
                type(string),                  intent(in)    :: cluster_projfile
                ...
                call simple_mkdir(outdir)
                call simple_chdir(outdir)
                call spproj_inout%get_cavgs_stk(cavgsstk_local, ncls_local, smpd_stk_out, fail=.false.)
                if( ncls_local <= 0 )then
                ...
                call spproj_inout%write(cluster_projfile) ! write project with quality-selected classes mapped in, for downstream steps
                call simple_chdir(cwd)
            end subroutine run_cavg_quality_selection_2
            ...
            subroutine balance_classes( spproj_inout, cluster_projfile, outdir )
                ...
                call spproj_inout%get_cavgs_stk(cavgsstk, ncls_all, smpd_dummy, out_ind=out_ind, fail=.false.)
                if( ncls_all <= 0 ) then
