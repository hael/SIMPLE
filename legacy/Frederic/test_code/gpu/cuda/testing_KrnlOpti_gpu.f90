! ============================================================================
! Name        : testing_KrnlOpti_gpu.f90
! Author      : Frederic Bonnet
! Version     :
! Date        : 20th of September 2016
! Description : tests environment for optimisation of kernels
!             : for Mike(NVidia)
! ============================================================================
!
program testing_KrnlOpti_gpu
  use simple_defs
...
!  type(cuda)                    :: cudaQ
  ...
  integer :: get_polarft_krnl_opti_gpu_c
...
!*******************************************************************************
     ...
     write(*,'(7(3x,i5),5x,i3,x,i3,x,i3,8x,i3,8x,i2)') &
          ...
          s_polar%ikrnl
     call start_timer_cpu("corr_ON_gpu")
     call gettimeofday_c(st_ZM_r)
     err = get_polarft_krnl_opti_gpu_c(a_devD,                     &
                                        s_polar,"O","N",            &
     ...
     call stop_timer_cpu("corr_ON_gpu")
     if (err /= RC_SUCCESS ) write(*,*) "get_polarft_krnl_opti_gpu_c_ = -1"
     !$omp parallel default(shared) private(iptcl)
     !$omp workshare
...
end program testing_KrnlOpti_gpu
!//////////////////// Helper subroutine and functions //////////////////////////
!//
!*******************************************************************************
