...
contains
    subroutine batch_fft_2D( even, odd, in, out, plan_fwd )
        ...
        do k = 1, ldim(1)
            do l = 1, ldim(2)
                in(k,l,1) = peven%rmat(k,l,1)
                in(k,l,2) =  podd%rmat(k,l,1)
            enddo
        enddo
        !$omp end parallel do
        ...
        do k = 1, ldim(1)/2
            do l = 1, ldim(2)
                if( mod(k+l,2) == 1 )then
                    peven%cmat(k,l,1) = -out(k,l,1)/product(ldim(1:2))
                    podd %cmat(k,l,1) = -out(k,l,2)/product(ldim(1:2))
                else
                    peven%cmat(k,l,1) =  out(k,l,1)/product(ldim(1:2))
                    podd %cmat(k,l,1) =  out(k,l,2)/product(ldim(1:2))
                endif
