...
!     real, allocatable    :: inpl_corrs(:) !< all in-plane correlations
!     integer, allocatable :: inpl_inds(:)  !< in-plane indices
...
type prime_srch
    ...
    integer                      :: sh         = 0        !< int(trs)
    integer                      :: angstep    = 0        !< discrete angular step size for in-plane weighting
...
!     integer, allocatable         :: inpl_inds(:)        !< in-plane indices for reference
    ...
    logical                      :: corrmat_set = .true.  !< indicates whether the correlation matrix has been set or not
...
contains
    ...
    subroutine new( self, e, p )
        ...
        allocate( self%proj_space(self%nrefs), self%refinds(self%nstates,self%nprojs),&
                  ...
                  self%angtab(self%nrots), stat=alloc_stat ) !  self%inpl_inds(self%nrots)
...
!             self%inpl_inds(i) = i
        ...
        x   = 0.1
        ang = 0.
        do while( ang <= self%atres )
            ang = x*rad2deg(dang)
            x = x+0.1
        end do
        self%angstep = max(1,nint(x/2.))
...
!                 if( allocated(self%proj_space(cnt)%inpl_corrs) ) deallocate(self%proj_space(cnt)%inpl_corrs)
!                 if( allocated(self%proj_space(cnt)%inpl_inds) )  deallocate(self%proj_space(cnt)%inpl_inds)
!                 allocate(self%proj_space(cnt)%inpl_corrs(self%nrots), stat=alloc_stat)
!                           self%proj_space(cnt)%inpl_inds(self%nrots), stat=alloc_stat )
!                 self%proj_space(cnt)%inpl_corrs = -1.
!                 self%proj_space(cnt)%inpl_inds  = 0
                self%proj_space(cnt)%inpl_corr = -1.
                self%proj_space(cnt)%inpl_ind  = 0
!                 call alloc_err( 'In: init; simple_prime_srch', alloc_stat )
            end do
        end do
    ...
    subroutine get_ori_best( self, o )
        ...
        if( self%refine .eq. 'shift' )then
            ...
            call o%set('x',x)
            call o%set('y',y)
!             corr = self%proj_space(ref)%inpl_corrs(self%nrots)
            corr = self%proj_space(ref)%inpl_corr
            call o%set('corr', corr)
        else
            ! get new indices
            proj_new  = self%proj_space(self%proj_space_inds(self%nrefs))%proj
!             rot_new   = self%proj_space(self%proj_space_inds(self%nrefs))%inpl_inds(self%nrots)
            ...
            call o%set('x',x)
            call o%set('y',y)
!             corr = self%proj_space(self%proj_space_inds(self%nrefs))%inpl_corrs(self%nrots)
...
!         euls(3) = 360.-self%calc_rot(self%proj_space(ref)%inpl_inds(self%nrots)) ! change sgn to fit convention
...
!         call o%set('corr',self%proj_space(ref)%inpl_corrs(self%nrots))
...
!             allocate( best_ones(cnt)%inpl_corrs(self%nrots),&
!                       best_ones(cnt)%inpl_inds(self%nrots), stat=alloc_stat )
!             call alloc_err( 'In: ang_sdev; simple_prime_srch', alloc_stat )
...
!         do i=self%nrefs,self%nrefs-self%npeaks+1,-1
!             cnt = cnt+1
!             deallocate(best_ones(cnt)%inpl_corrs)
!             deallocate(best_ones(cnt)%inpl_inds)
!         end do
    ...
    subroutine exec_prime_srch( self, pftcc, ptcl, lp, wcorr, o, nnmat )
        use simple_ori, only: ori
        class(prime_srch), intent(inout)       :: self
        ...
        call self%prep4srch(pftcc, ptcl, lp, o)
        call self%stochastic_srch(pftcc, ptcl, nnmat)
        call self%stochastic_weights(wcorr)
        if( debug ) write(*,'(A)') '>>> PRIME_SRCH::EXECUTED PRIME_SRCH'
    end subroutine
    
    !>  \brief a master prime search routine
    subroutine exec_shift_srch( self, pftcc, ptcl, lp, o )
        use simple_ori, only: ori
        class(prime_srch), intent(inout)       :: self
        class(polarft_corrcalc), intent(inout) :: pftcc
        integer, intent(in)                    :: ptcl
        ...
        class(ori), intent(inout)              :: o
        call self%prep4srch(pftcc, ptcl, lp, o)
        call self%shift_srch(pftcc, ptcl)
        if( debug ) write(*,'(A)') '>>> PRIME_SRCH::EXECUTED SHIFT_SRCH'
    end subroutine

    !>  \brief  prepares for the search
    subroutine prep4srch( self, pftcc, ptcl, lp, o_prev )
        use simple_ori, only: ori
        class(prime_srch), intent(inout)       :: self
        class(polarft_corrcalc), intent(inout) :: pftcc
        integer, intent(in)                    :: ptcl
        ...
        if( present(o_prev) )then
            ...
            cc_t = pftcc%corr(self%ref, ptcl, self%rot)
    ...
    subroutine gencorrs_all( self, pftcc )
        ...
        self%corrmat_set = .true.
    end subroutine
    
    !>  \brief  executes the stochastic rotational search
    subroutine stochastic_srch( self, pftcc, ptcl, nnmat )
        class(prime_srch), intent(inout)       :: self
        class(polarft_corrcalc), intent(inout) :: pftcc
        integer, intent(in)                    :: ptcl
        integer, intent(in), optional          :: nnmat(self%nrefs,self%nnn)
        ...
        do i=1,endit
            ...
            if( self%test .eq. 'yes' )then
                if( self%corrmat_set )then
                    ptcl_ind = ptcl-self%fromp+1
                    self%proj_space(ref)%inpl_ind  = self%inplmat(ptcl_ind,ref)
                    self%proj_space(ref)%inpl_corr = self%corrmat(ptcl_ind,ref)
                else
                    stop 'corrmat need to be set for this mode of execution; stochastic_srch; simple_prime_srch'
                endif
            else
                ! we don't use shifts here because the particle images are pre-shifted
                ! when the pftcc object is filled-up
                corrs = pftcc%gencorrs(ref, ptcl)
                loc   = maxloc(corrs)
    !             self%proj_space(ref)%inpl_corrs = pftcc%gencorrs(ref, ptcl)
                ! set the stochastic index in the index array that will be sorted
                ! copy the in-plane rotation index
    !             self%proj_space(ref)%inpl_inds = self%inpl_inds
                self%proj_space(ref)%inpl_ind  = loc(1)
                self%proj_space(ref)%inpl_corr = corrs(loc(1))
            endif
            ! sort in the plane
!             call hpsort(self%nrots, self%proj_space(ref)%inpl_corrs,&
!                                     self%proj_space(ref)%inpl_inds)
            ! update the correlation
!             self%proj_space_corrs(ref) = self%proj_space(ref)%inpl_corrs(self%nrots)
        ...
        do i=self%nrefs,self%nrefs-self%npeaks+1,-1
            ref = self%proj_space_inds(i)
            if( self%doshift )then
!                 call pftcc_shsrch_set_indices(ref, ptcl, self%proj_space(ref)%inpl_inds(self%nrots))
                call pftcc_shsrch_set_indices(ref, ptcl, self%proj_space(ref)%inpl_ind)
    ...
    subroutine shift_srch( self, pftcc, ptcl )
        class(prime_srch), intent(inout)       :: self
        class(polarft_corrcalc), intent(inout) :: pftcc
        integer, intent(in)                    :: ptcl
        ...
        call pftcc_shsrch_set_indices(self%ref, ptcl, self%rot)
        cxy = pftcc_shsrch_minimize()
