      ...
      contains
        ...
        subroutine inirefs
            ...
            if( .not.cline%defined('refs') .and. (.not.l_inpl) )then
                ...
                if( trim(params%ref_type).eq.'cavgvol' )then
            ...
            if( .not.file_exists(refs) ) THROW_HARD('File does not exits: '//trim(refs))
            if( ncls /= params%ncls )    THROW_HARD('Incompatible # of classes in: '//trim(refs))
            ...
            params%refs      = 'start2Drefs'//params%ext    ! initial references
        ...
        subroutine set_lplims
            ...
            character(len=:), allocatable :: frcs
        ...
        subroutine set_cline_cluster2D( istage )
            ...
            if( trim(params%polar).eq.'yes')then
                if( trim(params%ref_type)=='cavgvol') center = 'no' ! because the references form a volume
            endif
