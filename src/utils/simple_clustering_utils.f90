...
contains
    ...
    real function line_sim( img1, img2 )
        ...
        type(image), intent(in)  :: img2
        real,        allocatable :: sigma2_noise(:,:)
        type(polarft_corrcalc)   :: pftcc
        type(pftcc_shsrch_grad)  :: grad_shsrch_obj
        type(polarizer)          :: polar_img
        type(image)              :: img1_copy, img2_copy
        real    :: cxy(3), lims(2,2)
        integer :: irot
        call pftcc%new(1, [1,1], params_glob%kfromto)
        allocate( sigma2_noise(params_glob%kfromto(1):params_glob%kfromto(2), 1), source=1. )
        call pftcc%assign_sigma2_noise(sigma2_noise)
        call polar_img%new([params_glob%box_crop,params_glob%box_crop,1],params_glob%smpd_crop)
        call polar_img%init_polarizer(pftcc, params_glob%alpha)
        call img1_copy%copy(img1)
        call img2_copy%copy(img2)
        call img1_copy%norm
        call img2_copy%norm
        call img1_copy%fft
        call img2_copy%fft
        call img1_copy%clip_inplace([params_glob%box_crop,params_glob%box_crop,1])
        call img2_copy%clip_inplace([params_glob%box_crop,params_glob%box_crop,1])
        call polar_img%polarize(pftcc, img1_copy, 1, isptcl=.false., iseven=.true., mask=build_glob%l_resmsk)
        call polar_img%polarize(pftcc, img2_copy, 1, isptcl=.true.,  iseven=.true., mask=build_glob%l_resmsk)
        call pftcc%set_with_ctf(.false.)
        call pftcc%memoize_refs
        call pftcc%memoize_sqsum_ptcl(1)
        call pftcc%memoize_ptcls
        ! aligning img1 to img2
        lims(1:2,1) = -params_glob%trs
        lims(1:2,2) =  params_glob%trs
        call grad_shsrch_obj%new(lims, opt_angle=.true.)
        call grad_shsrch_obj%set_indices(1, 1)
        irot = 1
        cxy  = grad_shsrch_obj%minimize(irot)
        call pftcc%rotate_iptcl(1, irot, cxy(2:3))
        line_sim = pftcc%bestline_sim(1, 1)
    end function line_sim
