...
contains
    ...
    subroutine set_common_params
        ...
        &'low-pass FSC criterion(0.143-0.5){0.3}', .false., 0.5)
        ...
        call set_param(envfsc,         'envfsc',       'binary', 'Envelope mask e/o maps for FSC', 'Envelope mask even/odd pairs prior to FSC calculation(yes|no){no}',  '(yes|no){no}',  .false., 'yes')
    ...
    subroutine new_refine3D
        ...
        call refine3D%set_input('srch_ctrls', 10, 'refine', 'multi', 'Refinement mode', 'Refinement mode(shc|greedy|neigh|cont|cluster|clustersym){shc}', '(snhc|shc|greedy|neigh|cluster|clustersym){shc}', .false., 'single')
