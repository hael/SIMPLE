...
contains
    ...
    subroutine new_1( self )
        class(hash_dp), intent(inout) :: self
        call self%kill
    ...
    subroutine new_2( self, sz, buffsz )
        class(hash_dp),       intent(inout) :: self   !< instance
        integer,              intent(in)    :: sz     !< initial size
        integer, optional,    intent(in)    :: buffsz !< size of subsequent allocation increments
        call self%kill
    ...
    subroutine realloc_hash( self )
        ...
        if( self%exists )then
            ...
            stop 'cannot reallocate non-existent hash; simple_hash_dp :: realloc_hash'
        endif
    end subroutine realloc_hash

    !>  \brief  copies a hash instance
    subroutine copy( self_out, self_in )
        class(hash_dp), intent(inout) :: self_out
        class(hash_dp), intent(in)    :: self_in
        integer :: i, sz
