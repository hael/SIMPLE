...
program testing_dgetri_cpu
  ...
  complex(dp),allocatable            :: matSymDE_cpu(:,:)
  complex(dp),allocatable            :: invSymDA(:,:)  !inverted sym Z matrix

  !checking the matrix inversion has work correctly
  complex(dp),allocatable            :: matSymDF_cpu(:,:)  !matrix multiplication 
  ...
  write(*,*)'                                                               '
  write(*,*)'***************************************************************'
  write(*,*)'     Now testing the zgetri and zgemm Lapacks (CPU)            '
  write(*,*)'***************************************************************'
  ...
  do inmx = start_nmx, nmx, istep
     ...
     call getSymZRandomMat_cpu(inmx,lda,matSymDE_cpu)
...
!     end do
    ...
    ! now using the magma to get iunverse matrix
     ...
     call getBlockInverseZMatrix_cpu(inmx,inmx,lda,size_of_elt,inmx,matSymDE_cpu,invSymDA)
...
!     end do
     ...
     call start_timer_cpu("zz_zgemm")
     call zgemm("N", "N", inmx, inmx, inmx, alpha, matSymDE_cpu, &
          lda, invSymDA, ldb, beta, matSymDF_cpu, ldc)
     call stop_timer_cpu("zz_zgemm")

     write(*,'(1x,a,1x,i5,1x,a,2f20.8)')"When N =",inmx,&
          "--> Double complex precision Full sum(A * A^{-1}): ",sum(matSymDF_cpu)
