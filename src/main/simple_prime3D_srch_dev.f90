...
type prime3D_srch
    ...
    class(sym),              pointer :: se_ptr    => null()      !< pointer to b%e (reference orientations)
...
contains
    ...
    subroutine new( self, iptcl, pftcc, a, e, p, se )
        ...
        class(sym),              target, intent(in)    :: se     !< symmetry operator
    ...
    subroutine stochastic_srch_het( self, extr_bound, statecnt)
        ...
        if( self%prev_state > 0 )then
            if( .not. state_exists(self%prev_state) ) stop 'empty previous state; stochastic_srch_het; simple_prime3D_srch'
            ! initialize
    ...
    subroutine prep4srch( self, lp, nnmat, target_projs )
        ...
        select case( self%refine )
            case( 'no','shc','snhc','greedy')                               ! DISCRETE CASE
                self%prev_ref = o_refs(self%iptcl)%find_closest_proj(o_prev, self%prev_state) ! find closest ori with same state
