!==============================================================================
! MODULE: simple_qsys_worker_server
!
! PURPOSE:
...
!   4.  pack_task_queue() is called only when not in shutdown mode to
!       avoid unnecessary work during teardown.
...
!   simple_qsys_worker_message_*   — all wire message types and type enumerator
...
module simple_qsys_worker_server
    ...
    use simple_qsys_worker_message_types,     only: WORKER_TASK_MSG, WORKER_STATUS_MSG, WORKER_TERMINATE_MSG, WORKER_HEARTBEAT_MSG
    ...
    type qsys_worker_data
        ...
        logical :: l_debug     = .true. !< when .true. the listener emits verbose heartbeat / skip / idle log lines
        !> Latest heartbeat from each worker; indexed by worker_id (1..MAX_WORKERS)
        type(qsys_worker_message_heartbeat) :: workers(MAX_WORKERS)
        !> High-priority task queue; slots are active while job_id > 0 and end_time == 0
        type(qsys_worker_message_task) :: tasks_priority_high(TASK_QUEUE_SIZE)
        !> Normal-priority task queue
        type(qsys_worker_message_task) :: tasks_priority_norm(TASK_QUEUE_SIZE)
        !> Low-priority task queue
        type(qsys_worker_message_task) :: tasks_priority_low(TASK_QUEUE_SIZE)
    end type qsys_worker_data
    ...
    type qsys_worker_server
        ...
        logical                         :: l_debug      = .true.  !< enable verbose debug logging (propagated to worker_data)
        ...
        type(qsys_worker_data), pointer :: worker_data   => null() !< shared state (workers + queues); mutex-protected
    contains
...
contains
    ...
    !> \param[in] nthr_workers  number of worker thread slots to support
    subroutine new( self, nthr_workers )
        class(qsys_worker_server), intent(inout) :: self
        integer,                   intent(in)    :: nthr_workers
        integer(kind=c_int)                      :: rc
        if( associated(self%worker_data) .or. associated(self%listener_args) ) then
            write(logfhandle,'(A)') '>>> QSYS_WORKER_SERVER new: listener already running; no-op'
            return
        end if
        ...
        if( rc /= 0 ) then
            write(logfhandle,'(A,I0)') '>>> QSYS_WORKER_SERVER new: c_pthread_mutex_init failed, rc=', rc
    ...
    subroutine kill( self )
        ...
        self%nthr_workers = 0
        self%job_count    = 0
        call self%host_ips%kill() ! deallocate host_ips string if allocated
        ...
        ! Wait 2 s to allow workers to receive TERMINATE before we close the socket.
        ...
        ! been joined (which kill() -> join_listener() guarantees above).
        if( associated(self%listener_args) ) then
            rc = c_pthread_mutex_destroy(self%listener_args%mutex)
        end if
        if( associated(self%worker_data) ) then
            deallocate(self%worker_data)
            nullify(self%worker_data)
        end if
        if( associated(self%listener_args) ) then
    ...
    function queue_task( self, task, priority ) result( queued )
        class(qsys_worker_server),      intent(inout) :: self
        type(qsys_worker_message_task), intent(inout) :: task
        type(string),                   intent(in)    :: priority
        logical                                       :: queued
        integer(kind=c_int) :: rc
        integer             :: i
        queued = .false.
        if( .not. associated(self%worker_data) ) then
            write(logfhandle,'(A)') '>>> QSYS_WORKER_SERVER queue_task: server not initialised; dropping task'
            return
        end if
        rc = c_pthread_mutex_lock(self%listener_args%mutex)
        ...
        select case( priority%to_char() )
            case('high')
                do i = 1, size(self%worker_data%tasks_priority_high)
                    if( self%worker_data%tasks_priority_high(i)%job_id == 0 ) then
                        ...
                        queued = .true.
                        write(logfhandle,'(A,A,A,A)') '>>> QSYS_WORKER_SERVER queued job_id ', &
                            int2str(task%job_id), ' in high-priority slot ', int2str(i)
                        exit
                    end if
                end do
                if( .not. queued ) write(logfhandle,'(A,A)') '>>> QSYS_WORKER_SERVER high-priority queue full; dropped job_id ', int2str(task%job_id)
            case('low')
                ...
                if( .not. queued ) write(logfhandle,'(A,A)') '>>> QSYS_WORKER_SERVER low-priority queue full; dropped job_id ', int2str(task%job_id)
            case default  ! 'norm' or anything unrecognised
                ...
                if( .not. queued ) write(logfhandle,'(A,A)') '>>> QSYS_WORKER_SERVER normal-priority queue full; dropped job_id ', int2str(task%job_id)
        end select
        rc = c_pthread_mutex_unlock(self%listener_args%mutex)
    ...
    function get_port( self ) result ( port )
        class(qsys_worker_server), intent(in)  :: self
        integer                                :: port
        port = self%port
    ...
    !> Accept-loop pthread.  Receives messages from workers, updates the
    !> shared worker_status, and dispatches tasks or TERMINATE replies.
    !>
    ...
    subroutine worker_listener_thread( arg_ptr ) bind(c)
        ...
        type(qsys_worker_data),         pointer :: status
        type(qsys_worker_message_heartbeat)     :: heartbeat_msg
        type(qsys_worker_message_status)        :: status_msg
        type(qsys_worker_message_task)          :: dispatch_task
        type(qsys_worker_message_terminate)     :: terminate_msg
        ...
        if( .not. associated(status) ) then
            ...
            write(logfhandle,'(A)') '>>> QSYS_WORKER_SERVER worker_listener_thread: status pointer null — aborting thread'
        ...
        do
            ...
            select case(buffer_type)
                ...
                case(WORKER_TERMINATE_MSG)
                    write(logfhandle,'(A)') '>>> QSYS_WORKER_SERVER worker_listener_thread: received TERMINATE — exiting'
                ...
                case(WORKER_HEARTBEAT_MSG)
                    call dispatch_task%new() ! zero dispatch_task for use in the critical section below
                    call terminate_msg%new() ! zero terminate_msg for use in the critical section below
                    ...
                    if( worker_id >= 1 .and. worker_id <= size(status%workers) ) then
                        ...
                        if( status%workers(worker_id)%worker_uid /= '' .and. status%workers(worker_id)%worker_uid /= heartbeat_msg%worker_uid ) then
                            write(logfhandle,'(A,A)') '>>> QSYS_WORKER_SERVER worker_listener_thread: ignoring changed worker_uid for worker_id ', &
                                   int2str(worker_id)
                            send_terminate = .true. ! send TERMINATE — worker identity changed (possible crash/restart at same slot)
                        else
                            status%workers(worker_id) = heartbeat_msg
                        end if
                    else
                        write(logfhandle,'(A,A)') '>>> QSYS_WORKER_SERVER worker_listener_thread: ignoring out-of-range worker_id ', &
                                   int2str(worker_id)
                        send_terminate = .true. ! send TERMINATE — worker_id is out of range
                    end if
                    if( status%l_debug ) &
                        write(logfhandle,'(A,A,A,A,A,A,A)') '>>> QSYS_WORKER_SERVER heartbeat worker ', int2str(worker_id), &
                        ...
                        do i = 1, size(status%tasks_priority_high)
                            ...
                            if( status%tasks_priority_high(i)%job_id > 0 .and. &
                                ...
                                if( heartbeat_msg%nthr_total - heartbeat_msg%nthr_used < &
                                    ...
                                    if( status%l_debug ) &
                                        write(logfhandle,'(A,A,A,A,A)') '>>> QSYS_WORKER_SERVER high task ', &
                        ...
                        do i = 1, size(status%tasks_priority_norm)
                            ...
                            if( status%tasks_priority_norm(i)%job_id > 0 .and. &
                                ...
                                if( heartbeat_msg%nthr_total - heartbeat_msg%nthr_used < &
                                    ...
                                    if( status%l_debug ) &
                                        write(logfhandle,'(A,A,A,A,A)') '>>> QSYS_WORKER_SERVER norm task ', &
                        ...
                        do i = 1, size(status%tasks_priority_low)
                            ...
                            if( status%tasks_priority_low(i)%job_id > 0 .and. &
                                ...
                                if( heartbeat_msg%nthr_total - heartbeat_msg%nthr_used < &
                                    ...
                                    if( status%l_debug ) &
                                        write(logfhandle,'(A,A,A,A,A)') '>>> QSYS_WORKER_SERVER low task ', &
                    ...
                    ! Compact all three queues before releasing the lock (skip during shutdown)
                    if( .not. send_terminate ) then
                    ...
                    ! Blocking network reply is now outside the mutex
                    if( send_terminate ) then
                        write(logfhandle,'(A,A)') '>>> QSYS_WORKER_SERVER sending TERMINATE to worker ', int2str(worker_id)
                        ...
                        write(logfhandle,'(A,A,A,A)') '>>> QSYS_WORKER_SERVER dispatching job_id ', int2str(dispatch_task%job_id), &
                ...
                case default
                    write(logfhandle,'(A,A)') '>>> QSYS_WORKER_SERVER worker_listener_thread: unknown message type: ', int2str(buffer_type)
            ...
            if( .not. replied ) then
                if( status%l_debug ) then
                    if( worker_id > 0 ) then
                        write(logfhandle,'(A,A)') '>>> QSYS_WORKER_SERVER no tasks available for worker ', int2str(worker_id)
                    else
                        write(logfhandle,'(A)') '>>> QSYS_WORKER_SERVER no tasks available (non-heartbeat or unknown worker)'
                    end if
                end if
                call status_msg%new()
    ...
    contains
        ...
        subroutine pack_task_queue( queue )
            type(qsys_worker_message_task), intent(inout) :: queue(:)
            integer :: ii, jj
            jj = 0
            do ii = 1, size(queue)
            ...
            do ii = jj + 1, size(queue)
                queue(ii) = qsys_worker_message_task()  ! reset to default (job_id=0)
            end do
        end subroutine pack_task_queue

    end subroutine worker_listener_thread

end module simple_qsys_worker_server