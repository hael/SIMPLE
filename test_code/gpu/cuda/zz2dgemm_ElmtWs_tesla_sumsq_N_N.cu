...
/*
 *Kernel that takes element wise using normal copy
 * C = alpha*A*B + beta*C
*/
extern "C" __global__ void
main_zz2dgemm_kernel_sumsq_N_N( double *C, 
				...
				double alpha, double beta)
{
  ...
  C[i * ldc + j ] = cuDmul(alpha,cuReCCstar( A[ i * lda + j] ));
...
*/
extern "C" __global__ void
shared_zz2dgemm_kernel_sumsq_N_N( double *C, 
				const cuDoubleComplex *A, 
				int m, int n, int k,
				int lda, int ldb, int ldc,
				double alpha, double beta)
{
  ...
  C_tile[threadIdx.x][threadIdx.y] = cuDmul(alpha,cuReCCstar( A[ i * lda + j] ));
...
extern "C" int
zz2dgemm_kernel_sumsq_N_N( double *C, 
			   const cuDoubleComplex *A, 
			   int m, int n, int k,
			   int lda, int ldb, int ldc, 
			   double alpha, double beta)
{
  ...
  shared_zz2dgemm_kernel_sumsq_N_N<<< grid, threads >>>(C, A,
							m, n, k,
							lda, ldb, ldc,
							alpha, beta);
