...
void CPlot2D::AddDataSet(std::vector<RFLOAT> xValues, std::vector<RFLOAT> yValues)
{
    ...
    {
      std::cerr << "ERROR: xValues and yValues vectors do not have identical sizes." << std::endl;
    }
...
{
  This->SetDatasetColor(r, g, b);
}

CDataPoint* CDataPoint__new2(double x, double y)
{
  return new CDataPoint(x, y);
}
