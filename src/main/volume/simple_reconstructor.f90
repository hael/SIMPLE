...
public :: reconstructor, insert_plane_oversamp_multi_scaled, insert_plane_oversamp_coupled_scaled, project_fplanes_mean_basis
private
...
contains
            ...
            do l = 0, stride-1
                ...
                do h = fpllims(1,1)+l, fpllims(1,2), stride
                    ...
                    do k = k_lo, k_hi
                        ...
                        win(2,:) = win(1,:) + iwinsz
                        win(1,:) = win(1,:) - iwinsz
                        if( win(2,1) < recs(1)%lims(1,1) ) cycle
    ...
    contains
        ...
        subroutine kb_apod_vecs_3d_fast( loc, wx, wy, wz )
            ...
            base   = real(win_lo) - loc
            do i = 1, recs(1)%wdim
                ww3   = recs(1)%kbwin%apod_fast(base + real(i-1))
                wx(i) = ww3(1)
                wy(i) = ww3(2)
                wz(i) = ww3(3)
            end do
    ...
    subroutine insert_plane_oversamp_coupled_scaled( recs, rho_cross_exp, se, o, fpl, data_scales, density_scales )
        ...
        integer   :: exp_lb(3), exp_shape(3), npairs
        real      :: pf2, eps_norm, inv_wdim
        ncomp = size(recs)
        if( ncomp <= 0 ) return
    ...
    subroutine project_fplanes_mean_basis( mean_rec, basis_recs, o, fpl_ref, mean_fpl, basis_fpls, apply_ctf_amp )
        ...
        real    :: wx(mean_rec%wdim), wy(mean_rec%wdim), wz(mean_rec%wdim)
    ...
    contains
        ...
        subroutine projection_weights( loc, win, wx, wy, wz )
            ...
            win_lo   = win(1,:)
            base     = real(win_lo) - loc
            do i = 1, mean_rec%wdim
                ww3   = mean_rec%kbwin%apod_fast(base + real(i-1))
                wx(i) = ww3(1)
                wy(i) = ww3(2)
                wz(i) = ww3(3)
            end do
            ...
            sy        = sum(wy)
            sz        = sum(wz)
            inv_wdim  = 1.0 / real(mean_rec%wdim)
            eps_norm  = epsilon(1.0)
            if( abs(sx) > eps_norm )then
                wx = wx * (1.0 / sx)
            else
