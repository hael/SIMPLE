module pose_cont_refinement_recovery_test
...
subroutine run_pose_recovery()
    ...
    type(pose_refinement_summary) :: batch_summary, serial_summary, parallel_summary
    ...
    call refine_fixed_volume_poses(workspace,batch_rotmats,batch_observed,batch_shifts, &
    ...
    call refine_fixed_volume_poses(workspace,serial_rotmats,equivalence_observed,serial_shifts, &
    ...
    call refine_fixed_volume_poses(workspace,parallel_rotmats,equivalence_observed,parallel_shifts, &
    ...
    write(*,'(a,4(es14.6,1x))') 'POSE_CONT_REFINEMENT initial/final rotation/shift errors: ', &
        &initial_rotation_error,final_rotation_error,initial_shift_error,final_shift_error
    write(*,'(a,2(es14.6,1x),3(i0,1x))') 'POSE_CONT_REFINEMENT max rotation/shift, accepted/tried/switches: ', &
        &recovery_max_rotation_step,recovery_max_shift_step,recovery_naccepted,recovery_nattempted,recovery_nswitches
    write(*,'(a,2(i0,1x))') 'POSE_CONT_REFINEMENT serial/parallel threads: ',1,parallel_nthreads
    write(*,'(a)') 'POSE_CONT_REFINEMENT_RECOVERY: PASS'
...
subroutine assert_equivalent_summaries(serial_summary,parallel_summary)
    type(pose_refinement_summary), intent(in) :: serial_summary, parallel_summary
...
end module pose_cont_refinement_recovery_test
